...
**DreamGen: Unlocking Generalization in Robot Learning through Video World Models** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：06_Manipulation），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
- 风险同样落在这条链上：泛化上限受视频世界模型的保真度与伪动作恢复精度双重约束，本页未给出失败模式的量化。
