...
**Patch Policy 的主张是：机器人策略缺的不是更大的模型，而是被全局池化抹掉的局部空间结构——把预训练 ViT 的密集 patch token 原样接进策略 transformer，用约 0.7% 的参数量就能压过微调后的大型 VLA。**

- 真正起作用的是 **不做全局池化**：每帧压成单 token 会丢掉操作所需的局部空间结构，保留密集 patch 带来相对先进全局池化表征的 **40% 相对提升**。
- **block-causal attention** 是让这件事成立的必要设计：同一时刻的 patch 可以互相注意，同时阻止未来观测泄漏，保住标准策略的时间因果性。
- 定位是 **高频反应式控制**：冻结视觉骨干 + 小策略头，相对微调 OpenVLA-OFT 高 18%、参数量约为其 0.7%，可作大型 VLA 的低延迟替代基线；代价是放弃了经大型 VLM 编码所附带的语言对齐能力。
- 主要未决项是延迟–精度曲线：不同 ViT、相机数量与真实控制频率下的表现尚需核对，声称的高频优势不能默认在所有配置下成立。
- 开源状态：截至 2026-07-22，[项目页](https://patch-policy.github.io/) 的 GitHub 入口仍为 “coming soon”，复现须等代码释出。
