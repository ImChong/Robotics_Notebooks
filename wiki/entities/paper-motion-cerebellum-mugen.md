...
**MuGen 的取舍是把「多技能」的复杂度搬进一个离散化的生成式动作空间，让一个统一控制器承接多种 locomotion 技能，而不是给每个技能各训一套策略。**

- 真正起作用的机制是 **VQ-VAE 把运动技能离散化** 成可复用的动作词表，再由模型式 RL 与师生蒸馏把这套动作空间落成一个可执行的统一控制器；「可提示」是共享动作空间的结果，不是额外挂上去的接口。
- 定位在 **E 可提示控制**（运动小脑 64 篇 #39/64，北京大学），解决的是身体层「一套控制器覆盖多技能」的问题。
- 适用边界：输入端依赖多技能运动数据与任务条件，技能覆盖面因此受数据边界约束；它仍属身体层方案，不替代 VLA / 世界模型的任务规划。
- 本页是策展编译的清单索引归纳，**量化 benchmark、消融与实机指标均未在本页给出**，选型前需回到 [参考来源](#参考来源) 的原文 / 项目页核对。
