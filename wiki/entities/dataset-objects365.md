...
status: complete
updated: 2026-08-12
summary: "超大规模目标检测数据集（365 类、逾百万图像），常用作比 COCO 更强的检测预训练源，再向下游迁移。"
related:
...
**Objects365**：超大规模目标检测数据集（365 类、逾百万图像），常用作比 COCO 更强的检测预训练源，再向下游迁移。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| O365 | Objects365 | 大规模目标检测预训练集 |
...
| 维度 | 速查 |
|------|------|
| 规模 | 约 2M 图像、365 类、数千万框（版本以官网为准）。
| 模态 | RGB + 检测框。
| 许可证 | 需遵守 Objects365 官方使用条款。
| 适配形态 | 检测骨干预训练；再精调 COCO/自有域。
| 重定向就绪度 | 不适用；框标注可直接训练检测器。
...
- [Objects365 官方入口](https://www.objects365.org/)
