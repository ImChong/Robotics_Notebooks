# Gleboss1/mini_cheetah_ROS

> 来源归档

- **标题：** mini_cheetah_ROS — Mini Cheetah 控制的 ROS 移植/学习入口
- **类型：** repo
- **作者：** Gleboss1
- **链接：** https://github.com/Gleboss1/mini_cheetah_ROS
- **许可：** MIT
- **星标（截至 2026-07-25）：** ~3
- **入库日期：** 2026-07-25
- **一句话说明：** 社区 ROS 向移植，降低官方 LCM 栈的入门摩擦；星标低，作学习辅助而非生产主线。
- **开源状态：** **已开源**（社区移植，非 MIT 官方）
- **关联策展：** [mit_mini_cheetah_learning_stack_curator](../personal/mit_mini_cheetah_learning_stack_curator.md)
- **沉淀到 wiki：** [mit-mini-cheetah](../../wiki/entities/mit-mini-cheetah.md)

---

## 定位

- 相对官方 [`Cheetah-Software`](./cheetah-software.md)（LCM），提供更易上手的 ROS 叙事。
- 策展建议：与 [quadruped_ctrl](./derek_th_wang_quadruped_ctrl.md)、[CHAMP](./chvmp_champ.md) 同列为「学习入口」，不以复现官方全部功能为验收标准。

## 对 wiki 的映射

- [mit-mini-cheetah](../../wiki/entities/mit-mini-cheetah.md)
