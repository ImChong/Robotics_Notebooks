...
**PLANC 的关键选择是把降阶模型当「软引导」而不是硬约束：LIP 规划器只负责给出动力学一致的参考，再由 CLF 奖励把 RL 拉到这条参考上，从而同时拿到模型法的落脚精度与 RL 的鲁棒性。**

- 真正起作用的是 **CLF 奖励** 这一物理可解释的塑形信号：它把稳定性写成明确的衰减条件，替代堆砌手工奖励项，使训练围绕物理一致轨迹收敛；纯 model-free 基线在同样地形上会退化为「原地站立」。
