...
- **本页为索引级节点**（FreeDof 44 篇梳理 [19/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页要点是「多异构仿真器并行 rollout + 结构层 DR」，对应证据是多引擎混训与单引擎训练在跨引擎 / 真机上的成功率对比。
...
| **横比口径** | 收益取决于所选引擎之间的差异谱：引擎组合不同，结论不同；同 [20/44] Erez 的引擎差异表一起读才完整。 |
