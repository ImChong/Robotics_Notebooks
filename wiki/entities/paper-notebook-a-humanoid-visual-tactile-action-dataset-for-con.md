...
**A Humanoid Visual-Tactile-Action Dataset for Contact-Rich Manipulation** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：06_Manipulation），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
- 论文自陈目标是**激励后续研究**（开发能有效利用复杂多样触觉信号的优化策略），摘要中不报告具体数值；因此不要把它当作性能对比的引用来源。
- 适用边界：面向家务/护理这类软物体、接触丰富的场景；纯刚体抓取或低接触任务从中获益有限。
- 遥操作采集决定了规模与多样性受人力成本约束，本页也未给出规模数字；量化细节以深读笔记与论文 PDF 为准。
