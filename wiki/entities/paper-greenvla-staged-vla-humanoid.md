...
**Green-VLA 的主张是「配方比数据量更重要」：在刻意少于 π₀ 的机器人数据上，用质量过滤、语义统一动作与五阶段课程换取跨本体泛化，最后用不直接穿 flow 梯度的保守 RL 补 BC 在长程任务上的饱和。**

- 真正起作用的三件事是 **DataQA 质量过滤 + 光流速度对齐**、**64 维语义槽统一动作空间（掩码而非 padding）**、**L0–L1–R0–R1–R2 分阶段课程**；证据是约 3000 h 数据下 R0 清理桌 First-item SR 69.5% vs π₀ 35.6%。
- **R2 是取舍而非局限**：用 IQL 轨迹优化与初始噪声 actor 回灌，代价是探索被限制在 BC 流形附近，收益是稳定性——Simpler WidowX 上 R2 较 R1 +24% 绝对成功率。
- 人形侧的价值在 **同一 checkpoint 覆盖单臂到 32 DoF 双手灵巧**，并以冻结 GigaVision 做「慢规划 + 快 VLA」分层；但该规划器推理时冻结，规划能力不随部署迭代。
- 主要风险在上游：跨本体收益依赖 **retargeting 保真度** 与数据中灵巧技能的覆盖；CALVIN 评测未使用统一动作空间（论文自注），跨基准结论不宜整体外推。
- 与 [ROVE](./paper-rove-humanoid-vla-intervention.md) 对照：后者靠部署后干预 RL 补最后一英里，Green-VLA 走离线轨迹修正；与 π₀ 系的差别不在动作头（同为 flow），而在阶段划分与动作空间构造。
...
- [Temporal GRPO](./paper-temporal-grpo.md) — 对照：rollout 内阶段信用，不是 L0–R2 训练课（arXiv:2608.13026）
