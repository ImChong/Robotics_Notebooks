...
status: complete
updated: 2026-08-12
summary: "手写数字分类小规模基准：6 万训练/1 万测试灰度 28×28；教学与算法冒烟测试常用，非现代机器人感知主基准。"
related:
...
**MNIST**：手写数字分类小规模基准：6 万训练/1 万测试灰度 28×28；教学与算法冒烟测试常用，非现代机器人感知主基准。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| MNIST | Modified National Institute of Standards and Technology database | 手写数字分类经典小集 |
...
| 维度 | 速查 |
|------|------|
| 规模 | 70k 张灰度图（60k train / 10k test），10 类。
| 模态 | 单通道 28×28 位图；无深度/点云。
| 许可证 | 研究常用公开基准（以官方分发说明为准）。
| 适配形态 | 通用视觉入门；与机器人本体无关。
| 重定向就绪度 | 不适用（非运动数据）；可直接作张量分类输入。
...
- [具身大模型评测基准选型闭环知识链](../queries/embodied-eval-benchmark-selection-loop.md) — 冒烟级基准：典型的「测得漂亮但离真机最远」上游代理指标
...
- [MNIST 官方入口](http://yann.lecun.com/exdb/mnist/)
