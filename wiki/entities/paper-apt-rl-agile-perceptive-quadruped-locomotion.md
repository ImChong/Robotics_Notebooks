...
related:
  ...
  - ./paper-mgdp-generalized-depth-perception.md
sources:
...
**APT-RL 的取舍是「先验只管平地两种步态，复杂地形交给 RL 扩展」：用廉价 2D 轨迹优化而非更大的模仿数据集，换来单策略的野外多技能。**

- 起作用的结构是 **潜动作 + 辅助力矩**：预训练仅覆盖 2D flat trot/bound，跳木、腿损恢复、偏航等 OOD 行为由辅助项在 RL 中长出来，比固定 residual 更紧密地联合学习。
- 最硬的证据是 **零样本野外长程**：校园 **1.1 km**、森林 **0.34 km**，仅机载感知、无动捕；峰值 **6 m/s**、60 cm 高台 **4.25 m/s**。
- 感知侧结论是 **双模态 > 单模态且分工明确**（LiDAR 擅楼梯/沟/高台，深度擅栏/垫脚石）；**>4 m/s** 需减振机构抑制 **>10 g** 冲击——高速下外感知仍是最脆弱环节。
- 边界：运动覆盖偏 **矢状面前后**，横向行走与急转未做；步态仅 **trot + bound** 两种。
- 复现状态：项目页 **未列 GitHub**，[Zenodo:20645964](https://zenodo.org/records/20645964) 仅数据与作图代码，三阶段训练/部署栈需自行重建，且实机绑定自研 HOUND。
- 与 [Walk These Ways](./paper-walk-these-ways-quadruped-mob.md)、[Learning to Adapt](./paper-learning-to-adapt-bio-inspired-quadruped-gait.md) 的对照定位：步态选择从「人调参数 / 生物力学指标」转为 **按障碍高度与命令速度自动切换**。
...
- [MGDP（Adv. Sci. 2026 通用深度感知 + 9 构型迁移）](./paper-mgdp-generalized-depth-perception.md)
