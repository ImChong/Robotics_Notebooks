...
**LapSurgie: Humanoid Robots Performing Surgery via Teleoperated Handheld Laparoscopy** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：07_Teleoperation），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**LapSurgie 押的是「部署性」而不是「性能」：用通用人形握住未经改装的现成腹腔镜器械，绕开专用手术机器人的成本与基础设施门槛，代价是速度。**

- 技术枢纽是 **RCM 约束下的逆映射**：把被动腕式器械显式建模成运动链，用非线性最小二乘（TRF）从操作者手部目标位姿解出手柄该摆的姿态——比硬 IK 更能稳定驱动无主动驱动的工具，思路可迁移到其他「人形操作现成人类工具」的场景。
- 精度侧的结论站得住、速度侧不行：peg-transfer 上与 dVRK 金标准相当（新手 p=0.386，外科医生组甚至更优），但完成时间约为 dVRK 的 **2 倍**，受控制方案复杂度与系统延迟拖累。
- 证据边界很窄：仅 14 名受试且以新手为主、只测 peg-transfer 单一受控任务、从未真实临床部署；逆映射还依赖器械专属运动学参数，换器械即需重新建模，灵活性受限。
- 工程状态：截至当前未见公开代码仓库；同团队 [Humanoid Surgeon（Nature 2026）](./paper-humanoid-surgeon-in-vivo-laparoscopy.md) 已把该框架推进到 **in vivo 猪模型胆囊切除术**，是判断这条路线可行性的后续观察点。
