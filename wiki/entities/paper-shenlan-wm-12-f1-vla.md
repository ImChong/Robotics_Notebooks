...
**F1-VLA 在联合架构里再切一刀：用 MoE 把感知、视觉预见、控制拆成三个模块，让「预见」显式成为控制的前置条件，而不是隐含在同一份权重里。**

- 起作用的机制是 **下一尺度预测 + 预见引导 IDM**：先生成视觉预见，再由逆动力学模型反推动作，三模块经门控加权组合。
- 与同属 **路线 02 联合架构** 的项目共享「未来观测与动作同骨干、减少级联误差累积」的动机，但 F1-VLA 把模块边界显式拆开，代价是多一层 MoE 调度与训练复杂度。
- 本页为 **清单索引策展编译**；文内引用 24（2026-06-02 策展）低于同栈多数项目，但引用量与下游任务增益无简单线性关系，量化指标与 License / 权重均以原文和项目页为准。
