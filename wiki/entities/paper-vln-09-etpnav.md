...
**ETPNav 在连续环境里选择「拓扑建图 → 跨模态规划 → 底层控制（含避障）」三段串联，而不是端到端像素到动作，这让它成为 VLN-CE 里可被反复复用的规划骨干。**

- 起作用的是 **拓扑抽象**：把连续空间压成可规划的图结构，跨模态规划在图上决策，底层控制负责避障执行；R2R-CE、RxR-CE 上报告大幅提升。
- 「可复用骨干」有外部证据：[REALM](../entities/paper-realm-last-3-meter-vln-grounding.md) 把 ETPNav-ZS/FT 当作四类 plug-and-play 骨干之一，在 REVERIE-AIM 上 ONS@0.1m 相对无精修基线约 **翻倍**。
- 这同时划出了边界：ETPNav 强在 **把机器人带到目标区域**，最后米级以内的精确 grounding 需要 REALM 这类下游精修补齐。
- 通用注意事项：连续环境 (VLN-CE) 与离散图设定不可直接混比，benchmark 提升不等于真机部署。
