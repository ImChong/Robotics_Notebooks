...
**这篇的主张是「数据先于模型」：先用自动化流水线把免费的人类动作视频变成 260+ 小时、带语义标注的 Humanoid-Union，再用 SCHUR 把「数据/模型规模 → 高层控制性能」做成可量化的研究对象。**

- 真正起作用的是数据侧闭环：自动化流水线决定数据能否持续扩展，语义标注决定控制能否被语言/意图驱动；SCHUR 是承接这批数据的可扩展框架，而非独立的算法卖点。
