...
### play.py 回放流程（`run_play`）

入口为 [`scripts/play.py`](https://github.com/ImChong/AMP_mjlab/blob/main/scripts/play.py) 的 `main()` → `run_play()`（仓库内无名为 `play` 的独立函数）。AMP 任务注册时使用 `play_env_cfg=g1_amp_*_env_cfg(play=True)`，并绑定 [`AMPOnPolicyRunner`](https://github.com/ImChong/AMP_mjlab/blob/main/src/tasks/amp_loco/rl/runner.py)。

```mermaid
flowchart TD
  subgraph CLI["main()：CLI 解析"]
    M1["import mjlab.tasks + src.tasks\n填充任务注册表"] --> M2["tyro 解析 task_id\n例：Unitree-G1-AMP-Flat"]
    M2 --> M3["tyro 解析 PlayConfig\nagent / checkpoint / viewer / export_onnx 等"]
    M3 --> M4["run_play(task_id, cfg)"]
  end

  subgraph INIT["run_play：初始化"]
    M4 --> R1["configure_torch_backends()"]
    R1 --> R2["device ← cfg.device 或 cuda:0/cpu"]
    R2 --> R3["env_cfg ← load_env_cfg(task_id, play=True)\n→ g1_amp_*_env_cfg(play=True)"]
    R3 --> R4["agent_cfg ← load_rl_cfg(task_id)"]
    R4 --> R5{"cfg.agent\nzero / random / trained?"}
  end

  subgraph PLAY_ENV["play=True 环境覆盖（env_cfgs.py）"]
    R3 -.-> P1["episode_length_s ≈ ∞"]
    P1 --> P2["关闭 actor 观测 corruption"]
    P2 --> P3["移除 push_robot；清空 curriculum"]
    P3 --> P4["init_motion_loader delay_reset_env_ratio=1.0"]
    P4 --> P5["Rough：randomize_terrain\nFlat：平面 + 更宽 twist 指令"]
  end

  R5 -->|trained| T1{"checkpoint_file?"}
  R5 -->|zero / random| D1["DUMMY_MODE：无 checkpoint"]

  T1 -->|有| T2["resume_path ← Path(checkpoint_file)"]
  T1 -->|无| T2b["wandb_run_path → get_wandb_checkpoint_path"]
  T2 --> T3["log_dir = resume_path.parent"]
  T2b --> T3

  T3 --> CFG
  D1 --> CFG["PlayConfig 覆盖\nnum_envs / 视频分辨率 / no_terminations"]
  CFG --> CFG2["no_terminations → 清空 terminations"]

  subgraph ENV["仿真环境"]
    CFG2 --> E1["ManagerBasedRlEnv\nrender_mode=rgb_array if video"]
    E1 --> E2{"trained + video?"}
    E2 -->|是| E3["VideoRecorder → log_dir/videos/play"]
    E2 -->|否| E3b["RslRlVecEnvWrapper\nclip_actions"]
    E3 --> E3b
  end

  E3b --> POL{"agent 模式"}
  POL -->|zero / random| POL0["PolicyZero / PolicyRandom"]
  POL -->|trained| POL1["runner_cls = AMPOnPolicyRunner"]
  POL1 --> POL2["runner.load(checkpoint)\nload_optimizer=False"]
  POL2 --> POL3["policy = get_inference_policy()\nobs_normalizer 包装 act_inference"]

  POL3 --> EXP{"export_onnx?"}
  EXP -->|是| EXP1["export_runner_policy_to_onnx"]
  EXP -->|否| V0
  EXP1 --> V0
  POL0 --> V0

  subgraph ONNX["ONNX 导出细节"]
    EXP1 -.-> O1["优先 runner.export_policy_to_onnx\nAMP：_OnnxPolicyWrapper + normalizer"]
    O1 --> O2["输出 export/<task>_<ckpt>.onnx\nobs/actions，dynamic batch"]
  end

  subgraph LOOP["可视化主循环"]
    V0["viewer=auto → native/viser"] --> V1["NativeMujocoViewer 或 ViserPlayViewer.run()"]
    V1 --> V2["reset → 循环：obs → policy → step"]
    V2 --> V3["env.close()"]
  end
```
...
- **Play 环境**：`play=True` 时拉长 episode、关闭训练期随机化推扰，并打开 `delay_reset_env_ratio=1.0`，便于观察 recovery；Flat 任务额外放宽速度指令范围。
- **策略推理**：`AMPOnPolicyRunner.get_inference_policy()` 在 `empirical_normalization` 下会把 `obs_normalizer` 包进 callable，与 ONNX 导出语义一致。
- **ONNX**：`export_onnx=True`（默认）时写入 `logs/rsl_rl/<experiment>/export/<task>_<checkpoint_stem>.onnx`；训练期 `save()` 也会写同目录 `policy.onnx`。
