...
**用可解释的四维楼梯几何 token 条件化 PPO，比隐式高程/视觉 latent 更样本高效，且在 OOD 踢面与长户外楼梯上更稳。**

1. **接口对齐物理需求** — 踢面高度、踏面深度、航向与上/下/平状态直接进策略观测，对准净空与落点，而非让策略从高维地形里猜。
2. **相对 Blind / HeightMap 更强** — 成功率 96% vs Blind 52%、HeightMap 88%；学习曲线也更快收敛。
3. **OOD 踢面优于视觉 MoRE** — 未见 0.22 m 踢面成功率 82.7% vs 69.4%；不规则户外楼梯 93% vs HeightMap 75%、MoRE 72%。
4. **三阶段 teacher–student 可部署** — 真值几何预训练 → 教师监督感知 → 联合微调；部署仅需学生编码器 + 策略。
5. **G1 零样本实机** — 室内 5 级与户外 33 级连续上楼；几何估计约 1 cm 级 MAE、状态分类 >97%。
6. **选型边界** — 聚焦楼梯类结构化地形；与 FastStair（规划落点 + 高速）正交，暂无官方代码仓。
