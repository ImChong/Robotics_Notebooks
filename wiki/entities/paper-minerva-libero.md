...
summary: "MINERVA（arXiv:2609.03715，东大松尾–岩泽实验室）：0.54M 参数 task-ID 条件策略在标准 LIBERO 四 suite、2000 rollouts 上约 95% 平均成功率；容量 ~1M 饱和、低于 0.25M 崩塌；k1000dai/MINERVA 已开源。"
---

# MINERVA：LIBERO 需要多小的操作策略

**MINERVA**（*MINimal Efficient Robotic Vision-Action policy*，[arXiv:2609.03715](https://arxiv.org/abs/2609.03715)，[代码](https://github.com/k1000dai/MINERVA)）由 **东京大学** 松尾–岩泽研究室 Kohei Sendai、Tatsuya Matsushima、Yusuke Iwasawa 提出：十亿参数级 VLA 占据 LIBERO 榜，但基准真正需要的容量并不清楚。MINERVA 是一组刻意压小的 **task-ID 条件** 视觉运动策略，用来量这条 **闭集 40 任务** 的容量下限。官方 0.54M 模型用 **从头训练的 CNN + L1 action-chunk 头**，推理不使用语言编码器、预训练视觉骨干、VLM 或迭代生成采样。

## 一句话定义

**开源小模型不是缩水版大 VLA，而是衡量「固定任务集最少要多少容量」的尺子。**
...
| L1 | L1 regression | 直接回归 action chunk |
| FM | Flow Matching | 对照用的迭代生成头 |
...
| **开源** | **已开源** Apache-2.0 + HF `k1000dai/MINERVA` |

### 流程总览

```mermaid
flowchart LR
  img[相机图像] --> cnn[scratch CNN]
  tid[任务 ID] --> head[L1 / flow action-chunk]
  cnn --> head
  head --> act[关节动作 chunk]
  act --> env[LIBERO MuJoCo]
```
...
其它发现：flow matching 相对 L1 无稳定优势，regression 在 GPU 上最高快 **3.8×**；**LIBERO-Plus** 扰动下成功率降至 **46–56%**。
...
3. **L1 够用** — 生成式动作头不是这块闭集的必需品。
4. **Plus 才暴露脆弱** — 46–56% 说明标准套件低估扰动。
5. **它不是通用 VLA** — README 写明不能执行未见指令。
...
| 全量 | `n_episodes=50`、`--seed=1000` |
| 训练 | `uv sync --extra libero --extra training` + `train_t05_l1.sh` |
| 误用 | 不要把它部署成开放语言家庭机器人 |
