...
class TestPathsUnit(unittest.TestCase):
    ...
    def test_path_to_id(self):
        ...
        self.assertEqual(
            path_to_id(root / "roadmap/motion-control.md", root), "roadmap-motion-control"
        )
        self.assertEqual(path_to_id(root / "references/papers/rl.md", root), "reference-papers-rl")

        self.assertEqual(
            path_to_id(root / "tech-map/modules/control/mpc.md", root), "tech-node-control-mpc"
        )
        self.assertEqual(
            path_to_id(root / "tech-map/research-directions/humanoid.md", root),
            "tech-node-research-humanoid",
        )
