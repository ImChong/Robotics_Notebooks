...
### 💡 高频引用 methods/ 缺 queries/ 或 comparisons/ 落地（信息型，不阻塞 CI）（36 个）
...
- wiki/methods/motion-retargeting-gmr.md（被 30 个页面引用，无 queries/comparisons 落地）
- wiki/methods/motionbricks.md（被 7 个页面引用，无 queries/comparisons 落地）
- wiki/methods/neural-motion-retargeting-nmr.md（被 18 个页面引用，无 queries/comparisons 落地）
...
- wiki/methods/sonic-motion-tracking.md（被 36 个页面引用，无 queries/comparisons 落地）
...
📊 Sources 覆盖率：378/378 (100%) wiki/entity 页有 ingest 来源
