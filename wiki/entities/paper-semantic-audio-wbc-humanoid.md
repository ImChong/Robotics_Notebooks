...
**这篇把「人形跳舞」从离线编舞改写成在线语义调度：低层技能一律不重训，新增的是「听懂当前音频块该配哪个技能」以及「技能之间怎么切」。**

- 起作用的机制是 **路由 + 检索 + 统一 skill ID 队列**：AST/VAD 分流音乐、语音与跳过，音乐走指纹（低置信回退 CLAP），语音走 ASR + 语义匹配，两条链最终共用 RoboJudo 的同一调度接口。
- 关键指标是 **块级检索 84.8%**，但 M20 / M30 mashup 对照才是结论所在：M30 过渡时间充足、Gantt 与指令序列高度一致，M20 则 Stand priming 来不及、易回退 Walk 或失稳——瓶颈在 **技能切换动力学**，不在检索准确率。
- 适用边界：面向 RoboCup OnStage、展会互动这类 **非脚本化** 场景；嵌入空间允许未见曲目/语句映射到最近技能，但可表达行为的上限由 BeyondMimic 技能库决定。
- 主要风险：固定 5 s 分块引入对齐延迟，外呼 LLM/TTS 与硬件接口把真机端到端延迟推高于仿真；风格变化检测仍是 future work。
- 工程与开源：**已开源**（semantic-WBC）；仿真 MuJoCo G1 与真机共用同一编排逻辑，只替换硬件接口。

## 与其他工作对比

- **vs 生成 kinematics 路线：** 相较 AI Choreographer / EDGE 等离线「生成 kinematics」的编舞路线，本文 **不重训低层**，而是在线检索并组合已有 RL / 模仿技能（BeyondMimic）。
- **vs 端到端 RL 学跳舞：** 本文显式 **解耦技能学习（BeyondMimic）与语义调度（编排器）**，音乐节拍段与语音指令共用 skill ID 队列，可泛化到未见曲目 / 语句而无需为每首歌重训。
- **瓶颈定位：** M20 vs M30 mashup 对照表明瓶颈在 **技能切换动力学**（过渡时间）而非检索准确率本身（块级检索已达 84.8%）。
