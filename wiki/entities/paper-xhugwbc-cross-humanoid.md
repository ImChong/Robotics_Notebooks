...
| 维度 | 本工作（XHugWBC） | [H-Zero 跨人形预训练](./paper-notebook-h-zero-cross-humanoid-locomotion-pretraining-ena.md) | [全身控制（概念）](../concepts/whole-body-control.md) |
|------|-------------------|--------------------------------------------------------------------------------------------|------------------------------------------------------|
| 方法族 | 形态随机化 + 语义关节映射 + 本体图网络的单控制器 | 跨人形运动预训练，为迁移到新本体提供先验 | 全身协调控制的通用问题设定 |
| 跨本体机制 | 物理一致随机形态 + 32 槽语义关节空间 + mask 屏蔽缺失关节 | 通过大规模预训练学习可迁移表示 | 不限定具体跨本体手段 |
| 关键假设 | 不更新权重即可覆盖异构人形；缺失关节用 mask 而非零填充 | 预训练分布需覆盖足够本体多样性 | 假设共享身体上多目标需联合协调 |
| 优化目标 | 检验零样本跨人形控制的边界 | 提供可复用的跨本体运动先验 | 定义协调与稳定的评价维度 |
| 关系/取舍 | 强调表示与网络对齐，换取无需重训的泛化 | 强调预训练规模，通常仍需下游微调 | 作为上位概念供两者定位 |

跨本体策略选型的整体视角见 [跨本体迁移策略](../queries/cross-embodiment-transfer-strategy.md)。
