---
type: overview
tags: [depth, depth-learning, rl, il, imitation, reinforcement]
status: complete
updated: 2026-07-31
summary: "IL/RL 学习范式纵深汇总：强化学习、模仿学习、行为克隆与 model-based 路线的选型、数据需求与机器人落地注意点。"
---

# IL/RL 学习范式（纵深汇总）

> **图谱纵深视图**：本页是知识图谱「🎓 模仿/强化学习 (IL/RL)」纵深的统一入口；在 [图谱纵深视图](../../docs/graph.html?depth=learning) 筛选时，本节点为汇总锚点。

## 一句话定义

**机器人学习纵深** 覆盖 **从数据或交互中习得策略** 的主要范式：强化学习（RL）、模仿学习（IL）及其与 model-based、离线 RL、基础模型的组合。
...
- **[Sim2Real](./depth-sim2real.md)**：学习策略如何上真机。
- **[WBT](./depth-wbt.md)**：跟踪与 AMP 大量用 RL。
- **[安全微调](./depth-safe-fine-tuning.md)**：部署后在线 RL 的安全边界。
...
- 图谱纵深定义：[docs/depth-filters.js](../../docs/depth-filters.js)（`learning` 命中规则）
