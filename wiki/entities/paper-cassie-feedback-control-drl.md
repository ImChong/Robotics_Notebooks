...
**这篇的价值在于把「策略输出关节目标、底层 PD 跟踪」立成一个清晰可部署的 MDP 表述，而不在于某个 SOTA 数字。**

- 真正的论点是 **动作空间选择**：低维目标空间缩小探索、复用已知内环，因此不必一上来就学扭矩。
- 参考运动被当作 **条件而非硬轨迹**：不同速度由时间伸缩参考得到，策略只学反馈补偿。
- 读法提示：把 PD 视作已建模内环，调 `Kp/Kd` 等价于改变策略看到的 **有效 plant**——增益过大掩盖策略缺陷，过小则让策略扛下全部扰动。
- 边界：鲁棒性覆盖 **传感延迟、盲走不规则地形、骨盆扰动** 三类压力案例；量化指标、消融与实机结果本页不复述，需回原文。
- 谱系定位：上承 [DeepRL 动作空间对比 SCA 2017](./paper-deeprl-locomotion-action-space-sca2017.md) 的角色动画侧对照，下接 [迭代式 Cassie sim2real](./paper-cassie-iterative-locomotion-sim2real.md) 与 [双足多技能 RL](./paper-cassie-biped-versatile-locomotion-rl.md)。
