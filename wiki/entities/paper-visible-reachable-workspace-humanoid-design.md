...
| [机器人视觉感知栈选型闭环](../queries/robot-perception-stack-selection-loop.md) | 选算法之前先定构型 |

- **与感知栈选型闭环的分工：** 该闭环从 ①传感层 往下选算法；本页把问题往上推一层——**相机装在哪、关节链怎么排**，决定了后面各层能看到什么。它是感知栈选型的前置约束，不是替代。
