...
**TouchWorld 的主张是触觉要同时扮演两个角色——慢速预测「接触该长成什么样」与高频纠偏「实际接触偏了多少」——所以它选择四时钟层级，而不是把触觉再塞进单体 VLA。**

- 起作用的是 **分频**：TWM 仅在子任务/接触相位变化时刷新 17 帧视触觉子目标，名义 VLA 出 $H=32$ 动作块，TRT 在 $W=16$ 滑窗上每 $C=4$ 步提交残差；干净 **65.0%** / 扰动 **53.7%**，较 FTP-1 **+15.7 / +18.5 pt**。
- 增益结构本身就是证据：**扰动设置下优势更大**，且消融里去 TRT 特别伤扰动、去 SP 伤长程、去 TWM 弱化接触目标条件、去触觉降幅最大——四层各自负责的失败模式基本可分离。
- 规划侧的教训是 **任务相位监督比放大 VLM 更重要**：记忆增强 4B 子任务准确率 **88%**，高于 zero-shot 32B 的 **69%**。
- 适用边界与风险：仅 **六类** 代表任务，换触觉布局或手型需标定与小量适应；TWM 目前只做 **短视界** 子目标，长程未来与不确定性建模尚未覆盖。
- 工程状态：截至 2026-07-16 项目页 **仅列 Paper**，无代码/权重/数据集，复现入口不可用；可用的降级路径是关掉 SP/TWM，退回任务 prompt + 名义 VLA + TRT。
- 与同期路线对照：[T-Rex](./paper-trex-tactile-reactive-dexterous-manipulation.md) 押大规模触觉 mid-training + MoT 双专家，[OmniTacTune](./paper-omnitactune-tactile-residual-adaptation.md) 押冻结视觉 + 短预算真机 RL 残差——三者是选型并列项，不是替代关系。
