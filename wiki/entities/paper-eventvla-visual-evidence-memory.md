...
**EventVLA 的核心主张是「记忆要稀疏，且必须是原始像素」：规则锚点管住场景布局，学习式前瞻 KEM 抓住瞬态证据，两者合起来才越过长程非马尔可夫操作的门槛。**

- 分工是可量化的：**仅锚点** 在 RMBench 已达 **67.8%**（布局型记忆够用），但在 RoboTwin-MeM 只有 **18.0%**，加 KEM 后 **75.2%** — 中间瞬态证据必须靠动态写入捕获。
- 真正起作用的设计是 **chunk-wise 前瞻**（H=50）而非逐步分类，配 **NMS + 冷却** 保持稀疏；消融很陡：去 NMS **53.4%**、$N_{\max}=2$ **32.0%**、chunk=15 **13.6%**。
- **保留原始关键帧图像** 是必要条件：换成隐式潜记忆 bank 只有 **24.9%**；同时 RoboTwin 2.0 标准任务（Easy 83.8% / Hard 81.6%）说明记忆机制未损害普通马尔可夫闭环控制。
- 主要风险是 **FIFO 容量**：论文承认 >10 min、高密度事件场景会因缓冲饱和丢失早期证据；且实现绑定 **QwenOFT**，跨骨干需重训 KEM 与标注管线。
- 相邻定位：与 [KEMO](./paper-kemo-event-driven-keyframe-memory-vla.md) 的规则选帧 + cross-attention 融合不同，也不同于 [Chronos](./paper-chronos.md) 的全历史紧凑潜状态；基准侧的 **RoboTwin-MeM**（显式参数化中间记忆数 $n$）可能与方法同样重要。
...
- 全历史紧凑策略对照：[Chronos](./paper-chronos.md) — SSM 潜状态 + IMLE/二阶桥；RMBench **73.6%**（arXiv:2606.30318）。
- 3D 多视图记忆对照：[BridgeVLA++](./paper-bridgevla-plusplus.md) — heatmap + 𝒯/𝒮；RMBench **96.0%**（arXiv:2608.05042）。
