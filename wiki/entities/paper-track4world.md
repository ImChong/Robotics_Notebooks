...
> 上表数字来自论文与项目页；除 **WorldTrack × OpenD4RT** 这一条论文自带的公平对比脚本外，下列各行只做 **路线定位**，坐标系（世界系 vs 相机系）与尺度语义（metric vs 相对）不同的结果不可直接横比。

| 对照 | 差异读法 |
|------|----------|
| [D4RT](./paper-d4rt.md) | 最近的同题工作，解码形态相反：D4RT 走 **统一查询式 4D**（问哪点答哪点），Track4World 走 **VGGT 式全局表示 + 稠密流解码**（一次出全像素）。工程上的硬差别是 D4RT 截至入库日 **未开源**，而本页提供 OpenD4RT 公平对比脚本 |
| **SpatialTrackerV2 / POMATO** | 同做 3D 点跟踪，但多为 **多阶段或带优化** 的管线；Track4World 把它压成 **单次前馈**，TAPVid-3D 世界系平均 APD L-16 **0.5636** 领先。代价是吃整段视频的全局表示，显存与依赖更重 |
| **RAFT-3D / Any4D 等场景流** | 只解 **相邻帧场景流**，长程对应要靠串联累积误差；Track4World 直接预测 **任意帧对** 的 2D/3D 流再融合，Kubric-3D EPE3D **0.1537** 即在这一设定下取得 |
| **首帧稀疏点跟踪（TAP 系）** | 跟的是人工选定的稀疏查询点，回答不了「这一帧每个像素去哪了」；Track4World 的主张就是 **全像素 + 世界系**，2D 侧同时保持竞争力（Kinetics AJ 59.1 / RoboTAP 70.9） |
| [TAPVid-MV](./paper-tapvid-mv.md) | **评测侧的相邻工作**而非方法竞品：多视角跟踪评测协议；读本页 TAPVid-3D 数字时注意两者协议不同，不要混成一张表 |
| [VGG-TTT](./paper-vgg-ttt.md) | 同属 VGGT 系几何骨干的下游用法，但改的位置不同：VGG-TTT 在 **测试时适配**，Track4World 在 **表示之上加流解码头**；两者对「骨干已够好，缺的是任务头还是域适配」给出相反答案 |
| [状态估计](../concepts/state-estimation.md) / 静态 SLAM pointmap | 定位提醒：本页是 **动态场景 4D 特征提取器**，输出的是运动场而非位姿滤波或静态地图；Bonn ATE 0.009 是附带指标，不要当 SLAM 主张读 |
