...
**CMR: Contractive Mapping Embeddings for Robust Humanoid Locomotion on Unstructured Terrains** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：05_Locomotion），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**CMR 的取舍是「用一个辅助损失换可证明的抗噪」：不加独立估计模块、不动 PPO 主体，把鲁棒性问题从原始观测空间搬到收缩潜空间里去解。**

- 真正起作用的是**双目标的相互牵制**：Lipschitz 正则逼出 κ<1 的收缩（本文取 0.1），InfoNCE 负责不让收缩把任务相关语义一起压掉；缺任一边就退化成「只收缩不保信息」或「只保信息不抗噪」。
- 理论卖点是退化上界 `O(η/(1−κ))` **与时域无关**，跳出标准分析里 `O(L_f^H)` 的指数放大——这决定了它的收益主要体现在长时域 rollout。
- 实证边界清楚：六类非结构地形、五档噪声（α 0.01→3）下优于 HIM / LCP / Naive PPO；但超出训练噪声包络（α≫3）的极端情形仍可能退化。
- 主要风险是调参与验证深度：κ 与 Lipschitz 权重 λ 需要调，过强收缩会压掉有用信息；验证止于仿真 + sim-to-sim，真机表现待后续。
- 工程状态：截至论文未给出公开仓库链接，「即插即用」的收益需要自行实现，勿假设可直接复现。
