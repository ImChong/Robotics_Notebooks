...
同为四足步态，分歧在「**步态由谁决定**」——控制器、还是机械本身：

| 路线 | 步态从哪来 | 硬件刚度取向 | 换步态的代价 | 与本文 |
|------|------------|--------------|--------------|--------|
| **本文 eBert** | **机械共振涌现**：6 条 NNM + 步长黑箱优化 | **刻意低刚度**（SEA 按小型犬标定） | 换激发的模态即换步态 | 本页；控制器只做状态切换与最小摩擦补偿 |
| [QLaun](./paper-qlaun.md) | 控制器完全决定 | **QDD 高刚度**（全 3D 打印） | 改控制器 | 同为四足硬件论文，硬件哲学正相反：一个把动力学交给机械，一个交给控制 |
| RL 学习步态（见 [locomotion](../tasks/locomotion.md)） | 策略网络 | 通常高刚度，硬件近似 **理想力矩源** | 重训 | 本文不训策略；反过来说，其涌现步态也没有 RL 的地形泛化能力 |
| [双凸轮 PEA 踝](./paper-dual-cam-parallel-elastic-ankle.md) | 不管步态 | 并联弹性 | — | 同样往结构里放弹性件，但目标是 **静态持姿卸荷**，不是激发共振 |
| 生物肌肉 | 共振 + **在线调刚度** | 可变 | 实时 | 页内已注明的差距：eBert 刚度 **不可在线调** |

**读法：** 这是 [具身基础模型硬件共设计](../concepts/embodied-foundation-model-hardware-codesign.md) 的一个 proof-of-concept——贡献是证明 NNM 这套工具能用在 **完整 3D 四足** 上、让共振「可设计可预测」，**不是** 主张它在能效或鲁棒性上已胜过工业四足（论文明确不复刻动物能效数字，且开环弱反馈）。
...
- [双凸轮并联弹性踝](./paper-dual-cam-parallel-elastic-ankle.md) — 另一种「把弹性放进结构」的取向
