...
**GaitSpan 的核心主张是「别为跑步重学一遍」：把冻结的行走策略当种子，用节律组合（GaitWave）+ 步幅塑形（H-SLIP）+ 残差三件套，让走–慢跑–跑在单一速度条件策略里连续涌现。**

- 三件套缺一不可：消融显示仅 GaitWave 或仅 H-SLIP 都无法同时保住 OOD 速度跟踪与动态 flight，**直接微调种子** 易退化为原地踏步，**固定相位缩放** 在种子训练域外跟踪误差大。
- 关键取舍是「不用人体数据」：只靠机器人自身行走先验，换来相对 AMP 人体基线更准的速度跟踪、更清晰的速度依赖接触重组与中低速更低能耗——论文明确反对把「像人的跑姿」当优化目标。
- 适用边界：吃 **种子策略的质量与覆盖**，需要先有相位行走管线（如 [Holosoma](./holosoma.md)）；当前只是 **一步生长**（走→快），未把新技能回灌种子形成闭环。
- 能力上限清晰：训练偏置集中于约 2.2 m/s、评测含 2.5 m/s OOD、项目页展示至约 2.8 m/s，低于 [SPRINT](./paper-sprint-humanoid-athletic-sprints.md) 的冲刺向工作；两者分别对应「已会走的人形如何加速」与「极速外推」。
- 泛化面比峰值速度更有工程价值：同一管线覆盖五套 embodiment、Booster T1/K1 与 G1 真机，训练仅平地/缓坡却能上波场、砂石、草地、林间坡道并耐受负重与低摩擦鞋扰动；代价是代码尚未公开。
