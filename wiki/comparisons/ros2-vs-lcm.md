...
- [小脑与脊髓 (Low-level)]：在具备硬实时（PREEMPT_RT 补丁）特性的实时进程中，使用 MPC 或 RL 策略网络，以 1000Hz 处理控制逻辑，这一层内部以及它与电机驱动板之间的通信全部采用 **LCM**（或 **[EtherCAT 主站优化](./ethercat-master-optimization.md)**）。
