...
**这篇工作的贡献不是新算法，而是把「动作空间选什么」当成一等实验变量：在同一步态模仿任务上系统对照四种参数化，论证带局部反馈的高层接口通常比端到端扭矩更好学、更抗扰。**

- 真正被度量的是 **接口族之间的差异**（Tor / MTU / PD / Vel × 学习时间、鲁棒性、运动质量、策略查询率），而不是某个单点指标的最优。
- 起作用的机制是 **局部反馈内环**：PD 输出目标角、Vel 输出目标角速度、MTU 借肌肉–肌腱动力学，都把一部分稳定性交给底层，降低策略的探索负担；其中 **PD 的样本效率与运动质量往往最佳**。
- 适用边界是 **平面角色 + 周期步态模仿**：最优接口仍随任务与角色形态变化，不能脱离具体 plant 外推到真机人形。
- 它对比的是接口语义，**不含 Kp/Kd 扫参与随机化** 的工程细节 — 后者见 [Kp/Kd 设置 Query](../queries/legged-humanoid-rl-pd-gain-setting.md)。
- 定位上是 [Xue Bin Peng](./xue-bin-peng.md) 研究线的起点，也是后续 Cassie / 四足文献里「目标角 + PD 内环 vs 直驱扭矩」之争的前史锚点。
