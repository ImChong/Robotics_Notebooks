...
sources:
  ...
  - ../../sources/repos/humanoidarena.md
summary: "HumanoidArena（arXiv:2606.17833）：仿真优先的 egocentric 分层全身学习基准——高层策略输出 40D 中间全身动作、低层 GMT（TWIST2/SONIC）执行；7 项下肢关键 HOI/HSI 任务 + 视觉/语义/执行扰动与跨 GMT 迁移评测；代码/数据/模型已开源（multicam 待发布）。"
---
...
- **工程闭环：** PICO egocentric 遥操作 → GMR → Isaac Lab 录制 → **LeRobot** 数据集 → **已开源** checkpoint 与仿真资产，降低复现门槛。
...
| 代码 | <https://github.com/William-wAng618/HumanoidArena>（`release/open-source-prep`） |
...
| LeRobot 数据 | ModelScope / HF `…/HumanoidArenaV3.1` |
| Models | ModelScope / HF `…/HumanoidArena_models` |
| Assets / Raw | Drive 资产 + ModelScope raw |
| Multicam | **待发布** |

## 源码运行时序图

节点对齐 [`sources/repos/humanoidarena.md`](../../sources/repos/humanoidarena.md)。

```mermaid
sequenceDiagram
    autonumber
    actor U as 用户
    participant TW as TWIST2/teleop.sh
    participant IL as isaaclab_twist2_g1<br/>run_twist2 / run_sonic
    participant NPZ as NPZ 录制
    participant LR as lerobot/ 转换与训练
    participant EV as docs 评测脚本
    participant GMT as TWIST2 或 SONIC GMT
    U->>TW: PICO egocentric + 动捕遥操作
    TW->>IL: 35D 共享参考
    IL->>GMT: action provider 解释为 G1 目标
    GMT-->>NPZ: obs / state / action / video
    NPZ->>LR: 64D state + 40D 中间动作 → LeRobot
    U->>LR: 训练高层 IL / VLA 策略
    U->>EV: Base / Vision / Semantic / Execution / Cross-GMT
    EV->>GMT: 固定或切换后端评测
    GMT-->>U: 成功率与失败模式
```

关键复现路径：`setup_humanoidarena_envs.sh` → `teleop.sh` + `run_twist2.sh`/`run_sonic.sh` → 下载数据集与 models → 按 `docs/03_evaluation.md` batch 评测。
...
- **局限：** **仿真优先**；真机闭环与 sim2real 未作为主叙事；**multicam 数据仍待发布**；部分结果展示以论文 PDF 为准。
...
- [HumanoidArena 项目页归档](../../sources/sites/humanoidarena-github-io.md)
- [HumanoidArena 代码归档](../../sources/repos/humanoidarena.md)
- 论文 PDF：<https://arxiv.org/pdf/2606.17833>

## 推荐继续阅读

- [项目页](https://humanoidarena.github.io/) · [GitHub](https://github.com/William-wAng618/HumanoidArena)
