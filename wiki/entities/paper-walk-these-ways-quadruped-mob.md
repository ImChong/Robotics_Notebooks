...
**MoB 把「泛化」从「再训练一遍」改写成「部署时调参」：在同一平地训练分布上预埋一族等价最优解，由行为参数 \(b\) 索引，让现场试错代替重训。**

- 真正起作用的机制是 **多个等价最优解 + 结构化行为空间**：同一任务奖励下用 \(b\)（频率、摆腿高度、躯干姿态等）索引不同步态族，而不是靠把奖励偏置到某一种辅助行为。
- 适用边界很清楚：训练只在平地，OOD 能力全部来自 **部署时对 \(b\) 的在线调节**；楼梯、滑地、侧推能否解决，取决于可行解是否落在这族行为里。
- 调参主体是 **人类或高层模块**，本文并不给出自动选 \(b\) 的方案——这是使用时要自己补的一环。
- 低层仍是经典 PD 力矩接口，因此与 legged_gym 式默认 PD 路线天然衔接：本页的建议顺序是 **先拿到可切换的低层行为族，再决定增益随机化区间**，参见 [Legged / Humanoid RL 中 Kp/Kd 设置](../queries/legged-humanoid-rl-pd-gain-setting.md)。
- 本页刻意不复述量化结果：消融与 sim2real 数字以原文 PDF 为准；引用时注意 arXiv 号是 **[2212.03238](https://arxiv.org/abs/2212.03238)**，别与「Walk in Minutes」混为一谈。
- 与 [Learning to Adapt（Nature MI 2025）](./paper-learning-to-adapt-bio-inspired-quadruped-gait.md) 对照：本文走 **连续行为参数**，后者走 **离散 gait ID + 参考轨迹 + 生物力学指标** 的在线切换，两者是同一问题的两种索引方式。
