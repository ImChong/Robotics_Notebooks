...
**这篇工作真正交付的不是「人形能走」，而是把「盲走的全尺寸人形」压成一条可复述的 sim→厂商仿真→真机流水线，并把动作接口显式落在关节 PD 上。**

- 起作用的机制是 **教师–学生两阶段 + 因果 Transformer 的上下文内适应**：学生只吃本体观测与动作历史，靠自注意力在不更新权重的前提下适应地形与动力学变化。
- 可部署性来自 **闸门式流水线** 而非单点 sim2real 技巧：Isaac Gym 千级并行 + 地形/动力学随机化训练 → 厂商高保真仿真过闸 → 真机 Digit。
- 适用边界是 **盲走 / POMDP 设定**：观测不含外感知，对需要地形前瞻规划的场景不适用；Digit 的闭链机构在仿真中需专门处理，换本体不能照搬。
- 对 Kp/Kd 的引用要克制：本页只把它当 **人形 PD 量级的文献锚点**，逐关节增益以原文附录表为准，流传的髋部 \(K_p\approx 200\) 只是可读锚点，且必须与 **策略时间步 / 仿真子步 / PD 更新率** 放在同一张系统图里核对。
- 本页为方法结构与交叉引用层面的整理，定量指标、消融与实机结果不在正文，需回到 [参考来源](#参考来源) 的原文。
...
- [机器人论文阅读笔记：Real-World Humanoid Locomotion with RL](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/03_High_Impact_Selection/Real-World_Humanoid_Locomotion_with_RL/Real-World_Humanoid_Locomotion_with_RL.html)
