...
### 自回归 3D Gaussian 场景生成（示例：GaussianGPT）

[GaussianGPT](../entities/paper-sa-2603-26661-gaussiangpt.md)（ECCV 2026 Oral，arXiv:2603.26661）走 **VQ 离散化 + GPT 式 next-token 预测** 的 **显式 3DGS 生成** 路线：稀疏 3D CNN VQ-VAE 将 per-voxel Gaussian 压成 token 网格，固定 xyz 序列化后由带 **3D RoPE** 的因果 transformer 建模；**无条件生成、completion 与 outpainting** 共用同一 AR 采样（温度 / top-k 可控）。相对 diffusion 的 holistic refine，强调 **逐步组合** 与 **上下文编辑**；相对 [GaussianDream](../entities/paper-sa-2605-20752-gaussiandream-a-feed-forward-3d-gaussian-world-m.md) 等 **操纵 feed-forward WM**，更贴近 **Sim2Real / 仿真 splat 资产** 的内容先验。**已开源**（[`nicolasvonluetzow/GaussianGPT`](https://github.com/nicolasvonluetzow/GaussianGPT) + TUM 托管 checkpoint）。
...
### 可执行代码世界与 test-time Q 搜索（2026 样本）

- **[Code-as-World](../entities/paper-code-as-world.md)**（arXiv:2608.27549，MirroS）：把物理组成、动力学与渲染写成 **可执行代码**，经 **agentic propose–execute–render–verify** 发现环从语言/视频恢复世界；**Code-as-World-VL** 在 **QuantiPhy** 上 SOTA，GitHub 已开源推理/eval。与像素 rollout WM 互补：强项是 **定量、可编辑** 机制表示。
- **[QWM](../entities/paper-qwm.md)**（arXiv:2608.17163，Stanford/PKU）：学习到的 WM 用于 **Q-learning 上的 test-time 树搜索** 选动作，**不在 imagined rollout 上训练** policy/critic；Robomimic + LIBERO 样本效率显著提升，官方代码 **待发布**。
