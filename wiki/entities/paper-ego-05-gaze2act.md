...
**Gaze2Act 的定位是给 VLA 补一条语言之外的意图通道：把人的第一视角 gaze 跨视角映射到机器人视角，再作为策略的条件输入。**

- 它解决的不是「听懂拿杯子」，而是**拿哪个、从哪里接近、目标什么时候变了**——这类空间指代语言难以精确描述，而视线是低负担、高密度的信号。
- 关键机制是跨视角映射：gaze 必须先落到机器人视角里才谈得上作为条件输入，这一步不是可选的预处理，而是方法成立的前提。
- 适用边界：输入前提是人在场持续提供注视信号，因此天然属于「02 人→机器人」的协作/示教场景（见[分类 hub](../overview/ego-category-02-human-to-robot.md)），不覆盖无人监督的自主闭环。
- 风险提示：Ego 视频不天然等于机器人策略数据，仍需重建、对齐、重定向与物理过滤；本页为策展索引级归纳，量化指标以原文 PDF 与项目页 <https://zuo-kuangji.github.io/Gaze2Act/> 为准。
