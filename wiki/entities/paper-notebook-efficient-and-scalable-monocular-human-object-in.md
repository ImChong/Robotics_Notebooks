...
**Efficient and Scalable Monocular Human-Object Interaction Motion Reconstruction** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：14_Human_Motion），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇的关键不是又一个重建算法，而是把 4D HOI 数据的单位成本从「逐帧密集标注」压到「平均 6.7 个点 / 约 10 分钟一条」，让互联网单目视频真的能当交互数据源用。**

- 降成本是可扩展性的前提，但精度靠 **InterPoint 接触点预测 + 4DHOISolver 两阶段优化**补回来——稀疏标注不等于弱约束，这是该范式能成立的关键。
- 贯穿全文的第一性约束是**接触**：接触/碰撞/掩码损失把重建压到物理合理，接触引导奖励再驱动 RL 模仿；这条「接触约束贯穿重建与控制」的链路，对人形动作跟踪与操作的奖励设计可直接借鉴。
- 验收标准选得比重建误差更硬：用 **RL 智能体能否在仿真里复现交互动作**来证明数据可用，呼应 Kimodo / OMG 的「重建/生成动作即下游策略数据源」范式。
- 主要失败模式是**手物耦合的误差放大**：手部估计误差会传导并放大到物体位姿，这也提示人形灵巧操作里手部精度是瓶颈而非细节。
- 产出为 Open4DHOI 数据集，项目主页与代码仓库已列出，但论文声明数据与代码**将**公开——工程排期应按「尚未完整可用」而非「已开源」预期。
