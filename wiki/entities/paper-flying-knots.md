...
**Flying Knots 用「单次人类示教 + 轻量粒子绳模型 + 真机迭代修正」换掉大规模示教与海量仿真，代价是把泛化压缩到任务族与绳型参数以内。**

- 真正省样本的是把 ILC **从轨迹级降到任务级**：只对 critical point（首次自碰撞）的位置/速度误差加权，配 follow-through 项约束碰撞后短时跟踪，从而绕开可变形体全状态建模。
- 效率量级明确：7 种绳索（直径 **7–25 mm**、线密度 **0.013–0.5 kg/m**）各 **≤10 次真机试验达 100% 成功**，多数绳型间 **2–5 次** 即可迁移。
- 每轮的机制是 **局部线性化逆模型 QP**（Δx = M Δu）把任务误差映射为 Bézier 命令更新——可解释、可调试，但也意味着依赖模型在当前命令邻域内有效。
- 主要瓶颈在感知与标注：需 **Vicon** 跟踪绳 marker，critical point 与绳链标注是半人工管线，难直接搬到纯视觉场景；平台为 **xArm7 单臂**，与双手灵巧/人形全身栈有 gap；仓库为研究快照而非产品。
- 与 [Imitation Learning](../methods/imitation-learning.md) 共享人类示教起点但随即分道：后续是 **模型驱动迭代修正**，而不是扩充数据集做行为克隆。
