...
**Humanoid World Models: Open World Foundation Models for Humanoid Robotics** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：11_Simulation_Benchmark），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇的取舍很明确：不追生成保真的上限，而是把「以控制输入为条件的人形世界模型」压到 1–2 张 GPU 能跑，把开放世界预测的门槛降给学术与小实验室。**

- 关键数字是参数共享带来的 **33–53% 体积缩减**，且性能与视觉保真几乎无损——这是"可用性"层面的贡献，不是能力上限的突破。
- 两条生成路线并行验证：**掩码 Transformer** 与**流匹配**，同在 100 小时演示数据上训练，覆盖了当前生成式世界模型的两种主流选择。
- 模型输出是以控制输入为条件的**第一视角未来视频**，因此它服务的是"想象后果"式的规划与数据增广，而不是直接充当控制器。
- 适用边界由轻量化本身划定：面向算力受限的研究场景，不应期待它在开放世界覆盖上与更大规模模型等价。
- 与本页提到的 ZeroWBC / EgoHumanoid 的 egocentric 路线呼应，共享"第一视角"这一条件输入形式。
