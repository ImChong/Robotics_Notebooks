...
- **误区：Nav 模型 = 仅 VLN 榜。** 同一权重还覆盖 **ObjNav、跟踪、NAVSIM 驾驶**；评测需按 **任务 mode** 切换。对照端到端驾驶专用 VLA（如 [S²-VLA](./paper-s-squared-vla.md) 的双流规划）时，注意 **任务协议与 backbone 规模** 不同，PDMS 数字不可直接横比。
