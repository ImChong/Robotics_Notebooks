...
summary: "SMPC-to-RL（RAI/TUM/ETH，arXiv:2608.12063）：仿真 SMPC 当可交互专家数据机，稀疏奖励 offline-to-online FastTD3 接冻结 ReLIC 低层。Spot 推箱/扶胎/滚胎与 G1 推箱真机可部署，策略比教师更快。项目页未列代码。"
---
...
| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| SMPC | Sampling-based Model Predictive Control | 本文的仿真专家：采样轨迹 + warm-start，不部署到真机 |
...
| O2O | Offline-to-Online RL | 先用专家转移垫 critic，再撤出转纯在线 |
...
- **开源边界：** 截至 **2026-08-14** 项目页无 GitHub。judo 是通用采样 MPC 工具箱，不是本管线。
...
| 出处 | arXiv:2608.12063（2026-08）；Under submission |
| 平台 | 带臂 Spot；Unitree G1 |
| 栈 | MuJoCo Warp + mjlab；改版 FastTD3；低层 ReLIC |
| 开源（截至 2026-08-14） | **确认未开源**：项目页与 PDF 未列官方仓 |
...
| **稀疏 \(r\)** | 到目标 0；摔倒 \(-2/(1-\gamma)\)（\(\gamma=0.99\)）；否则 \(-1\) |
| **Replay 混合** | 早期 50% 专家转移；成功率过 ~10% 后撤出 |
| **有界 critic** | Q 夹在稀疏回报的理论 min/max，抗过估计 |
| **SMPC 采数** | tiled GPU；约 1M 样本/小时；最难任务 ~4M / 4 GPU 小时 |
...
**不适用**（截至 2026-08-14）：项目页与论文未提供训练/部署仓；judo 不能复现本文 tiled 采数 + FastTD3 + ReLIC 栈。放出后应补：SMPC tile → 专家 buffer → FastTD3 混合 replay → 导出高层策略 → 真机接 ReLIC。
...
- judo 采样 MPC 工具箱（**不是**本文官方仓）— 见项目页引用
