...
**这台机器人真正解决的不是「能飞又能游」，而是把界面穿越、水下降阻、多表面吸附三件互相打架的工程约束，用被动机构一次性理顺。**

- 起作用的是两个 **被动件** 而非更多驱动：PMP 靠扭簧预载与介质阻力差自动折叠/展开（阻力系数降约 **62%**）；RSD 靠主动泵建压 + 被动止回阀锁定（断电维持负压，吸附力 **>4× 机重**）。
- 关键指标是 **0.35 s** 空→水穿越与 **>20 次** 重复穿越无密封失效——它把验证口径从「概念演示」推到真实溪流与海洋的 **环境鲁棒性**。
- 适用边界清楚：桨叶折叠后推力大幅下降，水下速度/深度/续航 **不能与专业 AUV 对标**；吸盘在极光滑表面（油漆钢板）长时动态载荷下吸附时间下降。
- 主要风险在 **模式切换与载荷预算**：四模态切换需精确状态估计，入水冲击要求 IMU/飞控专门鲁棒化；防水舱 + 泵 + 电池挤占任务传感器空间。
- 复现门槛高：截至 2026-07-20 **无官方代码或 CAD 公开**，Supplementary 仅给结构参数与材料说明。
