...
**TesserAct 的定位是「级联世界模型的 4D 开源起点」：把预测目标从 2D 视频扩到 RGB + 深度 + 法线，再交由动作预测解出控制，用几何维度换跨域泛化。**

- 真正起作用的是 **4D 预测目标本身**——多出的深度与法线通道给了它相对 2D 视频 WM 的跨域泛化余量，而不是更大的生成骨干。
- 级联架构的取舍是对称的：模块易拆易复用（工程友好），代价是 **误差沿各段传递**；这是路线 01 的共性，不是本工作独有的缺陷。
- 本页为策展索引级：出处（ICCV 2025）与文内引用可查，但 **量化 benchmark、消融与实机指标须回原文 PDF / 项目页**；复现前核对 License 与权重，引用量与下游控制增益之间没有简单线性关系。
