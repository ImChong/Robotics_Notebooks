...
**Stability-Aware Retargeting for Humanoid Multi-Contact Teleoperation** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：07_Teleoperation），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇管的不是「重定向到哪」，而是「重定向时别失稳」：把 actuation-aware 质心稳定区域做成可微裕度，让遥操作回路能在不夺走操作员控制权的前提下，把机器人往更稳的位形拉。**

- 真正的技术关键是 **LP 灵敏度分析**：一次性得到稳定裕度对接触点与位形的解析梯度，免去反复重解 LP，才够 kHz 级实时嵌进遥操作 IK/QP。
- 共享自治的分寸靠工程细节保证：零空间投影 + 三档策略 + 迟滞，使稳定性修正成为操作员意图的补充而非覆盖。
- 适用边界是多接触作业——撑墙、撑天花板、按在斜面/不平表面这类接触面不共面、不规则的情形；它盯住的两个失败模式正是摩擦锥外打滑与关节力矩饱和。
- 可复用性大于本文用法：可微的稳定裕度还能进 RL 奖励、MPC 代价与共享自治，不必绑定遥操作场景。
- 与同模块的 SEW-Mimic 互补：后者给「重定向到哪」的闭式几何解，本文提供「重定向时」的稳定护栏。
- 落地状态：在 IHMC 液压-电驱人形上完成仿真与真机双验证，但截至记录日未见公开仓库。
