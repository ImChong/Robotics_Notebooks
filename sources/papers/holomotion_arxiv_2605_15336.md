...
- **实验：** **五个未见** 运动数据集零样本评测；相对最强基线（Sonic）**全局 MPKPE 约降 40%**；**KV-cache 推理最高约 11×**、**序列级 PPO 训练最高约 22×** 效率；**直接真机迁移**（无任务特化微调）。
- **规模（v1.3+ 工程对齐）：** 混合语料 **2000+ h**；策略容量至 **0.4B** 参数量级（以官方 README / 权重发布为准）。
