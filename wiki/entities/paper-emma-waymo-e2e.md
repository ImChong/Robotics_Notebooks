...
**在 Gemini 多模态大模型上把导航/状态输入与轨迹/检测/路网输出统一成自然语言，实现真正的多任务统一空间并涌现多任务互相增益。**

1. **一切皆语言** — 相机 + 导航/状态文本化输入，轨迹/3D 目标/路网元素亦表示为自然语言，同一模型换 Prompt 即可联合规划、检测与建图。
2. **多任务互增益** — nuScenes 与 Waymo Open 具竞争力；内部大数据的 EMMA+ / EFM+（w/ CoT）在 WOMD 等设定相对 MotionLM / Wayformer 显著提升（论文表）。
3. **可解释** — 可输出驾驶理由（CoT），缓解纯数值 E2E 黑盒；骨干为 Gemini，亦验证迁移到 PaLI-X 等开源 MLLM（EMMA†）。
4. **相对 DriveVLM** — DriveVLM 外挂式 CoT 更易接传统栈，EMMA 在表示层把任务空间语言化以榨取预训练世界知识。
5. **局限** — 未开源、专有数据 + Gemini 依赖，外部无法完整复现；语言化坐标仍受分词/数值精度与延迟制约，且勿与具身 Ego EMMA（2509.04443）混淆。
