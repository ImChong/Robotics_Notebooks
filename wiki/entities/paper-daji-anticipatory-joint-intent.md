...
**DAJI 争的不是「运动生成更像人」，而是语言与电机之间那层身体 API 该长什么样：它主张这层接口必须自带对下一步物理过渡的预期，否则流式指令下人形接不住自己的上一段动作。**

- 真正起作用的是 **DAJI-Flow 低频意图 + DAJI-Act 高频解码共享同一 joint-intent 空间**——生成输出直接被执行侧消费，而不是先转成易与当前动量/接触脱节的外部参考轨迹。
- 「预期」是可证伪的：去掉未来相关信息后，约 60s 闭环成功率从约 **87% 跌至约 10%**，说明片段间 **身体状态连续** 比单段动作美观更决定成败。
- 接口宽度本身是关键超参：64 维 MLP 成功率约 **80.8%**、CPU 延迟约 **4.71 ms**，16 维仅约 **49.5%**——可部署性来自窄而对的接口，而非更大的生成模型。
- 边界：开放场景的地形、碰撞、人机交互与失败恢复超出单一意图表示能承担的范围。
- 与 [Teleoperation](../tasks/teleoperation.md) 页的 OmniH2O 同属「身体接口」争夺线，但 DAJI 只押 **语言流式 + 预期意图** 这一格；相对 TextOp / MotionStreamer，它比的是闭环可部署性而非单段 kinematic 质量。
