...
**这项工作的突破点在驱动方式而非步态：用 DEA 人工肌肉替掉电机齿轮箱与气泵，蠕虫式机器人才能压到 2.2 g / 47 mm、进入外径 < 10 mm 的刚性机器人禁区并仍留得下摄像头。**

- 真正让它前进的是「三段蠕动 + 摩擦各向异性」——前向摩擦小于后向摩擦产生净位移，配合三相时序，在直/L/S/螺旋管、玻璃/金属/碳纤维壁与空气/油液介质中均可推进，速度 > 1 体长/秒。
- 它把演示做到了工具级而非运动级：机头集成微型摄像头并线缆回传实时视频，构成可用的遥控内窥检测闭环——能动，也能看。
- 适用边界由供能决定：kV 级高压驱动电路与线缆供电把可探索长度限制在数十厘米量级，无线化需先解决微型高压发生器；医疗级 < 3 mm 管径尚未演示。
- 主要失败模式是机动性缺口——目前只演示单向蠕动前进，未见原地倒退或转向，复杂管网需额外导向机构；DEA 薄膜长期高压循环的疲劳寿命论文也未系统评测。
- 落地与复现：截至入库日无公开代码或 CAD，复现依赖论文 Methods 与 SI；相对传统管道机器人（通常 > 50 mm、数百克起）与气动软体（需外部气源），本方案的独特位置是「亚厘米 + 无气源 + 检测集成」三者同时成立。
