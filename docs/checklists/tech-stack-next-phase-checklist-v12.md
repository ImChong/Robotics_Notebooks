...
最后更新：2026-04-18（V12 启动，基于 V11 完整交付）
...
- [ ] 在 `wiki/formalizations/gae.md`（GAE 依赖 Bellman 方程）的 `## 关联页面` 中添加 `bellman-equation.md` 回链
- [ ] 在 `wiki/methods/reinforcement-learning.md` 的 `## 关联页面` 中添加 `bellman-equation.md`
- [ ] 在 `wiki/formalizations/mdp.md` 的 `## 关联页面` 中添加 `bellman-equation.md`
- [ ] 在 `wiki/overview/robot-learning-overview.md` 中引用 `llm-wiki-karpathy.md`（知识库方法论入口）
- [ ] 在 `wiki/roadmaps/humanoid-control-roadmap.md` 的维护说明处引用 `llm-wiki-karpathy.md`

### P0.2 · 图谱节点健康着色（前端增强）

**目标**：在 `docs/graph.html` 中新增第三种着色模式——"按健康度"，帮助发现质量偏低的页面。

- [ ] 在 `scripts/generate_link_graph.py` 中，为每个节点添加 `health_score` 字段（0-3）：
  - `+1`：frontmatter 有 `summary:` 字段
  - `+1`：有 `sources:` 字段且非空
  - `+1`：`updated:` 字段距今 ≤ 365 天
- [ ] 在 `docs/link-graph.json` 中随节点一同输出 `health_score`
- [ ] 在 `docs/graph.html` 添加"按健康度"着色按钮（绿=3分、橙=2分、红=0-1分）
- [ ] 同步更新 `docs/exports/link-graph.json`

### 完成标准
- [ ] `graph-stats.json` 中 `singleton_communities` 列表为空
- [ ] `docs/graph.html` 有"按健康度"着色选项，点击后节点重新着色
...
- [ ] 新建以上 3 个 query 页（格式含 Query 产物说明 + 对比表 + 关联页）
- [ ] `make lint` 保持 0 issues

### P1.3 · 学习路径骨架（Roadmap Learning Paths）

Karpathy："*good answers can be filed back as new pages*" — 用社区结构生成学习路径。

- [ ] 新建 `roadmap/learning-paths/` 目录
- [ ] 新建 `roadmap/learning-paths/if-goal-locomotion-rl.md`（从 RL → Locomotion → Sim2Real 的最短路径）
- [ ] 新建 `roadmap/learning-paths/if-goal-manipulation.md`（从 IL → Manipulation → Loco-Manip 的路径）
- [ ] 每个路径页包含：前置知识 → 核心概念 → 实践 Query → 延伸阅读（格式统一）
- [ ] 在 `index.md` 和 README 的"从哪里开始"表格中添加新路径入口

### 完成标准
- [ ] `python3 scripts/search_wiki.py "调试策略"` 返回 `robot-policy-debug-playbook.md` 前 3
- [ ] `make lint` 0 issues，graph 节点数 ≥ 96
...
- [ ] 添加 10 条，总计 **40 条** CANONICAL_FACTS
...
- [ ] 检查 `log.md` 最近 30 天是否有记录，若无则追加一条当前 lint 通过记录
- [ ] 确认 log 格式：`## [YYYY-MM-DD] <操作类型> | <说明>` 可被 `grep "^## \["` 解析
- [ ] lint 的 `log_inactive` 检测项正常工作（非 0 天静默）

### P2.3 · 搜索回归用例扩展

- [ ] 将 `schema/search-regression-cases.json` 扩展至 **18 条**（新增 CBF/CLF/DAgger/VLA/双臂/地形等新页面用例）
- [ ] `python3 scripts/eval_search_quality.py` 通过率维持 ≥ 80%

### 完成标准
- [ ] `make lint` CANONICAL_FACTS 覆盖包含 V11 所有新增领域
- [ ] 搜索回归 18/18 通过（或 ≥ 80%）
- [ ] `log.md` 最近记录距今 ≤ 30 天
...
- [ ] 在 `scripts/ingest_paper.py` 中添加 `--suggest-updates` 标志：
  - 读取新 source 文件的标题和关键词
  - 在 wiki/ 中搜索提及相关关键词的页面
  - 输出："以下页面可能需要根据新来源更新：`wiki/xxx.md`"
- [ ] 在 `Makefile` 中更新 `ingest` 目标，默认加 `--suggest-updates`

### P3.2 · Index 自动同步检查

Karpathy："*LLM updates index on every ingest*"

- [ ] 在 `check_export_quality.py` 中添加检查：
  - `index.md` 的最近修改时间 ≤ `exports/index-v1.json` 的修改时间 + 1天（同步检测）
  - 若 `index.md` 落后，输出 `⚠️ index.md 可能未及时更新，建议 make catalog`

### 完成标准
- [ ] `python3 scripts/ingest_paper.py --suggest-updates` 能对新 source 给出相关 wiki 页推荐
- [ ] `make export-check` 包含 index.md 同步检测（共 11 项）
...
- [ ] 新建 `docs/manifest.json`（PWA manifest，含 name / short_name / icons / start_url）
- [ ] 新建 `docs/sw.js`（Service Worker，缓存 `search-index.json`、`link-graph.json`、`site-data-v1.json`）
- [ ] 在 `docs/index.html` 的 `<head>` 中注册 Service Worker 和 manifest
- [ ] 首次访问后，断网仍可搜索和浏览图谱
...
- [ ] 在 `scripts/update_badge.py` 中新增图谱节点/边数 badge 自动更新（从 `graph-stats.json` 读取）
- [ ] 消除 README 中手动维护节点数的需求（与 sources badge 同步自动化）

### 完成标准
- [ ] `docs/manifest.json` 和 `docs/sw.js` 存在，`index.html` 已注册
- [ ] 图谱 badge 由 `make badge` 自动更新，无需手动改 README
...
- [ ] 加深 `wiki/tasks/teleoperation.md`（补充：数据采集流程 / ACT/Diffusion Policy 与遥操结合 / 硬件方案对比）
- [ ] 加深 `wiki/concepts/gait-generation.md`（补充：周期 vs 非周期步态 / CPG 方法 / 与 MPC 结合）
- [ ] 加深 `wiki/formalizations/contact-complementarity.md`（补充：互补约束数学形式 / LCP 求解 / 与 QP 的关系）
...
| Karpathy 原则 | V11 末状态 | V12 目标 |
|--------------|-----------|---------|
| Three-layer architecture | ✅ | ✅ 维持 |
| 0 orphans，交叉引用完整 | ✅ 90 节点 521 边 | ✅ ≥ 98 节点，0 孤立社区 |
| "Good answers filed back" | ✅ query 页持续增加 | ✅ +3 query 页 + 学习路径骨架 |
| Lint 检测项 | ✅ 17 项 | ✅ 17 项（稳定） |
| CANONICAL_FACTS | ✅ 30 条 | ✅ **40 条** |
| Sources 覆盖率 100% | ✅ | ✅ 维持 |
| Hybrid BM25/vector | ⚠️ hashed fallback | ⚠️ 维持（环境限制）|
| 浏览器端搜索 | ✅ + 解释层 | ✅ + loading 状态 + PWA |
| 知识图谱节点健康 | ❌ | ✅ health_score 着色 |
| 孤立社区 | ⚠️ 2 个 | ✅ 0 个 |
| Ingest → suggest updates | ❌ | ✅ `--suggest-updates` |
| Log.md 活跃 | ⚠️ 可能静默 | ✅ 距今 ≤ 30 天 |
| 学习路径 | ❌ | ✅ 2 条路径页 |
| 图谱 badge 自动化 | ⚠️ 手动维护 | ✅ `make badge` 自动更新 |
...
- [ ] `make lint` 0 issues，CANONICAL_FACTS = 40 条
- [ ] `graph-stats.json` 无 singleton_communities
- [ ] graph 节点数 ≥ 98
- [ ] `make export-check` 11 项全通过
- [ ] `docs/manifest.json` 存在（PWA 支持）
- [ ] `make badge` 自动更新图谱 badge（无需手动改 README）
- [ ] `roadmap/learning-paths/` 下有 ≥ 2 个路径页
- [ ] `log.md` 最近记录距今 ≤ 30 天
