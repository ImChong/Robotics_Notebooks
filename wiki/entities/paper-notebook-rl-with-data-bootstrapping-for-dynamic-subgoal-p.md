...
**这是一篇务实分工的工作：让 RL 只负责「往哪走」的动态子目标，把「怎么稳稳走过去」整段交给 MPC，再用模型法生成的数据给 RL 冷启动。**

- 起作用的是两件事：**分层结构** 避开了端到端 RL 直接学稳定步态的难度；**数据自举** 借现成的基于模型导航方法生成多样、信息丰富的数据集，化解 RL 训练慢且不稳的冷启动问题。
