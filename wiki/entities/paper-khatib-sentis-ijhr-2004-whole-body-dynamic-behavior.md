...
**先说清楚这页没有什么：** 本文是 **理论/框架** 论文，不提供标准基准、成功率表或可复现评测协议——2004 年人形 WBC 也还没有这类共同基准。把它当「有数字的方法论文」读会读空。真机与工程集成叙述在 [ICRA 2006 框架](./paper-sentis-khatib-icra-2006-whole-body-control-framework.md) 与 Sentis 2007 博士论文里展开，本文停在数学结构与仿真级演示。

要验证这套框架是否在自己的栈上成立，该测的是 **优先级语义有没有被真正执行**，而不是任务成功率：

| 验收项 | 怎么测 | 通过判据 |
|--------|--------|----------|
| 约束不被下层破坏 | 给操作任务下发一个会违背平衡/限位的指令 | 约束层吸收，平衡与限位仍满足；不是靠指令饱和兜底 |
| 零空间投影是动力学一致的 | 只驱动姿态任务，观察操作任务加速度 | 操作任务加速度 **不受扰动**（用 $N=I-J^\top\bar J^\top$ 而非伪逆运动学投影，见 [null-space-control](../concepts/null-space-control.md)） |
| 浮基与接触进了同一方程 | 改变支撑接触（单脚/双脚切换） | 任务力矩随支撑分布改变；若不变，说明支撑力被当成外部后处理 |
| 层级可行性 | 给出彼此冲突的多任务 | 低优先级在冲突方向上被自然截断，而非全局加权折中 |

**读数提醒：** 本页与 [OSF 1987](./paper-operational-space-formulation.md)、ICRA 2006 同属理论线，三者之间不存在可横比的量化指标；要量化对比 WBC 方案，应在 [TSID](https://github.com/stack-of-tasks/tsid) / [ControlIt!](./controlit.md) 等现代实现上用同一机器人与同一任务集重测，并把「仿真结果 / 真机证据 / 仅演示视频」分开记账。
