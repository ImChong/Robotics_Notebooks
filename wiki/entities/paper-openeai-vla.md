...
| 维度 | OpenEAI-VLA（本页） | [π₀](./paper-pi0.md) | 仅开源软件栈的 VLA |
|------|----------------------|------------------------|---------------------|
| 覆盖范围 | **硬件 + 软件一体**：OpenEAI-Arm（6+1 DoF）+ VLA | 策略与模型 | 仅模型/训练代码 |
| 数据来源 | 两阶段训练 **仅用公开数据集** | 自有大规模数据 + 公开数据 | 视项目而定 |
| 动作侧 | Qwen3-VL-4B 骨干 + Diffusion Transformer 动作头 + rolling action-chunk 插值 | 流匹配动作专家 | 视项目而定 |
| 复现门槛 | 需复刻臂体；软件侧 **待发布** | 需算力与数据 | 取决于权重是否放出 |

- **「仅用公开数据集就对标 π₀」是本文最值得核的一条：** 若成立，它把 VLA 的门槛从 **私有数据** 移到 **工程实现**；但这属作者报告，且代码 **待发布**，在第三方复现前应当作 **待验证** 而非既成结论。
- **硬件比较的口径：** 「OpenEAI-Arm 在同策略下优于两款商业 6+1 臂」比的是 **同一策略换本体**，衡量的是机械/控制侧（MDH 优化 + 动力学补偿 PID），与模型能力无关，不要合并进 VLA 的成绩单。
- **逐项数值：** 四任务真机成功率与各 baseline **以 [原文 PDF](https://arxiv.org/abs/2606.03392) 为准**；本页为索引级摘要。
