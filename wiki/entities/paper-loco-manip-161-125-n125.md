...
**这篇的命题不是「怎么踢得更快」，而是「观测有噪声时还能不能敏捷」：用特权信息教师加学生蒸馏，把仿真里的理想状态换成部署时真拿得到的带噪观测。**

- 真正起作用的是教师-学生知识迁移这一层：教师用特权信息训练，学生只依赖部署观测，再由 PPO/RL 与 WBC/MPC 输出可执行命令。
- 前提是仿真交互数据能覆盖真实的噪声分布；噪声模型与实机不匹配时，蒸馏带来的鲁棒性会明显打折。
- 足球射门属于窄域接触任务（06 特殊任务、接触规划与视觉闭环坐标），不等价于通用移动操作能力。
- 有公开项目页；本页为清单索引条目，量化 benchmark 与实机指标以原文 PDF / 项目页为准。
