---
type: overview
tags: [depth, actuator-drive-chain, actuator, eda, foc, motor-control, ethercat, sim2real, hardware]
status: complete
updated: 2026-07-31
related:
...
summary: "执行器驱动链选型闭环纵深枢纽：把 EDA 电路设计 → 电机驱动固件 FOC → 执行器建模与摩擦辨识 → 实时总线闭环集成 四层驱动链，从分散的电子硬件/驱动固件/执行器建模实体页收拢为一条可导航的选型链，统一各层选什么、数据手册标称参数与实测曲线差在哪、建模保真度 vs 辨识成本如何取舍、总线周期 ≠ 闭环带宽的入口。"
---

# 执行器驱动链选型闭环（纵深汇总）

> **纵深定位**：本页是「EDA 电路设计 → 电机驱动固件 FOC → 执行器建模与摩擦辨识 → 实时总线闭环集成」四层驱动链的统一入口，把近周密集 ingest 的 KiCad / Altium / SimpleFOC / NeuralActuator / BAM / SAGE 等电子硬件与执行器建模页从分散的实体页收拢为一条可导航的选型链。它是「[具身大模型分类学选型闭环](./depth-embodied-foundation-model.md)」（选哪一类策略）与「[具身评测基准选型闭环](./depth-embodied-eval-benchmark.md)」（怎么评测/证明它）的**硬件侧姊妹链**——回答「策略算出的力矩指令能不能被真实驱动链忠实执行」。
...
| ③ 执行器建模与摩擦辨识 | 显式摩擦模型 vs 神经执行器网络 | BAM、NeuralActuator、执行器网络 | [BAM 摩擦辨识](../entities/bam-better-actuator-models.md)、[NeuralActuator](../entities/paper-neuralactuator-neural-actuation-modeling.md)、[隐式/显式执行器建模](../concepts/implicit-explicit-actuator-modeling.md) |
...
- **[通信协议（Communication）](./depth-communication.md)**：④ 层 EtherCAT/CAN 现场总线与通信纵深共享底层数据链路。
- **[物理保真度（Physics Fidelity）](./depth-physics-fidelity.md)**：③ 层执行器建模是物理保真度四层里最靠近硬件的一层，共享 sim2real gap 归因。
- **[接触力控（Contact Force Control）](./depth-contact-force-control.md)**：力矩指令能否忠实执行直接决定力控闭环的下限。
...
- [BAM 摩擦辨识](../entities/bam-better-actuator-models.md) · [NeuralActuator](../entities/paper-neuralactuator-neural-actuation-modeling.md) · [执行器网络](../methods/actuator-network.md)
