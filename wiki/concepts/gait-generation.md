...
- **显式多步态 + 生物力学切换**：[Learning to Adapt（Nature MI 2025）](../entities/paper-learning-to-adapt-bio-inspired-quadruped-gait.md) 用 **πG + BGS + πL** 部署 **8 种 gait** 并由 **CoT/τ%/Wext/接触误差** 在线选 gait（与 [Walk These Ways](../entities/paper-walk-these-ways-quadruped-mob.md) 的 **MoB 行为参数 b** 是不同接口）
- **优点**：策略驱动，不需要手工设计步态切换
- **局限**：涌现步态可能在某些速度区间不稳定，难以保证特定步态约束；显式多 gait 栈仍依赖 **BGS/指标设计**
...
- [Learning to Adapt（Nature MI 2025）](../entities/paper-learning-to-adapt-bio-inspired-quadruped-gait.md) — BGS 步态参考 + πG 生物力学切换
- [easy_quadruped](../entities/easy-quadruped.md) — Pupper Trot 相位调度 + MuJoCo 任务序列调参实例
