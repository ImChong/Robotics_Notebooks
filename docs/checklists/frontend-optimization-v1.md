...
- [ ] 图谱专项，#6：按下节 G0–G6 推进；先测量与减负，再做真实子图，Canvas/Worker/3D 批绘制由瓶颈决定。
- [ ] 并行门禁，#35/#11/#23/#24：每项记录同内容冷/热缓存、首次搜索和详情切换指标；版本兼容、数据契约、浏览器交互随变更验收，真机/GPU缺失明确留空。每完成一项独立提交并推送。


## 2026-09-08 图谱动态渲染性能计划

完整依据、估时、设计约束与验收见 [工程计划图谱专项](../../plan.md#2026-09-08-图谱动态渲染性能专项)。G0 已于 2026-09-08 完成并改写了 G4/G5 的先后判断；G1–G6 实现与真机性能验收仍未完成。原 #6 拆为 G1–G6，关联 #9/#24/#35，其他全站任务状态不变。
...
- [ ] G2 / #6：3D 普通边细线与独立粗线高亮、悬停差量更新、标签按需创建与投影；复核拖拽加热，不能将细线视为自动批绘制。
- [ ] G3 / #6：Top N/社区/路线使用真实模拟与渲染子集；缓存位置、恢复全图，保持原始邻接关系和 ID；固定背景与活跃模拟分开。
