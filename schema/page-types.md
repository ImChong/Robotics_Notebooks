...
python3 scripts/scaffold_wiki_page.py entity "论文短名" --slug paper-xxx --paper
```
...
### `wiki/entities/paper-*.md` 附加要求（所有论文实体页）

除统一阅读骨架外，**每一篇**升格的论文实体页还必须包含：

1. **`## 结论`** — 放在「实验与评测」（或等价评测节）之后、「局限与风险」或「与其他工作对比」附近；用 **1 句总判 + 3–7 条要点**，写清读者该记住的方法/工程取舍（参考 [YAHMP](../wiki/entities/paper-yahmp.md)）。不要只重复摘要；不要用空泛「本文提出了…」代替可操作结论。
2. **「方法 / 评测 / 对比」三段式** — 正文 `##` 级标题需能命中方法栈（或流程总览/核心机制）、评测（或实验/结果）、与其他工作对比（或对比/比较）等关键词（`lint_wiki.py` 信息型巡检）。

### `wiki/entities/paper-*.md` 附加要求（有官方可运行代码时）

步骤 2.5 判定 **已开源** 或 **部分开源且仓库含可运行入口** 时，除上文外还须：
