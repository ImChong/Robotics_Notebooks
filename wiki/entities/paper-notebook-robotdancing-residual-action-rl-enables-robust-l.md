...
**RobotDancing 的关键判断是「换动作表示比加训练阶段更划算」：把策略输出从绝对关节角改成参考轨迹上的残差，长时程追踪的误差累积就退化成一个可学的小修正量。**

- 真正起作用的机制是 q^tar = q^ref + a 这一残差参数化，把仿真-实机动力学差异交给策略在线补偿，而不是靠更精细的建模或分阶段训练去消除。
- 第二个杠杆是选择性残差化：只对承重最吃紧的髋 / 膝 pitch 加残差、其余关节透传参考，用「少数 DoF 集中纠偏」换取更低方差与更稳的训练。
- 工程取向是简洁而非堆料：单阶段非对称 actor-critic PPO、一套观测 / 奖励 / 超参覆盖全部序列、无部署期后处理，LAFAN1 全 8 段在 G1 上零样本跑通并迁移到 H1/H1-2；但论文未公开代码与项目页，复现需自行实现。
- 适用边界是「有高质量参考轨迹的追踪任务」：方法依赖 LAFAN1 这类参考动作，长尾分布还需靠「分布均衡 + 失败优先」的采样课程兜住，它不是无参考的自由动作生成。
- 本页为 **深读笔记编译** 的索引级摘要，量化 benchmark、消融与实机指标以笔记页与论文 PDF 为准（见 [参考来源](#参考来源)）。
