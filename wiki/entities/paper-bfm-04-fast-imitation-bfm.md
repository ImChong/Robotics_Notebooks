...
**这一页是 survey 坐标而非深读：Fast Imitation 的主张是「有了行为基座，学新动作就该少走弯路」，把成本问题从单技能训练搬到基座复用。**

- 归位在 **01 Forward-backward 表征**（#04/41）：在无监督或多任务 RL 中学 FB / successor 结构，把异构任务压进可被 prompt 检索的身体潜空间。
- 与单技能 motion tracking 的真正分歧在评价口径——BFM 线关心 **覆盖面与可组合性**，而不只是单一参考的跟踪成功率。
- 本页 **不含量化结论**：benchmark、消融与实机指标须回到原文 PDF，引用时不要把清单索引归纳当成实验结果。
