...
**Robot Crash Course: Learning Soft and Stylized Falling** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：04_Loco-Manipulation_and_WBC），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇工作把研究对象从「怎么不摔」换成「怎么摔」：承认跌倒无法完全杜绝，于是把落地终止姿态变成推理时可指定的控制量。**

- 真正起作用的是那个 **机器人无关的三目标奖励**——终姿达成、冲击最小化、关键部件保护——三者的平衡，而不是绑定某一本体的手工落地动作。
- 泛化来自 **基于仿真的初始/终止姿态采样**：它一次买到两样东西，对广泛初始跌倒条件的鲁棒性，以及推理时指定任意（含未见过）终姿的能力。
- 适用边界是「跌倒已经发生之后」这一段：它不提升防摔能力，与防摔控制是互补而非替代。
- 奖励与本体解耦意味着跨机器人复用成本低，但落地仍需仿真与真机两侧验证——本文两侧都做了，结论是双足也能做受控的柔和跌倒。
- 同簇对照：与 SafeFall、自保护跌落、Unified Fall-Safety 同属跌落安全研究群，本文（Disney/ETH 系）的差异化在「风格化 / 可指定终姿」这一维。
