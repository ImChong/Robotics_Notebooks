...
| 路线 | 记忆载体 | 测试时更新 | 与 RoboTTT 关系 |
|------|----------|------------|-----------------|
| **RoboTTT** | **层内 fast weights**（固定大小） | **每步**自监督更新 | 本页方法；训练与部署都在线递推 |
| [TTT-Parkour](./paper-notebook-ttt-parkour.md) | 已训练策略权重 | 仿真里 ≤10 min **离线** TTT 微调再部署 | 同名缩写、不同问题：感知跑酷短时微调 |
| [WAM-TTT](./paper-wam-ttt-human-video-test-time-steering.md) | 冻结 WAM 上的 fast-weight 记忆 | 人视频**批次** TTT，依赖 meta-training 对齐 | 非机器人 visuomotor 递推 |
| GDN（Gated DeltaNet） | 固定大小循环状态 | 无（仅前向递推） | RoboTTT 的固定状态记忆基线，context scaling 无上升趋势 |
| KV / 关键帧记忆类 VLA | 变长 KV cache / 关键帧 | 无 | 记忆随上下文线性增长；尚未同 benchmark 系统对比 |
