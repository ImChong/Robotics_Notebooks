...
把「特权教师 + 本体学生 + 自适应课程」三件套落成训练到部署的流水线：

1. **特权教师训练**：在仿真中让教师直接看到地形几何、接触与环境参数真值，用 RL 学会跨越分布的运动策略。
2. **本体学生蒸馏**：学生仅接收命令与 IMU/关节历史（TCN 窗口约 2 s），用 DAgger 在学生轨迹上请教师标注，同时模仿教师动作并回归环境 latent。
3. **自适应地形课程**：以粒子式采样保留通过率约 0.5–0.9 的地形参数区间，把训练信号集中在能力边界。
4. **结构化动作执行**：策略输出相位/足端残差等结构化动作，经轨迹生成与 IK 落到关节 PD——不是无结构端到端力矩。
5. **零样本部署**：部署时删除特权与奖励，只留命令 + 本体历史，直接迁移到泥雪碎石等真实地形。

一句话读法：把「接触后才能可靠观测的地面属性」压进可部署的盲走策略。
...
| 维度 | 本工作（盲走复杂地形 / P008） | [Robust Perceptive Locomotion](./paper-robust-perceptive-locomotion-wild.md) | [Privileged Training](../concepts/privileged-training.md) | [Terrain Latent](../concepts/terrain-latent-representation.md) |
|------|------------------------------|------------------------------------------------------------------------------|-----------------------------------------------------------|----------------------------------------------------------------|
| 方法族 | 特权教师 + 本体历史学生 + 自适应课程 | 特权教师 + 循环 Belief Encoder 融合带噪高程图 | 教师-学生蒸馏范式（概念页） | 地形隐表征学习（概念页） |
| 部署观测 | 仅命令 + 本体历史（盲走） | 本体历史 + 带噪高程图（外感知，失效可退回本体） | 依实例而定 | 依实例而定 |
| 关键假设 | 接触后本体反馈比远距视觉更可靠 | 外感知有价值但会失效，需鲁棒退化 | 特权信息可蒸馏进可部署观测 | 地面属性可压成低维 latent |
| 优化目标 | 零样本穿越自然复杂地形 | 外感知失效时仍稳健行走 | 缩小特权-部署观测差距 | 可迁移地形表征 |
| 关系/取舍 | 盲走适应，不做提前选路 | 带噪地图信念，覆盖提前感知 | 本工作是其经典实例 | 本工作学生的 latent 即其实例 |
