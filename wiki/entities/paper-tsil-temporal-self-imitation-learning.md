...
**TSIL 的核心洞察是「成功轨迹的时间结构本身就是可扩展的自监督信号」：不动 PPO 主体，用自适应时间目标 + 效率加权回放，把偶发的快成功变成持续压力。**

- 两条机制互补且缺一不可：$D(\phi)$ 只负责收紧监督标杆，回放才负责让快轨迹不在不稳定的 PPO 更新中消失——与经典 SIL（按回报筛选）、ATTL（无回放）形成明确对照。
- 主表 15 任务均值成功率 **98.6%**、成功完成时间 **0.674**（IH 为 1.246），且训练期成功 episode 数最高，说明增益路径是「更多自生成成功 → 更强目标更新与回放」的正反馈。
- 前提是基座策略要能偶尔成功：**TSIL 不是探索算法**，长期零成功时无快轨迹可挖；也不替代 reward 设计，时间效率只是附加自监督。
- 时间不是唯一部署指标——需要更慢、更安全或更平滑的行为时必须与其他行为指标组合；另有 per-configuration 成功轨迹 buffer 的记忆开销未被系统研究。
- 工程侧是本页最强的一环：[generalroboticslab/TSIL](https://github.com/generalroboticslab/TSIL)（Apache-2.0）提供 Hydra 启动脚本、捆绑 Isaac Gym Preview 4 + MTBench 子集与论文图表管线，但主证据在仿真，仓库未含真机栈。
