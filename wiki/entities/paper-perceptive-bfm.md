...
flowchart TB
  subgraph offline [Stage 1：TCRS（仅训练）]
    raw["原始人体运动库\n+ 采样高程场"]
    tcrs["接触落脚 · MPPI 摆动\n根重建 · 碰撞修复 · IK"]
    ref_tcrs["地形一致参考 q_ref^tcrs"]
    raw --> tcrs --> ref_tcrs
  end
  subgraph teacher [Stage 2：盲 adapted-reference teacher]
    blind["盲 Transformer actor–critic"]
    ppo1["PPO 跟踪 q_ref^tcrs\n特权状态 · 无视觉"]
    ref_tcrs --> ppo1
    blind --> ppo1
  end
  ...
  direction TB
    inp["本体 + 历史 + raw 命令窗口\n+ 机器人中心高程图"]
    backbone["Transformer 跟踪骨干 → u_t"]
    terrain["Map CNN → MapTransformer → z_vis"]
    gate["Identity gate tanh(α)≈0\nu′ = u + tanh(α_u)·Δu\nμ = μ_base + tanh(α_a)·r"]
    align["目标帧动作对齐蒸馏\na* = (q_ref^tcrs + μ_tea) − q_ref^raw"]
    ppo2["PPO 微调"]
    inp --> backbone
    inp --> terrain
    backbone --> gate
    terrain --> gate
    ppo1 -->|"teacher μ_tea"| align
    gate --> align --> ppo2
  end
  subgraph deploy [部署]
    cmd["任意 raw 人体参考\n（动捕 / 遥操作 / 库）"]
...
- **盲 teacher：** 无视觉 Transformer，PPO 跟踪 `q_ref^tcrs`，学「若已知地形一致目标，全身应如何动」。
- **视觉 student：** 接收 **raw `q_ref`** + **局部高程图**；MapTransformer 以运动 query 读地形。
- **Identity gate：** `tanh(α)` 初值≈0 → 网络起步为 **纯 raw-reference tracker**，仅在地形逼迫时打开残差；避免感知支路破坏已有运动先验。
- **Target-frame alignment：** teacher 动作在 adapted 帧表达；蒸馏把有效 PD 目标 **重表达到 raw 帧**：`a* = (q_ref^tcrs + μ_tea) − q_ref^raw`，使跨帧蒸馏可优化（去此项 **54.6 → 50.1**）。
