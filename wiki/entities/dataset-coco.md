...
status: complete
updated: 2026-08-12
summary: "目标检测/实例分割/关键点主流基准：约 20 万图、80 物类；mAP 与机器人检测迁移评测的事实标准之一。"
related:
...
**MS COCO**：目标检测/实例分割/关键点主流基准：约 20 万图、80 物类；mAP 与机器人检测迁移评测的事实标准之一。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| COCO | Common Objects in Context | 检测分割关键点综合基准 |
...
| 维度 | 速查 |
|------|------|
| 规模 | train2017 约 118k 图；80 类实例标注。
| 模态 | RGB + bbox / 实例 mask / keypoints。
| 许可证 | COCO 条款（研究常用；商用需自查）。
| 适配形态 | 检测预训练与迁移；机器人常再 fine-tune 域数据。
| 重定向就绪度 | 不适用；检测标签可直接训练。
...
- [具身大模型评测基准选型闭环知识链](../queries/embodied-eval-benchmark-selection-loop.md) — mAP 是感知层可复现代理：检测强 ≠ ③策略任务成功率高
...
- [MS COCO 官方入口](https://cocodataset.org/)
