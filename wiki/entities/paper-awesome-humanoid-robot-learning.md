...
- **不适用（策展仓库无自有实验）**：本节点是社区策展 README + 分类链接，既不提出方法也不报告指标，没有可转存的实验表。
- **可核查的对象**：链接时效、分类是否仍在维护、以及 README 覆盖范围与库内[44 篇技术地图](../overview/freedof-sim2real-44-papers-technology-map.md) 的出入。
- **读法：** 把它当选题入口而不是证据来源；任何被它收录的工作，指标仍以各自原文为准。

## 与其他工作对比

| 维度 | 读法 |
|------|------|
| **文内角色** | 44 篇中唯一的 **资源** 类条目（[44/44]）：其余节点是方法 / 理论文献，本条是资源入口，不与它们并列比较 |
| **横比口径** | 与库内其他 awesome-* 策展实体的分工见 [44 篇技术地图](../overview/freedof-sim2real-44-papers-technology-map.md)；策展仓之间比的是覆盖面与维护度，不是性能 |
| **开源状态** | **已开源** — 以仓库实际链接与 LICENSE 为准 |
