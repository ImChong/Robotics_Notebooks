...
**抓取末段真正要解决的是「触觉可分辨维上的相对增量」，而不是满 6-DoF 绝对位姿或纯手内 gait。**

1. **直接回归腕部 \(\Delta\)** — 仿真三阈值 85.9% vs 估位姿再差分 54.1%；稀疏/对称接触下绝对位姿病态。
2. **按几何裁减可观 DoF** — 杆 \(\{z,\mathrm{roll}\}\)、板 \(\{z,\mathrm{roll},\mathrm{pitch}\}\)、盘 \(\{y,z,\mathrm{pitch}\}\)；硬做满 6-DoF 会灌噪声。
3. **真机 seen 物体读 \(10^\circ/10\,\mathrm{mm}\)** — 固定目标 **80.7%**、随机目标 **59.3%**；五步后均值约 5 mm / 3.5°；更严 \(5^\circ/5\,\mathrm{mm}\) 仅 26%/22%。
4. **未见边缘几何掉点大** — 同阈值降至 **36.7% / 13.3%**；误差可降但过阈值成功率差，细杆相对更可迁移。
5. **触觉与多指融合不可省** — 去触觉近乎崩盘；2 指明显变差，3→4/5 指增益变小。
6. **定位是粗抓后的局部精修头** — 全程 MuJoCo 156k 样本零样本部署（≈10 Hz）；不是全局规划，也非真机触觉 RL。
...
- [触觉纵深](../overview/depth-tactile.md) / [抓取纵深](../overview/depth-grasp.md)
