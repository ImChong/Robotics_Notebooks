...
**DA-Nav 的赌注不在更强的语言理解，而在把「已经规模化存在的商业导航方向指令」+「图像平面离散网格动作」+「显式偏离恢复监督」凑成一条能上真机的户外 VLN 路径。**

- 真正起作用的是 **ReDA 恢复数据 + CoT 结构化生成**：去掉 recovery 后 CSR 从约 98% 崩到 15.46%，去掉 CoT 后 SR 降至 38.91%、DF 升到 4.30——纯专家 BC 在闭环 OOD 下不成立。
- 动作表示是刻意的取舍：放弃连续 3D waypoint 回归改用图像平面离散网格（约 3 s、6 个网格目标），是为对齐自回归 VLM 的推理方式，而非追求几何精度。
- 适用边界很窄：指令接口只有 FORWARD / LEFT / RIGHT / STOP 四类离散方向，依赖 GPS 与商业导航覆盖；作者自述受多径、更新延迟与未覆盖区限制，与室内 R2R 式细粒度语言栈不是同一件事。
- 主要失败面：无可靠深度时回退 IPM 平地假设，坡道/崎岖会把几何误差传导到机体系轨迹；7B 自回归推理需远端 GPU，实时性仍是部署约束。
- 评测口径比绝对分更重要：SR（CARLA 59.00%、真机闭环 46.7%）并不高，差异化在 **CSR / DF**；只看 SR 会误判这类工作。
- 工程状态：截至入库日代码、权重与 ReDA 均未公开，只能作方法选型与对照，不能当可跑栈——与 [VLN 四范式开源复现](../overview/vln-open-source-repro-paradigms.md) 里的可跑通栈需区别对待。
