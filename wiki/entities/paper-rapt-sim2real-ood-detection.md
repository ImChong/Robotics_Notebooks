...
- **本页为索引级节点**（FreeDof 44 篇梳理 [37/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页「为什么重要」已录 89% TPR / 87.5% Top-1 诊断两个数字，须回原文核对其**判异阈值与误报率口径**——本页未转存完整实验表。
...
| **横比口径** | TPR 随判异阈值移动，必须连同 FPR 一起读；只报 TPR 的单点数字无法与其他监控方案横比。 |
