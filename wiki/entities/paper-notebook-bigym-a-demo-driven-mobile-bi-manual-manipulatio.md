...
**BiGym: A Demo-Driven Mobile Bi-Manual Manipulation Benchmark** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：11_Simulation_Benchmark），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**BiGym 的定位是「演示驱动」的评测床：它押的是人类演示自带的轨迹多样模态，而不是再造一批脚本化任务。**

- 规模与形态是主要贡献：40 个家居任务覆盖从简单到达到复杂厨房清洁的难度谱，且 **每个任务都配人类采集的演示**，让评测更贴近真实表现。
- 观测侧同时给本体感受与 3 个相机视角的 RGB / 深度，因此同一套环境既能评模仿学习，也能评演示驱动 RL——作者也在其上跑了当前最佳算法的基线。
- 演示驱动 RL 是 IL 与 RL 之间的折中，本基准的价值之一就是把这条中间路线放到与 IL 同一把尺子上比。
- 定位对照：与 RoboCasa、ManiSkill-HAB 等家居基准互补，独特点在 **移动 + 双手 + 人类演示** 这个组合。
- 边界：它是仿真基准与学习环境（Dyson Robot Learning Lab，2024 年 7 月），本页不含基线数字，量化结果以 [参考来源](#参考来源) 中的深读笔记与论文 PDF 为准。
