...
**GuideWalk 解决的是接口问题而不是步态问题：把人形步态包装成标准导航栈可以直接调用的 SE(2) 速度接口，让导航与地形自适应各归其位。**

- 真正起作用的是两步：先把导航参考轨迹投影到可落脚地形上，再训练低层策略跟踪 SE(2) 速度指令。
- 适用边界在身体层——它让既有导航栈能驱动人形走路，但不替代 VLA/世界模型的任务规划。
- 证据强度：本页是在公众号/survey 策展编译基础上的机制归纳，量化 benchmark、消融与实机指标以原文 PDF / 项目页 <https://GuideWalk.github.io> 为准。
...
- [机器人论文阅读笔记：GuideWalk](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/08_Navigation/GuideWalk__Learning_Unified_Autonomous_Navigation_and_Locomotion_for_Humanoid/GuideWalk__Learning_Unified_Autonomous_Navigation_and_Locomotion_for_Humanoid.html)
