...
**Stubborn 的真正主张是「跟踪失败不该被截断，而应当作进入倒地流形的入口」：用单一策略、单阶段训练把全身跟踪与跌倒恢复合并，代价是恢复动作的极限敏捷性让位于「回到参考轨迹」。**

- 起作用的是三件小改动的组合而非某个大模块：yaw-aligned 表征削弱全局漂移下的过度纠偏，Bernoulli 概率终止（$p_{term}=0.005$，期望多存活约 200 步）留出倒地探索窗口，AdpS 在 PT 让终止统计失效后接管难段采样。
- 关键证据是两个指标同时成立：LAFAN1 全库 MPBPE **48.85** mm 最优，且 5 m/s 扰动下恢复成功率 **100%**；去掉 PT 后恢复率降至 77.5%/85.0%，说明恢复能力来自训练机制而非额外奖励塑形。
- AdpS 的增益是边际级（MPJPE 120.7→119.4、MPJVE 548.5→541.5），它更像 PT 的配套修补，单独抽出来收益有限。
- 适用边界是「跟踪被打断后在线恢复并回到参考」；面向多姿态站起的专用栈如 [HoST](./paper-host-humanoid-standingup.md) 在极限倒地场景仍更激进，Stubborn 不替代它们。
- 工程侧具备可部署形态（非对称 Actor-Critic、策略仅用本体感知、29-DoF [Unitree G1](./unitree-g1.md) 闭环），但代码尚未公开，复现须以 arXiv 附录表格为准。
