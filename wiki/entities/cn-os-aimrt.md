...
**AimRT** 是 [智元机器人](https://github.com/AgibotTech) 公开的 **部署运行时** 开源项目：C++运行时通过插件和声明式配置组织线程、资源、通信与部署，兼容ROS 2、HTTP和gRPC，并把日志、监控与性能分析作为统一基础设施；它适合承载模型服务和机器人应用模块，不直接提供运动策略。
...
1. 从官方 GitHub/Gitee 组织检索 `AimRT` 仓库并核对 README 许可与依赖。
