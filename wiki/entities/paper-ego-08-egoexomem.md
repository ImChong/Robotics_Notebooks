...
**EgoExoMem 在本库的价值是给出一个数据侧判断：ego 与 exo 不是二选一的替代关系，跨视角 + 记忆才是回答长时程问题的前提。**

- 核心论点是互补性：**第一视角知道操作者看到了什么，外部视角知道人与物体的整体关系**，很多问题单靠任一侧都答不出来。
- 对机器人的直接映射是「只看机载相机会丢空间关系，只看外部相机会丢局部意图与接触细节」，据此推向 **Ego + Exo + 3D 环境记忆** 的数据系统形态。
- 本页为 **策展索引级**：量化 benchmark、消融与实机指标须回原文 PDF / 项目页，不宜据此页判断方法强弱。
