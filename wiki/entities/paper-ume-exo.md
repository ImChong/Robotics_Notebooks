...
**UME 的核心主张是「力矩是一种必须被采集的模态」：外骨骼把从臂接触阻力实时透传给操作者，同时把整条臂的关节位置与力矩一并写进数据集，策略才可能学到主动柔顺。**

- 真正起作用的是两件事——ACT 中 **关节力矩与位置同权 embedding**，以及记录 **全身臂形而非仅 6D 末端**；四任务成功率 0.90 / 0.85 / 0.95 / 0.95，而 No-torque 在 box flipping 上为 0，UMI 基线在 box flipping 与 GPU picking 上均为 0。
- 失败模式因此很清晰：去掉力矩后，需要「持续加力直至阻力饱和」的任务会过早停止，也无法区分视觉相同但力矩不同的状态；仅末端位姿监督则在需全臂伸直或腕载相机/夹爪避障时频繁碰撞。
- 采集侧收益同样被实测：力反馈让 box flipping 的采集吞吐达无力矩版的 **3.3×**，约 $9 的板载 IMU 替代昂贵 VR 全身追踪来驱动移动底座，外骨骼整机约 $1900。
- 适用边界：这是需要从臂在线的 **具身 leader–follower**，与 [BifrostUMI](./paper-bifrost-umi.md) 的「无目标机器人采集」互补而非替代；PLA 连杆偏重、载荷有限且长度不可调。
- 开放问题与开源状态：策略范式固定在 [ACT](../methods/action-chunking.md)，未系统对比 Diffusion Policy / VLA；代码在项目页仍标注 Coming Soon，复现需等官方仓库与 BOM。
