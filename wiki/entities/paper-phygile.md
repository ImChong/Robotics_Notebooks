...
- **GMT 作为可复用执行底座**：课程式 **MoE tracker + 无标注后训练 + 动态专家扩展**，为大规模机器人运动与后续 prefix 适配提供 **鲁棒跟踪器**；与 [GMT 计划实体](paper-loco-manip-161-009-gmt.md) 同族但本工作给出 **与生成闭环耦合** 的完整栈。
...
- **与独立 GMT 论文关系**：本库 [GMT 实体](paper-loco-manip-161-009-gmt.md) 仍为 Paper Notebooks **待深读** 占位；PhyGile 复用 **GMT 命名与跟踪范式**，但是 **生成–跟踪统一框架** 的独立工作。
...
- [GMT（计划实体）](paper-loco-manip-161-009-gmt.md) — 跟踪器命名与任务族交叉索引
...
- [GMT（计划实体）](paper-loco-manip-161-009-gmt.md)
