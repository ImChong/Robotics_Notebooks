...
| 维度 | autoresearch | LLM Wiki（本库） | Superpowers |
|------|--------------|------------------|-------------|
| 人类编写 | `program.md` | `schema/`、`AGENTS.md`、ingest 规范 | `SKILL.md` 技能库 |
| 代理修改 | `train.py`（训练代码） | `wiki/`、`sources/` 映射 | 应用代码 + 测试 |
| 验证 | val_bpb + 固定时长 | `make ci-preflight`、lint | TDD、code review |
| 目标 | 更优 LLM 训练配置 | 知识编译与交叉引用 | 可交付软件质量 |
