...
**HEFT 的真正命题是「部署端只吃 raw VR」：把干净参考、专家窗上界与仿真特权全部关在训练期，换来单一可部署策略同时扛住 VR 噪声与双手 24 kg 负载。**

- PMG 的关键是把 **可部署输入** 与 **可评估目标** 解耦：actor 吃 raw VR，reward / critic 对齐后的 clean 参考评估——它不是在线运动重建，部署时 teacher、重建参考与特权态全部剥离，只留 student + adapter。
- WPC 的关键是负载课程按 **5 s 运动窗** 而非全局常数：用 clean 专家 rollout 估每窗上界再采样、在双手腕间随机分配，因为支撑、转向、深蹲、摆臂各段的可行负载本就不同（对照 TWIST2+FC 与 w/o expert 消融）。
- 适用边界：策略 **不观测负载状态**，鲁棒性完全来自仿真随机化；极端未见负载分布或非双手腕施力场景可能失效，这与力传感闭环是两条路。
- 工程门槛在数据而非代码：训练与 `sim2real` checkpoint 已开源，但完整 VR→重建→配对工作流、训练集与 WPC 窗标签仍待发布，当前仅有 G1 烟测 memdataset。
- 路线对照：[TeleGate](./paper-telegate.md) 用门控冻结专家覆盖多动态类，HEFT 坚持 **单策略** 同时吃重载 + 高动态 + 噪声 VR，更贴近「一条 LLC 直接跟 VR」的部署形态；G1 上以 SONIC / TWIST2 官方权重为基线，而全尺寸 L7（175 cm / 65 kg）的重载验证是本文独有的公开案例。
