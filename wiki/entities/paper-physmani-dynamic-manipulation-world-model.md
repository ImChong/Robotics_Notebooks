...
**PhysMani 用一次干净的同骨干消融把话说死：动态目标操作的增益来自「物理可信的 3D 未来预报」，不是更大的策略骨干，也不是更强的 2D 运动线索。**

- 增益归因明确：策略骨干与训练策略与 3DFA 完全相同，唯一差异是注入 $D_t$——仿真 Mean SR **45.9% vs 37.8%**、真机 **62.5% vs 45.3%**，去掉 $D_t$ 即回落至 3DFA 量级。
- 2D 替代方案被证伪：3DFA-OF（+2D 光流）**37.5%** 相对 3DFA 无增益，说明关键在 3D Gaussian 速度场提供的物理有意义轨迹，而非任何「运动信息」。
- 真正起作用的机制是**无散度速度场 + 在线优化**的组合：既保证物理可信，又把世界模型更新压到 **~200 ms/帧**，相对 FreeGave **3.0×** 帧率且预测更准（PSNR **26.90** / traj err **0.008 m** vs **19.47** / **0.010 m**）。
- 适用边界要说清：依赖多路固定 RGB-D 与脚本专家数据；语言仅作任务条件，不走大规模 VLM 预训练，因而不是通用 VLA；~200 ms 的 WM 延迟对乒乓球级极高速交互仍不够。
- 工程与开源状态是当前最大的落地障碍：官方仓库仅为项目 landing page，训练代码、PhysMani-Bench 与权重截至 2026-07 未公开，复现须等官方 release；Astribot 真机与 Franka 仿真之间还有域差。
