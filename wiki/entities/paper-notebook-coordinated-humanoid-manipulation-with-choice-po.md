...
**这篇工作把人形全身操作的瓶颈拆成「采得到」与「学得动」两半：模块化遥操作解决前者，「生成候选 + 学打分」解决后者，在推理速度与多模态之间走出 BC 与扩散之外的第三条路。**

- 真正起作用的机制是 Choice Policy 的 **生成多候选 + 学习打分**：既避免行为克隆塌缩到均值，又不必付扩散策略的推理代价，因此适合实时全身操作。
- 数据侧的关键不是量而是质：把高自由度人形拆成手眼协调 / 抓取基元 / 臂末端跟踪 / 行走四个直观子模块之后才采得到高质量演示——遥操作接口设计本身就是方法的一部分。
