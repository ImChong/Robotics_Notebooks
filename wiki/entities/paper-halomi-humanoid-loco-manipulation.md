...
| 维度 | HALOMI（本文） | BifrostUMI | HuMI |
|------|----------------|------------|------|
| 示范接口 | 仅头 + 双手（稀疏） | 五关键点 + 下身参考 | Vive + UMI 全身参考 |
| 下身处理 | 隐式交给 BFM-Zero WBC | 显式骨盆/脚参考 | 显式骨盆/脚参考 |
| 跟踪空间 | BFM-Zero 潜空间（流形约束） | raw action / 关节空间 | — |
| 主动感知 | 3-DoF 主动颈写入数据闭环 | 无专门主动颈 | — |

- **vs raw action-space 跟踪**：直接在关节空间追世界系关键点在 OOD 大跳变下易多模态、激进失稳；HALOMI 在 [BFM-Zero](./paper-bfm-zero.md) 潜空间规划再解码，保持可行、非激进的全身行为。
- **接口取舍**：HuMI/BifrostUMI 的显式下身参考利于 VLA 预测步态；HALOMI 把下身隐式交给 WBC，换来更贴近 UMI 的便携性，但需靠相对头手表示与控制器鲁棒性补偿长时域漂移。
