...
**NMR 的主张是把重定向从「逐帧几何优化」改写成「数据分布学习」：既然传统优化式 retargeting 非凸、易陷局部最优，就训练一个网络去学更稳的重定向分布。**

- 判据是伪影而非观感：self-penetration、foot sliding、物理不可行是优化式重定向的典型失败模式；NMR 用 CEPR 等机制、层次化与 VAE-based motion clustering 把大量动作组织成 latent motifs 来缓解。
- 它成立的前提是数据源变杂：未来参考不会只有干净 mocap，还有单目视频、生成视频、遥操作与互联网混合数据，手工规则越来越难覆盖——这是相对 GMR「retargeting matters」再进一步的动机。
- 定位与边界：本页属 42 篇栈 **01 数据 · 重定向 · 遥操作**（#02/42），把「重定向质量」推到模型化层面；参考质量的上限往往 **早于** RL 策略决定，别把重定向当训练前脚本。方法族细节见 [neural-motion-retargeting-nmr](../methods/neural-motion-retargeting-nmr.md)。
- 本页为策展编译，量化 benchmark、消融与实机指标以原文 PDF / 项目页为准。
...
- [机器人论文阅读笔记：Make Tracking Easy](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/02_Motion_Retargeting/Make_Tracking_Easy__Neural_Motion_Retargeting_for_Humanoid_Whole-body_Control/Make_Tracking_Easy__Neural_Motion_Retargeting_for_Humanoid_Whole-body_Control.html)
