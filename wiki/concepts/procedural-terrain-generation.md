...
### 学习式扩散后继（SIGGRAPH 2026）

经典噪声/高度场擅长 **低成本无限延展** 与 **seed 确定性**，但难以学到 **大陆尺度连贯地貌** 与 **气候–高程耦合**。[InfiniteDiffusion / Terrain Diffusion](../entities/paper-infinite-diffusion-terrain-diffusion.md) 以 **惰性扩散采样** 提供 **O(1) 坐标查询** 的 **学习式程序化地形**：分层扩散 + Laplacian 编码覆盖地球尺度高程动态范围，并开源 **[Minecraft Fabric mod](https://modrinth.com/mod/terrain-diffusion)** 作引擎集成示范。机器人仿真若采用此类资产，仍需 **碰撞对齐、摩擦/接触 DR 与课程调度**——几何更逼真不等于动力学自动可迁移。
...
- [InfiniteDiffusion / Terrain Diffusion 论文摘录（SIGGRAPH 2026）](../../sources/papers/infinite_diffusion_terrain_diffusion_siggraph_2026.md) — 学习式程序化地形与 Minecraft 集成
