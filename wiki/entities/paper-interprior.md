...
**InterPrior 的立场是「蒸馏给自然性与技能宽度、RL 给鲁棒与失败恢复」：把物理 HOI 从单条轨迹跟踪改写成稀疏目标条件的生成式运动先验，用三阶段把离散技能片段焊成连续可泛化流形。**

- 真正起作用的是 **阶段 III 的 RL 微调 + 失败态重置**：显式抬高恢复数据占比以学出再接近/再抓取，补掉纯蒸馏在巨大 HOI 构型空间上的分布外脆性，同时用正则回拉避免孤立 RL 的非自然奖励投机。
- 接口是关键设计：**多 horizon 掩码目标** 让推理期只需填稀疏分量（用户或上游生成器），全序列信息在训练期由残差后验压进 encoder，推理折叠为 prior 采样。
- 适用边界在 **本体 + 几何交互特征（体段–物体有符号距离、接触指示）** 这一层，不与 RGB 像素端到端策略竞争同一接口。
- 主要证据仍在 **物理仿真** 内：G1 上展示的是 sim-to-sim 与实时键盘交互，真机部署潜力应与后续系统线分开阅读；复现前提是并行仿真吞吐与高质量 HOI 参考数据管线。
- 与 [ContactMimic](./paper-contactmimic.md) 互补：后者走稠密 keypoint+contact 跟踪与运行时接触开关，本页走稀疏条件下的生成式采样；相对 InterMimic 则是「专家 → 可条件采样先验」的下一级。
...
- [机器人论文阅读笔记：InterPrior](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/13_Physics-Based_Animation/InterPrior__Scaling_Generative_Control_for_Physics-Based_Human-Object_Inter/InterPrior__Scaling_Generative_Control_for_Physics-Based_Human-Object_Inter.html)
