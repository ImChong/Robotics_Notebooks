...
- **端到端可制造验证：** 流水线输出可直接送 3D 打印 + 舵机制造；论文与项目页展示多款不同面部造型的 Animatronic 头部原型，能完成说话、微笑、皱眉等表情序列。
...
- **vs 其他机构合成思路：** 与 Deimel & Brock 的 [欠驱动柔顺手](../entities/paper-deimel-compliant-underactuated-robotic-hand.md) 同属「少驱动 + 机械耦合 / 结构化设计」的机构合成路线，但本文面向面部表情连杆的自动合成，而非抓取。
