...
**X-Cache 的真正贡献是换缓存轴：少步蒸馏把「沿去噪步复用」的前提抹掉后，相邻生成 chunk 成了唯一还剩冗余的维度。**

- 起作用的是「复用 + 护栏」整套：跨 chunk 残差缓存、结构与动作指纹的双度量门控、anchor block 永不跳，换来约 **71% block skip** 与 **2.6–2.7×** 加速且 PSNR 近无损。
- 护栏比缓存本身更关键：写清洁 K/V 的 chunk 必须强制全算，一旦误 skip，PSNR 可从 ~51–55 dB 崩到 **~21 dB**。
- 适用边界明确：**少步蒸馏的自回归视频扩散 + 闭环无前瞻**；换骨干或非 chunk 生成器，指纹与护栏假设都要重做。
- 工程状态是最大折扣项：training-free 可插拔听起来友好，但截至入库日 **未开源**、无基座权重，外部只能借鉴设计而无法直接集成验证。
- 与 TeaCache / DeepCache / ΔDiT 对照：它们沿去噪步复用，在少步后必然失效；轨迹外推与 block cascading 则需前瞻或平滑动作，闭环交互驾驶不适用。
