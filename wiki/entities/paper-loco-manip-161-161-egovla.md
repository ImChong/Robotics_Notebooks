...
**EgoVLA 的核心主张是「人类第一视角视频也能当机器人训练数据」：保留 VLM 的语义理解，再挂上机器人状态输入与动作头，逼模型不要只停在语言规划层。**

- 关键改动在接口而非骨干：加机器人状态与动作头，输出末端执行器/腕手目标与全身轨迹，而不是输出文本计划。
- 训练路径是行为克隆加 VLA 多模态动作模型加潜变量/动作 token，语义能力靠继承、执行能力靠新增的动作分支。
- 固有代价是跨形态对齐：数据源是人类第一视角视频，人手到机器人腕手的映射是这条路线绕不开的问题。
- 作为 161 篇的收尾条目，它代表 10「从人类第一视角视频学习」整组的坐标；有项目页与外部深读笔记入口，量化指标以原文为准。
...
- [机器人论文阅读笔记：EgoVLA](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/06_Manipulation/EgoVLA__Learning_Vision-Language-Action_Models_from_Egocentric_Human_Videos/EgoVLA__Learning_Vision-Language-Action_Models_from_Egocentric_Human_Videos.html)
