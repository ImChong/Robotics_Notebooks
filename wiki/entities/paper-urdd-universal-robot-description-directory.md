...
**URDD 不去改 URDF 规格本身，而是在其上加一层带版本号的「派生产物」目录——用磁盘占用与再生成成本，换掉各栈各自重复预处理与结果不一致。**

- 被标准化的正是 URDF **不直接给出、却几乎人人都要算** 的东西：DOF 计数与关节↔DOF 映射、链上路径、凸分解与 OBB/包围球近似、自碰撞跳过对等。
- 扩展方式是关键设计取舍：新能力以 **新子目录 / 新模块文件** 独立版本化加入，避免「改单一巨型 XML 导致全局解析器连锁升级」。
- 可见性被当成正确性手段：Bevy GUI 与 Three.js 浏览器两套检视器让预处理结果对开发者可核对，减少黑盒离线脚本的 silent error。
- 成本侧交代得诚实：五台代表机型完整生成约 **21–90 s** 量级，磁盘占用相对「仅 URDF + 原网格」显著增大，因此流水线必须自带缓存失效 / 再生成策略。
- 定位与 URDF+ / Extended URDF 等 **规格层扩展正交互补**，不是又一种 XML 替代格式；权威模块集合以作者维护文档为准，本页不复制枚举。
