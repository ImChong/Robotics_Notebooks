# Kimodo: Scaling Controllable Human Motion Generation

> 来源归档（ingest · 人形 AMP 运动先验 19 篇 · 第 04/19）

- **标题：** Kimodo: Scaling Controllable Human Motion Generation
- **类型：** paper
- **AMP 叙事段：** 01 分布约束与先验组件化
- **机构：** NVIDIA
- **论文链接：** <https://arxiv.org/abs/2603.15546>
- **项目页：** <https://research.nvidia.com/labs/sil/projects/kimodo/>
- **索引来源：** [具身智能研究室 · AMP 专题长文](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)（<https://mp.weixin.qq.com/s/YZsm3855iP3TNTTt1aou7w>）
- **原始抓取：** [wechat_humanoid_amp_19_survey_2026-05-26.md](../raw/wechat_humanoid_amp_19_survey_2026-05-26.md)（Agent Reach + Camoufox）
- **入库日期：** 2026-05-26
- **深读归档：** 2026-06-25 — [kimodo_arxiv_2603_15546.md](kimodo_arxiv_2603_15546.md)
- **一句话说明：** Kimodo 不是机器人控制论文，但它应该放在这里。

## 核心摘录（策展，非全文）

- **在 AMP 四段地图中的位置：** 01 分布约束与先验组件化，编号 **04/19**。
- **公众号导读要点：** Kimodo 不是机器人控制论文，但它应该放在这里。
- **方法要点（arXiv 编译）：** 700h 动捕运动学扩散；两阶段 root/body 去噪；文本/关键帧/2D 路径约束；G1 演示数据与 ProtoMotions/GEAR-SONIC 下游。

## 对 wiki 的映射

- [kimodo](../../wiki/entities/kimodo.md) — AMP 专题策展页
- [kimodo](../../wiki/entities/kimodo.md) — 主实体页（完整架构与 Benchmark）
- [motionbricks](../../wiki/methods/motionbricks.md) — 同段实时姊妹对照

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)
- 姊妹篇 42 篇栈：[wechat_embodied_ai_lab_humanoid_rl_motion_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)
- arXiv 深读：[kimodo_arxiv_2603_15546.md](kimodo_arxiv_2603_15546.md)
