...
- [x] **详情页"同专题相关页"提示**：
    - [x] 复用 `docs/topic-filters.js` 单一事实源（`renderMetaTopicBadges` → `topicsForNode` 已数据驱动），评测基准/新建页命中「具身评测基准」专题时自动渲染对应轻量徽标 + 跳转 `graph.html?topic=embodied-eval-benchmark`（空态降级隐藏）。P3① 把 `embodied-eval-benchmark` 写入单一事实源后，详情页「所属专题」徽标行即自动联动；选一页评测实体页端到端验证并归档截图至 `.cursor-artifacts/screenshots/detail-topic-embodied-eval-benchmark.png`。（已落地：详情页「所属专题」徽标行（`main.js renderMetaTopicBadges` → `topic-filters.js topicsForNode`）本就数据驱动，P3① 把 `embodied-eval-benchmark`（3 段 `bench`/`eval`/`benchmark` 干净片段 + 7 页 `ids`）写入单一事实源后即自动联动，无需二次实现。node 端逐页复核：`robo-bench`/`ewmbench`/`esi-bench`/`paper-gigaworld-1-policy-evaluation`/`simulation-evaluation-infrastructure`/`embodied-eval-benchmark-selection-loop`/`sim-vs-real-eval-gap` 7 页均命中 `embodied-eval-benchmark`，非评测的 `vlm-vln-vla-vlx-world-model-taxonomy` 未命中；端到端截图 `detail.html?id=entity-robo-bench`「所属专题」行同渲「🏋 全身控制 (WBC)」+「🧪 具身评测基准 (Embodied Eval Benchmark)」双徽标，后者跳 `graph.html?topic=embodied-eval-benchmark`，多专题命中互不干扰、空态整行降级隐藏。截图归档 `.cursor-artifacts/screenshots/detail-topic-embodied-eval-benchmark.png`。至此 v29 全数完成。）
