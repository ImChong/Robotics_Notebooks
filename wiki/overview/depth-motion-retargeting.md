---
type: overview
tags: [depth, depth-motion-retargeting, motion-retargeting, mocap, humanoid]
status: complete
updated: 2026-07-31
summary: "动作重定向纵深汇总：把人体/动物参考动作映射到人形与异构机器人骨架，衔接 MoCap、IK/优化重定向、AMP 先验与 WBT 训练数据的全链路导读。"
---

# 动作重定向（纵深汇总）

> **图谱纵深视图**：本页是知识图谱「🤸 动作重定向 (Motion Retargeting)」纵深的统一入口；在 [图谱纵深视图](../../docs/graph.html?depth=motion-retargeting) 筛选时，本节点为汇总锚点。
...
- **[WBT](./depth-wbt.md)**：消费重定向轨迹做全身跟踪策略。
- **[跨具身](./depth-cross-embodiment.md)**：重定向是跨形态迁移的前置步骤。
- **[IL/RL](./depth-learning.md)**：重定向数据常作为 BC 示范或 AMP 风格约束。
- **[训练数据](./depth-data-pipeline.md)**：重定向是训练数据管线的第③段，承接质量评估、产出策略输入。
...
- 图谱纵深定义：[docs/depth-filters.js](../../docs/depth-filters.js)（`motion-retargeting` 命中规则）
