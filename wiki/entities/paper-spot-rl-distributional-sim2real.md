...
**这项工作的重点不是「RL 让 Spot 跑得更快」，而是把 sim2real gap 变成一个可优化的标量：分布距离取代人工调 DR，再把 Isaac Lab 训出的策略推到 Spot 的低层电机接口上。**

- 真正起作用的机制是 **Wasserstein / MMD 评分 + CMA-ES**：摩擦、电机等难测参数交由无梯度黑盒优化搜索，而非靠经验设定随机化范围，可重复性明显强于手工 DR。
- 另一半价值在 **接口**：Spot RL Researcher SDK 的低层电机访问 + 开源 Isaac Lab 训练栈 + 官方部署路径三者齐备，把四足数据驱动低层控制的复现门槛降了下来。
- 性能叙事（>5.2 m/s、含腾空相的动态步态、湿滑面与扰动抑制）应读作给 BD 经典模型驱动 WBC/MPC 的 **RL 对照上限**，而不是产品替代——本文是研究演示，与出厂控制器的能力边界不同。
- 方法自身的边界很明确：**分布匹配的有效性取决于数据采集覆盖与特征选择**，极端地形或未建模动态仍可能失效。
- 分层看待更合适：本页聚焦低层 locomotion，与 [BD 足式控制专利栈](./patent-boston-dynamics-legged-control-stack.md) 的在线轨迹优化路线可并存或分层；同平台的任务级自主见 [Autonomous Spot / NeBula](./paper-autonomous-spot-nebula-exploration.md)。
