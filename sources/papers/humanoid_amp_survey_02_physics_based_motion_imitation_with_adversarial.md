# Physics-Based Motion Imitation with Adversarial Differential Discriminators

> 来源归档（ingest · 人形 AMP 运动先验 19 篇 · 第 02/19）

- **标题：** Physics-Based Motion Imitation with Adversarial Differential Discriminators
- **类型：** paper
- **AMP 叙事段：** 01 分布约束与先验组件化
- **机构：** Simon Fraser University、Sony PlayStation、NVIDIA
- **出处：** SIGGRAPH 2024
- **索引来源：** [具身智能研究室 · AMP 专题长文](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)（<https://mp.weixin.qq.com/s/YZsm3855iP3TNTTt1aou7w>）
- **原始抓取：** [wechat_humanoid_amp_19_survey_2026-05-26.md](../raw/wechat_humanoid_amp_19_survey_2026-05-26.md)（Agent Reach + Camoufox）
- **入库日期：** 2026-05-26
- **深读归档：** 2026-06-25 — [add.md](add.md)
- **一句话说明：** ADD 可以看成 AMP 作者线里对 reward 工程的一次继续追问。

## 核心摘录（策展，非全文）

- **在 AMP 四段地图中的位置：** 01 分布约束与先验组件化，编号 **02/19**。
- **公众号导读要点：** ADD 可以看成 AMP 作者线里对 reward 工程的一次继续追问。
- **方法要点（深读编译）：** 判别器输入改为时空差分特征，抑制滑步与关节漂移；仍属对抗运动先验框架，集成于 MimicKit/ProtoMotions。

## 对 wiki 的映射

- [paper-amp-survey-02-physics_based_motion_imitation_with](../../wiki/entities/paper-amp-survey-02-physics_based_motion_imitation_with.md) — 主实体页（深读归纳）
- [add](../../wiki/methods/add.md)、[amp-reward](../../wiki/methods/amp-reward.md)

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)
- 姊妹篇 42 篇栈：[wechat_embodied_ai_lab_humanoid_rl_motion_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)
- 深读：[add.md](add.md)
