...
**DreamZero: World Action Models are Zero-shot Policies** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：06_Manipulation），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**DreamZero 押的是一个换算：把「提升机器人能力」换成「提升视频生成 + 动作对齐」——动作不再从状态-动作对模仿，而是从预测出的视觉未来里反解，于是数据不必重复、泛化跟着视频先验走。**

- 真正的杠杆是**视频扩散先验 + 逆动力学对齐**：未见动词/动作/环境上真机任务进度平均 **>2× SOTA VLA**，任务特定后训练之后环境泛化仍 **+10%**。
- 数据经济学是最可迁移的结论：**~500 h 非重复**真实轨迹即可训出通才策略，打破「每个任务都要大量重复 teleop demo」的惯例；跨本体只靠视频示范（人类 12 min / 异构机器人 20 min）就有 **+42%** 相对提升，30 min play data 可 few-shot 适配新 embodiment 且保留零样本能力。
- **38× 推理加速是它能被叫作策略而不是世界模型的前提**：14B 扩散 WAM 压到 7 Hz 真机闭环，这一步不成立，整条路线就退回离线预测。
- 落地状态明确：开源权重、推理与 RoboArena / PolaRiS / Genie Sim 3.0 评测代码；NVIDIA 称 GR00T 2 基于本研究，可视为该路线的产品化出口。
- 与 VLA 路线的对照定位：VLA 强在语义泛化（换物体、换指令），DreamZero 补的正是**新环境与新物理动作/技能**这块短板，而非取代语义理解。
