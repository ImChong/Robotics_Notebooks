...
related:
  ...
  - ../entities/paper-simplememvla.md
...
- **THAW-VLA（UW–Madison/UIUC，arXiv:2609.24682）**：离线缓存 Cosmos3-Nano 特征，对 [StarVLA](./star-vla.md) QwenGR00T 0.8B 加 cosine 对齐；部署图与 undistilled 相同（32 ms / 1.86 GB RTX 5090），LIBERO **97.9%**、RoboCasa-GR1 **50.5%**；**GitHub + HF 权重已开源**（见 [THAW-VLA](../entities/paper-thaw-vla.md)）
- **RouteRLT（多伦多大学，arXiv:2609.26467）**：冻结 **SmolVLA** + 多枚 phase-specific **RL specialist**；从 VLA latent **学习 controller ownership**（含 mid-chunk 换控）；LIBERO **92.22%**、真机 insertion **35.0%**；**代码未列链接**（见 [RouteRLT](../entities/paper-routelt.md)）
...
- **云–边交错：** [VLA-ULAP](../entities/paper-vla-ulap.md)（arXiv:2609.18663）用 **~7.4M ULAP** 在 Orin Nano **19.9 ms** 预测 chunk，与远程 VLA 交错，保留 **95–97.5%** baseline SR 并减 **48.8–76.7%** cloud 调用。
- **固定 VLM 代理 ICL：** [GPT-Policy](../entities/paper-gpt-policy.md)（arXiv:2609.19138）context compiler + 约束 Cartesian adapter；GPT-6 Astra 真机十任务 in-context ablation；[cheng-haha/GPT-Policy](https://github.com/cheng-haha/GPT-Policy) 已开源。
...
- [PhysBrain 1.5](../entities/paper-sa-2512-16793-physbrain-human-egocentric-data-as-a-bridge-from.md) — 统一 token 具身理解 / ActionPiece / 未来态预测 + 28 benchmark EvalKit
...
- [Universal Post-Training for Robotics](../concepts/universal-post-training-robotics.md) — Perry Dong 框架：VLA 预训练后需通用 RL 配方 + 标准协议（EXPO-FT 案例）
...
- [RoboDawn（冻结 VLM + 语义命令 ICL）](../entities/paper-robodawn.md) — **零 robot 训练**；RoboTwin C2R **73.6%** 1-shot、RoboDojo **47.17%**；710 episode 可回放（arXiv:2609.22966，[已开源](https://github.com/Hugo-AGI/RoboDawn)）
...
- [SimpleMemVLA（原生视频上下文记忆）](../entities/paper-simplememvla.md) — Qwen3.5 时间戳视频 + sub-task 条件 DiT；四套记忆基准 SOTA、LIBERO 97.5（arXiv:2609.05533，[已开源](https://github.com/OpenBMB/SimpleMemVLA)）
...
- [THAW-VLA](../entities/paper-thaw-vla.md) — WAM 特征蒸馏进 0.8B QwenGR00T，零部署税；LIBERO 97.9%（arXiv:2609.24682；GitHub+HF 已开源）
- [RouteRLT](../entities/paper-routelt.md) — 冻结 VLA + 多 RL specialist 自动路由（arXiv:2609.26467；未列代码）
- [RoboTwin-Phys](../entities/paper-robotwin-phys.md) — 13 维物理参数多样性 benchmark（arXiv:2609.26292；数据待跟进）
- [FLUX 3 Action](../entities/flux-3-action.md) — BFL 7B WAM 训练/部署 + LeRobot SO-101 LoRA（已开源）
