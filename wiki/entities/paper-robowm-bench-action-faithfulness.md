...
summary: "RoboWM-Bench（具身世界模型六路线专题）：把生成行为还原为机器人动作并在真机执行评测。"
---

# RoboWM-Bench

**RoboWM-Bench** 收录于 [具身智能研究室 · 具身世界模型六路线综述](https://mp.weixin.qq.com/s/mmIJRp9g6NqblMCjd9D5GQ) **趋势与判断** 段。本页为知识库 **策展编译** 详情节点；量化指标以原文 PDF / 项目页为准。

## 一句话定义

把生成行为还原为机器人动作并在真机执行评测。
...
**RoboWM-Bench 在六路线框架下的读法：先看预测输出被谁消费，再看是否改善真实行动——而非只看网络新旧或画面观感。**
...
- **同路线邻接工作**（综述 **趋势与判断** 段）：[WorldArena](./paper-sa-2602-08971-worldarena-a-unified-benchmark-for-evaluating-pe.md)、[WorldEval](./paper-sa-2505-19017-worldeval-world-model-as-real-world-robot-polici.md)。
- **对照要点**：RoboWM-Bench 把生成行为 **还原成机器人动作并在真机执行** 来判忠实度；WorldArena 走 **统一多维打分**——两者共同支撑综述「评价从画质转向行动效用」的判断。
