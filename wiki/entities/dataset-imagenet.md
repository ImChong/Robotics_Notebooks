...
status: complete
updated: 2026-08-12
summary: "大规模图像分类与预训练基石：ILSVRC-1K（约 1.28M/1k 类）与 ImageNet-21K 更广词表；ViT/ResNet 等视觉骨干的标准预训练源。"
related:
...
**ImageNet / ImageNet-21K**：大规模图像分类与预训练基石：ILSVRC-1K（约 1.28M/1k 类）与 ImageNet-21K 更广词表；ViT/ResNet 等视觉骨干的标准预训练源。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| ILSVRC | ImageNet Large Scale Visual Recognition Challenge | 经典 1000 类竞赛设定 |
...
| 维度 | 速查 |
|------|------|
| 规模 | 1K：约 1.28M 训练图；21K：约 14M 图、2 万+ 类（版本相关）。
| 模态 | RGB 自然图像 + 类别标签（检测/分割另有扩展标注）。
| 许可证 | 需遵守 ImageNet 条款与下载注册要求。
| 适配形态 | 通用视觉预训练；迁移到机器人域常需微调。
| 重定向就绪度 | 不适用；作分类/自监督预训练输入。
...
- [具身大模型评测基准选型闭环知识链](../queries/embodied-eval-benchmark-selection-loop.md) — 分类预训练基准处评测链最上游，与③真机成功率相关性最弱
...
- [ImageNet / ImageNet-21K 官方入口](https://www.image-net.org/)
