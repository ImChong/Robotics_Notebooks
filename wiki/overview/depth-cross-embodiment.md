---
type: overview
tags: [depth, depth-cross-embodiment, transfer, any2any, retargeting]
status: complete
updated: 2026-07-31
related:
...
summary: "跨具身迁移纵深汇总：不同机器人形态、仿真与真机之间的策略/动作迁移，重定向、域随机与 Any2Any 类方法的选型与失败模式；含末端/工具接口多样性（如 GEN-1 千手）轴。"
---

# 跨具身迁移（纵深汇总）

> **图谱纵深视图**：本页是知识图谱「🔀 跨具身迁移 (Cross-Embodiment)」纵深的统一入口；在 [图谱纵深视图](../../docs/graph.html?depth=cross-embodiment) 筛选时，本节点为汇总锚点。
...
- **[动作重定向](./depth-motion-retargeting.md)**：跨骨架动作对齐。
- **[WBT](./depth-wbt.md)**：跟踪策略的跨形态扩展。
- **[Sim2Real](./depth-sim2real.md)**：仿真-真机是跨具身特例。
...
- 图谱纵深定义：[docs/depth-filters.js](../../docs/depth-filters.js)（`cross-embodiment` 命中规则）
