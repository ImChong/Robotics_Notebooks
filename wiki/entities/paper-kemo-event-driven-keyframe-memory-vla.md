...
**KEMO 的核心主张是「记忆不必稠密」：用运动学减速峰 + 视觉去重挑出的少量过渡帧就足以消解长程 VLA 的阶段歧义，前提是记忆构建与训练信号同源。**

- 起作用的是三件事的组合，而非单一模块：事件选帧（Cover Blocks 上事件帧 **25%** vs 近帧 **8.3%** vs 均匀采样 **0%**）、门控 cross-attention（去掉即 **0%**）、$\lambda=8$ 的关键帧邻域 loss 加权（去掉降到 **25%**），完整配置 **75%**。
- 可插拔性来自挂载位置：记忆只作用于 **SigLIP 视觉 token** 层、不改 π₀.₅ 的 LLM 前缀结构；关键帧检测只需本体运动 + 冻结 DINOv2，无需子任务标注或每步 VLM 推理。
- 真机六项双臂任务聚合 **TSR 51.4% vs π₀.₅ 27.8%**、**SCR 76.4% vs 42.3%**；但 Drawer Items Replacement 三方全为 **0/12**——最长轨迹叠加「物体–容器对应」的记忆需求仍超出当前检测与 bank 容量。
- 适用边界：$w$、峰窗、不应期 $r$、bank 容量 $K$ 都按任务阶段数手工设定，跨任务迁移要重新调参；代码尚未公开。
- 谱系对照：[EventVLA](./paper-eventvla-visual-evidence-memory.md) 走学习式前瞻选帧，[FM-VLA](./paper-fm-vla.md) 在阶段变化视觉不可见时改用力觉历史，[Chronos](./paper-chronos.md) 干脆用 SSM 压全历史；KEMO 占的是「运动学启发式选帧 + 门控注入现成 VLA」这一格。
...
- 3D 多视图记忆对照：[BridgeVLA++](./paper-bridgevla-plusplus.md) — heatmap 对齐 + 𝒯/𝒮 时空记忆；RMBench **96.0%**（arXiv:2608.05042）。
