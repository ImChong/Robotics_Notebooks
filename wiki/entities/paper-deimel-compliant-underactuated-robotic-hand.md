...
**这篇的贡献不是「又一只手」，而是把控制难题外包给材料与接触力学——用极少气动执行器换取容错、安全与低成本，代价是力量、精度与可建模性。**

- 真正起作用的机制是 **欠驱动耦合 + 顺应性**：一路气压驱动多关节，指节角度由接触力被动调适形成 form / force closure，因此不需要逐关节精确规划，容错性反而更高。
- 工程指标比单项抓取成绩更有传播力：约 5 路气压、约 300 美元原材料、硅胶本质柔软抗冲击——它证明的是「廉价安全灵巧手」这条路可行。
- 适用边界：对圆柱、球等规则几何效果好；扁平薄片与不规则物体的被动自适应可能不足，需要强力夹持或精细位置控制的工业任务不适用。
- 主要代价有三层：弹性耦合与接触力学难以精确建模，基于模型的高精度规划困难；气泵/气管/阀门的基础设施限制部署环境；充放气延迟限制高速动作。
- 定位：与 [MIDAS Hand](../entities/midas-hand.md) 是软 / 硬两条工程路线（顺应性换容错安全 vs 刚性换力量精度），与 [Yale OpenHand](../entities/yale-openhand.md) 同属欠驱动自适应但走腱驱 + 打印件；本文无软件开源，核心贡献在机械设计，RSS 2026 Test of Time 认可的是十二年的路线影响。
