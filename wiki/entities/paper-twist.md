...
官方仓库 [YanjieZe/TWIST](https://github.com/YanjieZe/TWIST)：`train_teacher.sh` → `train_student.sh` → `to_jit.sh`；sim2sim / 真机分别起 `server_low_level_g1_sim.py` 或 `server_low_level_g1_real.py`，高层 `server_high_level_motion_lib.py` 经 Redis 下发运动。一次完整运行如下：

```mermaid
sequenceDiagram
    autonumber
    actor U as 用户
    participant TEA as train_teacher.sh
    participant STU as train_student.sh
    participant JIT as to_jit.sh
    participant HI as server_high_level_<br/>motion_lib.py
    participant LO as server_low_level_<br/>g1_sim / g1_real
    participant REDIS as Redis
    participant G1 as MuJoCo / Unitree G1
    U->>TEA: 训练教师跟踪器
    TEA-->>STU: teacher checkpoint
    U->>STU: 训练 RL+BC 学生
    STU->>JIT: 导出 JIT
    U->>LO: 启动低层服务
    U->>HI: 启动高层 motion server
    HI->>REDIS: 发布目标运动
    loop 控制环
        LO->>REDIS: 拉取参考
        LO->>G1: 关节指令
...
- **高低层同构**：sim 与 real 只换低层 server 后端，接口保持一致。
- **TWIST2** 为后续全栈仓；本页时序对应该论文的 `YanjieZe/TWIST` 实现。
