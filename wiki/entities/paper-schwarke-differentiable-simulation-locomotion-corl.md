...
- **本页为索引级节点**（FreeDof 44 篇梳理 [41/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页要点是「兼顾梯度信息量与物理保真的接触模型 + 端到端策略优化」，对应证据是不同接触平滑化程度下**梯度质量与最终策略性能**的权衡曲线。
...
| 维度 | 读法 |
|------|------|
| **文内路线** | 归类 **可微仿真**；同路线其他节点见 [44 篇技术地图](../overview/freedof-sim2real-44-papers-technology-map.md) 与 [四条路线对比](../comparisons/sim2real-four-routes-identifiability.md) |
| **横比口径** | 与无梯度 RL 基线对比时须同时对齐样本预算与 wall-clock；只比样本数会高估可微仿真的收益。 |
