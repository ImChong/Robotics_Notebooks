...
type: entity
tags: [legged, reinforcement-learning, impedance, quadruped, sim2real, eth]
status: stable
...
**这篇给「RL 能不能自己学刚度」一个明确答案：能，但决定成败的是刚度参数化的粒度与结构约束，而不是把增益直接开放成无结构的自由通道。**

- 真正的设计变量是 **分组方式**：论文比较 PJS / PLS / HJLS 等逐关节、分腿与混合方案——「每关节完全自由」往往不如结构化分组稳定、也不如它好迁移。
- 阻尼侧通常保留物理一致或结构约束；把 `Kp` 与 `Kd` 都当成无结构独立通道，容易与动力学或执行器模型冲突。
- 工程连带成本容易被低估：刚度从 URDF 常量改成策略输出后，观测归一化、动作缩放与冲击/滑移惩罚都要同步改，否则同一 `Kp` 数值的语义已经变了。
- 与 [Learning Variable Impedance…](./paper-variable-impedance-contact-rl.md) 分工明确：后者偏 **操作接触与阻抗 shaping**，本文偏 **腿足户外 loco**；量化指标、消融与 sim2real 结果以原文为准，本页只保留方法结构。
