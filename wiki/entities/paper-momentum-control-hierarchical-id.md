...
Momentum Control 与本库同族的层级控制页面差异，主要在于「求解落在哪一层」以及「是否显式处理接触力」：

| 维度 | 本工作（Momentum Control） | [Stack of Tasks](./paper-hmi-stack-of-tasks.md) | [tsid](../concepts/tsid.md) |
|------|----------------------------|-------------------------------------------------|-----------------------------|
| 方法族 | 层级逆动力学（加速度 + 接触力 + 力矩一致求解） | 层级广义逆运动学（速度层任务栈） | 任务空间逆动力学求解框架 |
| 求解输出 | 广义加速度、接触力、关节力矩 | 广义速度或其积分后的姿态命令 | 加速度/力矩（含接触约束） |
| 接触与摩擦 | 显式纳入摩擦锥、单边接触与力矩/关节限制 | 不含接触力，仅几何/速度任务 | 含接触力与动力学约束 |
| 高层目标 | 质心线动量 + 全身角动量作为第一层任务（见 [centroidal-dynamics](../concepts/centroidal-dynamics.md)） | 末端/质心的几何误差 | 任务加速度跟踪 |
| 关系/取舍 | 比纯 IK 多出动力学与接触可行域，可直接产出力矩；代价是求解更重、依赖正确的接触计划 | 需下游动力学层补力控 | 提供本工作所需的任务空间动力学求解骨架 |
