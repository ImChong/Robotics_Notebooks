...
**去中心化 ACO + flocking 在仿真 4v4 上证明：角色重分配可以亚秒到数秒级完成，且比集中式更能扛丢包与单机失效——但还不是真机联赛答案。**

1. **战术层可与单机 RL 正交** — swarm 管谁去踢/怎么站，踢球技能另嵌。
2. **读进球 +25–40% 时看基线** — 对照的是集中式/静态角色，不是 Adult-Size 冠军全栈。
3. **通信仍是周期性 UDP** — 与 SPL 事件驱动极低带宽是不同轴。
4. **真机缺口** — Webots 为主；换真草与联赛规则需重标定。
5. **选型** — 要仿真快协商选本文；要真机夺冠栈选 [ARTEMIS](./paper-notebook-a-hierarchical-model-based-system-for-high-perfo.md)。
