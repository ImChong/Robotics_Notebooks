...
Whole-Body Behaviors 把 OSF 的单任务动力学推广到多优先级行为合成。与相邻页面对照，可看清它在层级实现与约束语义上的位置：

| 维度 | 本工作（Whole-Body Behaviors） | [OSF](./paper-operational-space-formulation.md) | [Stack of Tasks](./paper-hmi-stack-of-tasks.md) | [hqp](../concepts/hqp.md) |
|------|--------------------------------|-------------------------------------------------|-------------------------------------------------|---------------------------|
| 方法族 | 层级行为基元 + 动态一致零空间投影（力矩层） | 操作空间单任务运动/力动力学（前驱） | 软件化广义逆运动学栈（速度层） | 层级二次规划 |
| 层级如何实现 | 严格优先级，低优先级投影到高优先级动态一致零空间 | 单主任务 + 零空间次任务 | 前一任务雅可比零空间内速度层递归 | 各层间以不等式约束求解 |
| 求解层 | 关节力矩 | 关节力矩 | 广义速度 | 速度或力矩 |
| 冲突/约束语义 | 优先级硬保证平衡不被牺牲，但可能长期饿死低层、切换处不连续 | 按方向选择运动/力子空间 | 以等式任务为主 | 原生表达限位/摩擦/接触等不等式 |
| 关系/取舍 | 把 OSF 从单任务推广到多优先级全身行为合成 | 提供其依赖的任务空间动力学基础 | 后来更工程化、落在速度层的实现 | 后来用 QP 统一硬约束的求解形态 |
