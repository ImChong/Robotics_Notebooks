...
**X-World 把多摄驾驶世界模型定位成「可复现的评测与数据扩增底座」：用生成式保真换可控性与长尾覆盖，而不去争解析物理引擎的位置。**

- 它的成立条件是三项同时守住：7 摄跨视角几何一致、动作严格跟随、连续数十秒 rollout 不崩；任一失守，闭环策略评测就会系统性失真。
- 可控性是它区别于普通视频生成的核心接口——除动作外还能叠加他车/静态要素、天气与地区风格文本，从而支撑反事实与长尾扩增。
- 证据强度需打折：公开叙事偏定性，定量表以 PDF 为准、项目页以演示视频为主；且 **未开源**，无法独立复现或做外部基准对齐。
- 主要风险是把「像行车记录仪」当成物理正确——生成式保真 ≠ 可证明动力学；主战场是车载环视，迁移到人形操作需另证。
- 栈内定位：[X-Cache](./paper-x-cache.md) 加速它，[X-Foresight](./paper-x-foresight.md) 的 Renderer 建于其上，[X-Mind](./paper-x-mind.md) 沿用同一传感配置——本页是 **级联仿真底座**，后两者才把世界预测嵌进 VLA。
