...
**Psi0 的定位是人形 loco-manip 的通用基座：胜负手放在异构动捕与合成数据的配比上，而不是新的动作头结构。**

- 链路是「多模态编码 → VLM 语义规划/路由 → 潜变量/动作 token → 动作 chunk」，高层语义与低层动作被放进同一条训练/部署路径以减少断点。
- 数据侧的「异构 + 合成平衡」是真正的差异点；复现难度也主要落在这里，而非模型结构。
- 追求的是覆盖面而非单任务最优：归在 09 人形 VLA、世界模型与通用操作，与窄域专用策略不是同一个评价维度。
- 题目自称 open foundation model，但本页只记录了项目页链接，权重与代码的实际开放范围需回项目页确认；量化指标同样以原文为准。
