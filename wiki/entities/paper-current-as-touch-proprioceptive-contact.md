...
**Current as Touch 真正的贡献不是「用电流测力」，而是把接触反馈翻译成位置控制栈本来就能吃下的东西——用接口兼容性换传感精度。**

- 起作用的是 **CRP 这一层接口**：电流 + 关节状态 → 柔顺目标关节角，底层仍是标准 PD，靠位置误差 **间接** 产生交互力，全程不切力矩模式。
- 电流–力标定（Dex3 RMSE **10.09g** / $R^2{=}0.99$，LEAP **17.75g** / $R^2{=}0.95$）只是 **动机证据**——说明执行器自带信号里含接触信息，不是精确力估计的终点。
- 适用边界清晰：面向 **输出目标关节角** 的主流 teleop / IL pipeline，任务集中在纸杯、擦拭、薄片、动态负载等 **以法向力为主** 的 contact-rich 场景。
- 主要失败模式在信号侧：电流受 **电机模型、温度、摩擦** 影响，**切向微滑** 观测弱于 dedicated tactile，跨手迁移需重新采集。
- 与 [VT-WAM](./paper-vt-wam-visuotactile-contact-rich.md)、[TACO](./paper-taco-tactile-wm-vla-posttrain.md) 对照：后两者用外置触觉 / 腕部 F/T 换分辨率，本页用 **零额外硬件** 换部署门槛，占据传感成本光谱的低端。
