...
**ASAP 把 sim-to-real 差距当成一个可学的量：先在仿真里用人类动作数据预训练 motion tracking 策略，再用真实数据训练 delta action / model 去修正仿真与真实之间的动力学偏差。**

- 修正作用在 **动力学层面** 而非策略容量：论文把敏捷全身动作的瓶颈定位为 sim–real 动力学偏差，所以补的是偏差项，不是继续堆更强的跟踪策略。
- 它承认了多数 sim-to-real 工作回避的现实约束：真机采集敏捷动作会遇到电机过热、硬件损伤与数据规模受限——高动态真机数据本身昂贵且危险，这直接限制了 delta 模型可用的数据量。
- 定位与边界：本页属 42 篇栈 **03 感知式高动态运动**（#25/42）；这一层的难点在闭环时延与几何误差，不是「加个相机输入」就能解决。
