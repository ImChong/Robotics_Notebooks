...
| 对照对象 | 差异要点 |
|----------|----------|
| [Rapid Locomotion RL](./paper-rapid-locomotion-rl.md) | Rapid 面向连续地形高速盲走；本文补齐间断地形（沟/障）的视觉前瞻跳跃 |
| 假设完美深度图的感知栈 | 本文把跳跃时相机晃动/运动模糊作为一等问题，而非假设干净深度输入 |
| [视觉辅助动态探索](./paper-vision-aided-dynamic-exploration-mini-cheetah.md) | 探索文用轻量落脚评估过非结构地形；本文用像素级策略执行敏捷跳跃技能 |
