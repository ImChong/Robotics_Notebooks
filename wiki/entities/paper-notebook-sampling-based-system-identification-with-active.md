...
**SPI-Active 的主张是：高精度腿足技能不该靠域随机化「把未知量全抖一遍」，而该把参数辨识出来——并且连「采什么数据」也一起优化。**

- 起作用的是两阶段闭环：**SPI** 用 GPU 大规模并行采样（CMA-ES）最小化仿真与真实的轨迹误差，反推质量-惯量与电机扭矩参数；**Active** 再以 D-最优（最大化 Fisher 信息）优化指令序列，激发最能暴露参数的高扭矩步态后回炉重新辨识。
- 它绕开了传统 SysID 的两个硬前提——动力学可微、能直接测关节扭矩——这两条在富接触腿足系统上都不成立；采样式优化只需标准状态轨迹。
- 与 DR 的取舍很明确：DR 以保守换鲁棒、牺牲精度且随机化范围靠手调；本文以辨识换精度，代价是必须真机采数据并额外跑一轮主动探索。
- 实证边界清楚：Unitree Go2（含 33% 体重负载）上精准前跳/偏航跳与速度、姿态跟踪较基线提升 **42–63%**，前跳落点误差低至 ~3.6 cm，并在 G1 人形上验证泛化。
- 工程落地友好：全套代码开源（[LeCAR-Lab/SPI-Active](https://github.com/LeCAR-Lab/SPI-Active)），可直接复用为新平台的辨识流水线。
