...
**DexCap: Scalable and Portable Mocap Data Collection System for Dexterous Manipulation** 已列入 [Humanoid Robot Learning Paper Notebooks](https://imchong.github.io/Humanoid_Robot_Learning_Paper_Notebooks/index.html) 的 **PROGRESS.md 待深读** 清单（分类：06_Manipulation）。本页为 **计划索引实体**，深读笔记尚未撰写；笔记完成后应链向笔记站并深化归纳。
...
**DexCap 在本库目前只是一条「已排期、未深读」的占位实体：它补的是知识图谱的完整性，而不是可直接引用的方法结论。**

- 本页能确认的只有索引层信息——arXiv 2403.07788、分类 06_Manipulation、计划中的笔记文件夹；论文的方法机制、评测口径与实机结果在本页一概未展开。
- 适用边界：适合经[分类父节点](../overview/paper-notebook-category-06-manipulation.md)与[人形论文笔记总索引](../overview/humanoid-paper-notebooks-index.md)做检索与交叉定位，不适合作为技术选型或方法对比的依据。
