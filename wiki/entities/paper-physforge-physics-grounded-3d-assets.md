...
**PhysForge 的核心判断是「物理规划」与「物理实现」应当解耦：VLM 出分层物理蓝图，扩散在 KineVoxel Injection 里把连续关节参数与几何一起去噪，目标是生成能进仿真的关节资产而不是只能看的静态外壳。**

- 分工是机制主线：VLM 负责结构/语义/离散物理属性与部件分解消歧（共预测物理属性可减轻部件粒度歧义，因而支持无 mask 规划），扩散头负责连续关节参数回归，KVI 则是二者在同一去噪框架内的耦合接口。
- 数据侧 PhysDB（约 15 万对象、整体/静态/功能/交互四档标注）是整条流程的前提，但论文自陈大规模下精确 3D 轴标注困难，关节数值真值仍需 PartNet-Mobility、Infinite-Mobility 补监督——不要把数据规模等同于可用真值。
- 落地门槛高：依赖 VLM 微调、多数据源与扩散训练栈，复现与算力成本显著；相比纯程序化生成，逐字段人工改蓝图的可编辑体验需以官方代码为准。
- 定位对照：与 [Articraft](./articraft.md) 的 Agent + SDK 程序化路线、[PhysX-Omni](./physx-omni.md) 的 sim-ready 统一生成，构成「学习式端到端 vs 符号程序 + 验证闭环 vs PhysX 系统一生成」的谱系。
- 本页正文侧重方法结构与知识库交叉引用；定量表格与 ablation 见原文 PDF。
