...
**飞控与通信** 以 [PX4](../entities/px4-autopilot.md) + **MAVLink**（[MAVSDK](https://github.com/mavlink/MAVSDK)）为事实标准；**运动规划** 常用 [EGO-Planner Swarm](https://github.com/ZJU-FAST-Lab/ego-planner-swarm)；**仿真** 分三路——**Gazebo 教学栈**（[XTDrone](https://github.com/robin-shaun/XTDrone)）、**高保真视觉**（[AirSim](../entities/airsim.md)、[Flightmare](https://github.com/uzh-rpg/flightmare)）、**轻量 RL**（[gym-pybullet-drones](https://github.com/utiasDSL/gym-pybullet-drones)）；**真机微四轴 swarm** 走 [Crazyflie 固件](https://github.com/bitcraze/crazyflie-firmware) + [Crazyswarm2](https://github.com/IMRCLab/crazyswarm2)。
...
| 伴机 Offboard / 自动化测试 | MAVSDK | MAVROS（XTDrone 教程栈） | 手写裸 MAVLink 除非必要 |
| 未知环境快速重规划、多机避碰 | EGO-Planner Swarm | 其他 ESDF planner | 把规划器当飞控内环 |
| 中文教程 + Gazebo + PX4 全链路 | XTDrone | — | 期待与 AirSim 相同画质 |
| 视觉 SLAM / 深度学习、UE 场景 | [AirSim](../entities/airsim.md) | Flightmare | 用作高保真接触/桨叶 FEM |
| 敏捷飞行 RL、高并行 | Flightmare | gym-pybullet-drones | 首版就上真机无保护 |
| 课程/论文标准 UAV RL 基准 | gym-pybullet-drones | quad-swarm-rl | 与 Isaac Lab 腿式环境混为一谈 |
| 室内 50+ 微四轴灯光秀 | Crazyswarm2 | — | 无动捕硬飞 swarm |
...
- **[MAVSDK](https://github.com/mavlink/MAVSDK)**：高层 C++/Python API（起飞、任务、Offboard、遥测），连接 PX4/ArduPilot。
- **[crazyflie-firmware](https://github.com/bitcraze/crazyflie-firmware)**：微四轴嵌入式栈，CRTP + 甲板扩展；与 PX4 **不同赛道**。

### 规划

- **[ego-planner-swarm](https://github.com/ZJU-FAST-Lab/ego-planner-swarm)**：ESDF + B-spline 局部规划，swarm 避碰；输出通常转为 **位置/速度设定点** 而非电机 PWM。

### 仿真

- **[XTDrone](https://github.com/robin-shaun/XTDrone)**：PX4 + ROS + Gazebo 教学与实验集合。
- **[AirSim](../entities/airsim.md)**：UE/Unity 视觉仿真；维护模式下降，仍是视觉 UAV 文献常见基线。
- **[Flightmare](https://github.com/uzh-rpg/flightmare)**：研究向敏捷飞行与并行渲染；Unity 客户端。
- **[gym-pybullet-drones](https://github.com/utiasDSL/gym-pybullet-drones)**：Gymnasium 四旋翼 RL，轻量可复现。
- **[quad-swarm-rl](https://github.com/Zhehui-Huang/quad-swarm-rl)**：多机 Gym 环境补充，社区较小。

### 真机群体

- **[Crazyswarm2](https://github.com/IMRCLab/crazyswarm2)**：Crazyflie 大规模编队，ROS2 + 动捕/UWB。
...
- [PX4 Autopilot](../entities/px4-autopilot.md)
- [AirSim](../entities/airsim.md)
