...
summary: "PETS（具身世界模型六路线专题）：概率动力学集成+不确定性传播增强 CEM，真机样本高效 MBRL 代表。"
---

# PETS

**PETS** 收录于 [具身智能研究室 · 具身世界模型六路线综述](https://mp.weixin.qq.com/s/mmIJRp9g6NqblMCjd9D5GQ) **规划主导型** 段。本页为知识库 **策展编译** 详情节点；量化指标以原文 PDF / 项目页为准。

## 一句话定义

概率动力学集成+不确定性传播增强 CEM，真机样本高效 MBRL 代表。
...
|----|------|
| **路线** | 规划主导型 |
| **出处** | [arXiv:1805.08034](https://arxiv.org/abs/1805.08034) |
| **文内角色** | 概率动力学集成+不确定性传播增强 CEM，真机样本高效 MBRL 代表。 |

## 结论

**PETS 在六路线框架下的读法：先看预测输出被谁消费，再看是否改善真实行动——而非只看网络新旧或画面观感。**
...
- **本页无量化数字**：六路线综述只给出该工作在 taxonomy 中的定位，未转述实验表格；成功率、消融与实机协议以 [arXiv:1805.08034](https://arxiv.org/abs/1805.08034) 为准。
...
- **同路线邻接工作**（综述 **规划主导型** 段）：[Visual Foresight](./paper-visual-foresight-latent-mpc.md)、[PlaNet](./paper-planet-latent-dynamics.md)、[TD-MPC2](./paper-td-mpc2.md)。
- **对照要点**：PETS 用 **概率集成显式建模模型不确定性**，把不确定性直接写进 MPC 目标；视觉/隐空间路线则先解决「从高维观测里学出可规划的状态」。
...
- [arXiv:1805.08034](https://arxiv.org/abs/1805.08034)
