...
**Discovering Self-Protective Falling Policy for Humanoid Robot via Deep Reinforcement Learning** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：04_Loco-Manipulation_and_WBC），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇工作的核心主张是「护身姿态不该由人规定」：刚性机体的物性与人不同，让 RL 自己搜出来的「三角」结构，才是贴合自身形态的降损解。**

- 真正起作用的是**奖励设计 + 域多样化课程**的组合：前者把目标锁死在损伤最小化，后者负责覆盖方向、初速、地形各异的长尾跌倒场景——这正是控制法难以覆盖的那部分。
- 最有信息量的结果是涌现的**「三角」支撑结构**：它是策略搜索的产物而非人为先验，反过来印证了「人类护身动作先验未必适配机器人」这一出发点。
- 定位边界要说清：这是「摔得聪明」而不是「不摔」，属于跌落后的硬件保护层，与平衡/恢复类工作互补而非替代。
- 与 SafeFall、Robot Crash Course、VIGOR、Unified Fall-Safety Policy 同属人形「跌落安全」研究簇，本文的区分点在于**姿态由 RL 发现**而非设计。
- 已迁移到真实平台，但损伤量化指标与消融本页未展开，以深读笔记与论文 PDF 为准（见 [参考来源](#参考来源)）。
