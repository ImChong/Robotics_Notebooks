...
**SRU 的真正论点不是「换一个更强的循环单元」，而是指出端到端无地图导航里 RNN 的短板在空间配准而非时序记忆——补上一个 star operation 空间变换项就足以把差距拉开。**

- 真正起作用的是 **对 LSTM/GRU 递推的最小修改**：楼梯、坑洞这类需 3D 空间记忆的场景成功率从 **~33%** 提到 **~83%**，整体 **78.9%** vs LSTM 63.5% / GRU 61.0%（**+23.5%**），相对显式建图 EMHP **+29.6%**、相对堆叠帧 GTRL **+105%**。
- 长程是这条路线的主场：显式历史窗口约 **20 m** 后陡降，SRU 隐式记忆在 **50 m+** 仍 >80%、**120 m** 轨迹 >70%；真机森林单次穿行 **100 m+**，远超 30 m 的训练最长距离。
- 适用边界很明确：目标是 **相对向量/坐标**，既非 image-goal 也非语言指令，不可与 [NavWAM](./paper-navwam-goal-conditioned-visual-navigation-wam.md) 或 [视觉–语言导航](../tasks/vision-language-navigation.md) 的 benchmark 混比；输入刻意只用 **单前向深度**，换成 360°/多相机需重新验证。
- 主要局限：它不替代 SLAM——真机仍靠 DLIO 做状态估计，定位误差会直接传导到目标相对几何；上游平台是轮足 **B2W**，纯腿式需重新验证动力学（[SRU-Odin](./sru-odin.md) 在 Go2 上做的是工程移植）。
- 工程成本不对称：SRU 模块本身可 `pip install`、易嵌入既有 RL 栈，但完整复现依赖 IsaacLab + rsl_rl + 多仓扩展，推理侧即便 ONNX 化，预训练深度编码器仍占算力。
