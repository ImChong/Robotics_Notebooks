...
**这篇的主张是把 retargeting 从「训练前处理一下数据」升格为独立研究对象：参考轨迹的物理可行性，往往在 RL 开始之前就决定了 motion tracking 策略的上限。**

- 论证落在 **失败机制** 上：脚滑、不可行姿态、起始姿态不合理会把策略逼进两难——严格跟踪会摔、保持稳定又会偏离参考，最终控制器 **既不像人也不够稳**。
- 评价标准是 **双目标** 而非单一相似度：重定向结果既要接近源动作，又要适合训练控制器；论文用 **用户研究 + sim2sim 成功率** 同时衡量这两面。
- 比较面覆盖 GMR、PHC、ProtoMotions 与 Unitree 官方重定向，把 embodiment gap 的处理差异变成可对照的工程选型。
- 边界：本页为策展编译，量化 benchmark 与消融以原文 / 项目页为准；方法层细节见 [motion-retargeting-gmr](../methods/motion-retargeting-gmr.md)。
...
- [机器人论文阅读笔记：Retargeting Matters](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/02_Motion_Retargeting/Retargeting_Matters__General_Motion_Retargeting_for_Humanoid_Motion_Tracking/Retargeting_Matters__General_Motion_Retargeting_for_Humanoid_Motion_Tracking.html)
