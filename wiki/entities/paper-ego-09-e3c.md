...
**E³C 把第一视角视频生成的评判标准从「画面像不像」改成「空间稳不稳」：3D 环境记忆存在的理由，是让生成的未来可被推演，而不是好看。**

- 方法骨架由三个关键词撑起：**3D environmental memory** 提供空间一致性，**ego human control** 与 **exo human control** 提供人体姿态的可控性。
- 问题动机来自 ego 视频本身的性质：相机与身体绑定、视角变化快、遮挡多、人的动作常只露出一部分——没有 3D 记忆，生成结果很容易在空间上漂。
- 放到机器人语境里，这条标准直接对应世界模型的可用性：想象出的未来必须保持 **场景结构、身体动作与物体变化** 的一致，观感达标不等于可推演。
- 边界：本页为策展编译，量化 benchmark 与消融以原文 / 项目页为准；Ego 视频仍须经重建、对齐、重定向与物理过滤才能变成策略数据。
