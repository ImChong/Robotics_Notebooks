...
- [x] 技术路线页 `roadmap.html` 本库超链接同样悬停浮窗：正文、知识地图叶子、阶段速览相关项中的 `detail.html?id=` / `roadmap.html?id=` 复用同一套 hover 卡片；无迷你图时类型徽标回退 `roadmapKmapNodeType`（path 前缀）。验证脚本 `scripts/verify_roadmap_inline_link_preview.cjs`。
