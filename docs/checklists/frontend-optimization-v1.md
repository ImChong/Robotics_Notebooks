...
- [x] 更新记录页 `change-log.html`：超量日先预览 10 条；提供「再展开 10 项 / 展开全部 N 项 / 收起至前 10 项」，视觉沿用箭头+文案样式（PR#1245）。
  ...
  - [x] 图谱页 3D 社区漂浮标签：随相机距离（滚轮缩放）与节点一起放大缩小（`translate3d` + `scale`，相对首次适配基线距离，桌面钳制约 **0.4–1.85**）；2D 标签在 `gRoot` 变换内天然跟随。
  ...
  - [x] 图谱页 `graph.html`：筛选浮窗「按类型 / 按社区 / 按健康度」改为与「路线视图 / 研究机构」一致的可折叠 `<details>`；三者着色/筛选仍为三选一（切换即清空其它维度勾选），折叠标题在有选中时显示数量（如 `3 个社区`），展开某一维度时切换着色落点并收起另外两段。
