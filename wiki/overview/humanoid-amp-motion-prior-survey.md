...
| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| AMP | Adversarial Motion Prior | 对抗判别约束状态转移接近人类运动分布 |
| ADD | Adversarial Differential Discriminator | 差分判别减碎片 reward，AMP 组件化演进 |
| SMP | Score-Matching Motion Prior | 可复用 score-matching 运动先验模块 |
| RL | Reinforcement Learning | AMP 与任务 reward 联合优化的训练范式 |
| GMP | Generative Motion Prior | 生成式细轨迹指导人形走跑的代表路线 |
| VLA | Vision-Language-Action | 身体先验成熟后上层调用的策略接口 |
...
> 标题以公众号 2026-05-21 抓取版为准；**站内已有页面** 以链接标出，其余暂时只能读原文。
