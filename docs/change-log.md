...
**补齐 detail page 的异步锚点落点能力**
- 扩展 `docs/main.js`，为 detail page 新增 hash 深链恢复逻辑：正文渲染完成后会读取 `window.location.hash`，定位对应标题并滚动到目标位置
- 同步监听 `hashchange`，让正文内跨页带锚点链接、TOC 跳转和标题复制锚点后的再次打开都能命中正确章节
- 扩展 `docs/style.css`，为当前 hash 命中的标题增加短暂高亮态，降低长文中“跳到了哪”的识别成本
- 更新 `docs/detail.html` 文案，明确 detail page 已支持带锚点的站内精确跳转
- 扩展 `tests/test_content_sync.py`，锁定 hash 深链恢复与目标高亮钩子

### 项目阶段变化

- detail page 现在不只是“有 TOC / 有内链 / 能复制锚点”，而是把 **跨页锚点打开 → 异步正文渲染 → 命中目标标题** 这一段真正闭环补齐了
- 这一步和刚完成的正文内链路由打通直接衔接，进一步把 detail page 从“能看”推进到“能稳定导航长文”
...
- 再继续扩展 `docs/main.js`，把 tech-map 节点列表改为按 layer 分组的可折叠 `details` 区块
- 扩展 `docs/style.css`，补齐 tech-map 分组容器、折叠标题和分组内网格样式
- 再次扩展 `tests/test_tech_map_filter.py`，验证分组折叠渲染逻辑接入点
