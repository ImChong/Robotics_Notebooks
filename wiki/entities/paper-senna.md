...
**Senna 把「VLM 出自然语言高层决策 + E2E 出精确数值轨迹」解耦——语言模型做常识、专用网络做几何，避免直接让 VLM 回归轨迹的大误差。**

1. **双模块解耦** — Senna-VLM 由多视角图像→自然语言高层决策（如「红灯，直行减速停车」）；Senna-E2E 以该决策为条件出精确数值轨迹。
2. **报告增益** — DriveX 预训练 + nuScenes 微调设定下，盘点称规划误差约 **-27.12%**、碰撞率约 **-33.33%**（以论文为准）。
3. **定位对照** — 与 DriveVLM 的 CoT、EMMA 的一切皆语言构成「VLM 如何接入控制」对照三角。
4. **开源** — `hustvl/Senna` 已开源，依赖 LLaVA 等基座。
5. **边界** — 决策语言空间设计不当会成条件信息瓶颈；两套模型的延迟与一致性需系统层调度；DriveX 等数据若不完全公开则复现受限。
