...
**这篇的赌注是「离散地形的关键不确定性只在下一脚落点」：既然如此，就把传感器搬到接触点上，用足底 4×4 ToF 换掉整条高程图重建管线。**

- 真正起作用的是 **传感位置** 而非传感量：足端直连读数绕开了「足位运动学 + 机载地图」的二次投影；仿真对照显示无扰动时理想高程扫描与足端传感 **持平**，一旦关节偏置 / 状态估计失配（里程计噪声 0→400% 时融合高程图 **51.7%→24.3%**），越靠近接触点的传感失败点越少。
- 可迁移性靠 **两阶段课程 + 感知退化随机化**（静态偏置、比例+高斯噪声、随机缺失、20–60 ms 实测时延注入），真机 ANYmal-D 在 60 cm 沟、20×20 cm 错落踏石、20 cm 平衡木上以约 **0.52 m/s** 通过，并涌现摆腿扫描等行为。
- 适用边界是 **近场落脚**：既不是「完全无感知」（仍依赖每足 16 点局部几何），也不替代 LiDAR 建图——远距离导航与语义障碍不在此文范围。
- 主要失败模式在物理层：**强吸收/镜面地面、足端泥污堵塞** 会让 ToF 失效，红外透明护罩 / 自清洁被列为后续方向。
- 定位是 **正交而非替代**：相对 [Extreme Parkour](../entities/extreme-parkour.md) 的全局视觉与 [Learning Quadrupedal Locomotion over Challenging Terrain](./paper-notebook-learning-quadrupedal-locomotion-over-challenging.md) 的本体盲走，它占的是「最小感知 × 离散地形」这一格，可与重型感知栈互补或作低功耗备选。
