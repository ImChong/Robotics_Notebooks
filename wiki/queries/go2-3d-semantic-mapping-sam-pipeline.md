...
| SAM | Segment Anything Model | 提示式二维分割，输出像素 mask（非三维点） |
| SAM2 | Segment Anything Model 2 | 增加视频时序跟踪的 SAM 系模型 |
...
SAM / SAM2 从图像生成 **二维像素掩码**：不自动生成三维点，也不自动给出物体名称。原始 SAM 接受点/框/掩码提示；SAM2 增加视频跟踪。
...
[CMU MSCV Semantic 3D Mapping](../../sources/sites/cmu-mscv-semantic-3d-mapping.md) 正是：DETR 框 → SAM 实例 mask → 外参/位姿映射到 3D 点云（伪标注/检测管线）。
