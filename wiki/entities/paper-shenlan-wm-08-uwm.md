...
**UWM 的取舍是把视频与动作塞进同一个扩散 Transformer，再用「模态特定扩散时间步」这一个旋钮，让同一模型分别当策略、动力学模型或视频生成器。**

- 真正起作用的是联合建模而非多头拼接：未来观测与动作在同一骨干中一起去噪，因而不像级联架构那样逐段累积误差，适合端到端 VLA / 操控闭环。
- 代价在于耦合——模型必须同时把视频生成与动作预测学好，不再能像级联路线那样单独替换或复用某一段模块。
- 使用边界：本页为策展索引级（RSS 2025，[路线 02 联合架构](../overview/world-models-route-02-joint.md)），量化 benchmark 与实机指标以原文为准；引用量不代表下游增益，复现前需核对 License 与权重。
