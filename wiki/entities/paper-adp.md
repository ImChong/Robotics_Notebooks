...
**把 AMP 式对抗先验从运动学风格换成 SRBD-TO 动力学时间窗，能显著提升推扰恢复，且不必强迫全身关节跟踪。**

1. **先验对象换层** — 判别 CoM 速度、质心角动量、接触力与二值接触指示的时间窗，而非 mocap 运动学片段。
2. **动力学偏差更早暴露** — 推后约 20 ms 即可放大（相对运动学约 160 ms），对抗奖励对瞬态更敏感。
3. **相对 AMP 量化优势** — 四向推扰成功率 91.4% vs 73.4%；\(J_{80}\) 115.5 vs 99.0（+16.7%）；恢复 2.48 s vs 4.76 s（−47.9%）；速度误差 0.84 vs 1.30（−35.4%）。
4. **需要分布级窗，而非点式匹配** — Dynamics Reward 同特征无对抗，远不及 AMP/ADP；接触指示消融跌幅最大，\(K=8\) 为最佳折中。
5. **与 SD-AMP 正交** — 后者做 regime 门控，ADP 做特征空间；可组合思考。
6. **选型注意** — 真机仅为定性；开源 coming soon；纯 ADP 可能稳但不够像人，文中建议未来再组合运动学先验。
