...
**Mobile-TeleVision 的重心从「怎么把人的动作传过去」移到「机器人怎么把传过来的目标走稳」：遥操作只负责给可跟踪的身体目标，吃力的是背后那套预测式运动先验驱动的全身控制。**

- 关键机制是把多源输入（本体状态与关节序列、人类视频/动捕轨迹、接触力/触觉信号）先归一成可跟踪的身体目标，遥操作接口与全身控制器因此解耦。
- 策略侧是 PPO/RL、AMP 运动先验与扩散/流匹配的组合而非单一方法，本页未说明各自权重，具体配方以 [原文 / 项目页](https://mobile-tv.github.io/) 为准。
- 输出里带地形/场景表征，对应「Mobile」这一半：移动中的遥操作必须处理地形，不只是原地的上半身操作。
- 它被归入 **08 硬件平台、感知配置与部署扩展**，落点在整机部署形态；底层 WBC 鲁棒性仍须与运控/接触控制条目对照。
