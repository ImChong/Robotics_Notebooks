...
- **触发：** 计划任务巡检——审计 `roadmap/*.md` 自 2026-08-16 structural 提交（"补齐 2026-08-09 以来入库的 11 篇里程碑级知识节点"）以来 `log.md` 顶部约 210 条记录（2026-08-17 → 2026-09-06）新增的全部知识节点；期间 TRAMP / CMoE / CReF / WARP / AHMP / SE(3) 切空间 TO / Instant NuRec / τ₀-VLA / GlanceWAM / HarnessEval-W / IRL 一手谱系（Ng 2000 / MaxEnt / GAIL / AIRL）/ 强化学习史 / HumanTracker 等已由后续 ingest 提交直接补入对应 roadmap 文件，跳过；[`depth-icl.md`](roadmap/depth-icl.md)（09-04 新建第 23 条纵深路线）与 [`depth-embodied-eval.md`](roadmap/depth-embodied-eval.md)（09-03 新建 + 补 Stage 4）在建页当时已吸收各自窗口内的 ICL / 评测节点（HOST、StellaVLA、Zero-WAM、Skild S1、运控评测指标等），本轮不重复回填。整体排除：国内开源全景 424 项（298 个 `cn-os-*` 机械节点化实体）与 Awesome 索引级 `paper-sa-*` 页（含 PAI-Bench，索引级非里程碑）；具身智能小站 / CLAP 九篇 / REALab 十四篇等约十组公众号「N 篇盘点」批量节点化的近 80 个 `paper-*`（逐篇复核后未见单点里程碑，多为未开源或问题面窄）；NVIDIA 工具族（OSMO / Holoscan Sensor Bridge / Jetson / GTC / Cosmos 全族 / Warp / MuJoCo Warp / Newton / Kamino / Isaac Launchable / Learn OpenUSD / NuRec 产品页）与 Plotly / EasyMocap / robot_descriptions.py / URDF Files Dataset / Isaac Lab 197 环境清单 / LeTools / Robocurve / Lightwheel RoboFinals 等厂商或通用工具与评测平台页（非方法论文级贡献）；RT-1 / RT-2 / OpenVLA / π₀ / Octo / Diffusion Policy / CLIP / ACT / SPOC / DINOv2 十篇 reading-roadmap 新建经典页（对应方法页已在 depth-vla / depth-imitation-learning 既有条目中覆盖，避免同一知识点双链）；统一机器人学习 / 操作基础模型 / VLA / 操作鲁棒性四篇综述与世界模型定义–功能分类文（通用综述不专属单一纵深）；另跳过 SONIC-Transfer（无 arXiv 的 draft 预印本，且与跨具身迁移既有 Any2Any 条目重叠）、CLAP / Riemann-1.0 / LAWA / LD4WAM / DECOWAM / HiTac-WAM / Motus2 / WALL-SS / AnyWorld / RISE（与 [`depth-wam.md`](roadmap/depth-wam.md) Stage 3 已高度饱和的 Joint WAM 条目叙事重叠）、Sparse-3D Traversal / SOLO / EATR-Stereo（与 Stage 1–3 既有感知越障条目重叠且未开源）、Fail-Passive Gap（西门子工业功能安全认证；[`depth-safe-control.md`](roadmap/depth-safe-control.md) 现有 Stage 0–3 为 Lyapunov→CBF→Safety Filter→Safe RL 经典链，无对口 Stage，按「不新建 Stage」口径不强插，存疑从严排除）、DemoMimic / Blind Dexterity / DexHoldem / DexBench / TwinDEX / NestDex（单次示范或灵巧基准/采数硬件，与既有 ADEPT / 灵巧采数指南条目重叠且多数未发布）、行为克隆四条 mystery 与 PPO 万字长文（工程配方或既有页加深，非新里程碑）、轮腿三仓 / 赛车漂移景观 / HydroGym / 多旋翼 Quiver / Multi-Agent CAD / Tennis-Vision（非人形机器人纵深主线）；仅在既有 Stage 内追加 bullet，未新建 Stage、未改动其余既有内容
- **[`roadmap/depth-motion-retargeting.md`](roadmap/depth-motion-retargeting.md)：** Stage 3 方法谱系补 [UMR](wiki/entities/paper-umr-unified-motion-retargeting.md)（HKUST-GZ / 诺亦腾 / 汉阳大学；规范 T-pose 稠密点云索引对应替代手写关键点表，表面位姿与接触向量同走一套点对，换源换机复用对应而非重画映射；接触任务关节误差相对 OmniRetarget 约 −40–56%，G1 真机旋踢/捡球/爬楼，代码待发布）
- **[`roadmap/depth-vla.md`](roadmap/depth-vla.md)：** Stage 4 部署与系统整合补 [FlashVLA](wiki/entities/paper-flashvla.md)（UCSD/MIT；交错噪声缓冲 + chunk 级因果注意力做流式异步动作解码，一块缓冲同时压低逐步延迟与异步观测错配；LIBERO 异步 2.43×（96.9%→97.8%）、真机单卡 ≥30 Hz，代码与 π₀.₅ 权重已开源）
- **[`roadmap/depth-perceptive-locomotion.md`](roadmap/depth-perceptive-locomotion.md)：** Stage 4 系统整合补 [VB-Com](wiki/entities/paper-notebook-vb-com-learning-vision-blind-composite-humanoid.md)（ICRA 2026；只吃历史本体的回报估计器判定高程图失效并切回已学会接触恢复的盲策略，G1/H1 双机验证高速迎面行人与零高程输入降级）——正对该 Stage「何时该退回盲走模式」这一此前无对应读物的核心问题
- **[`roadmap/depth-loco-manipulation.md`](roadmap/depth-loco-manipulation.md)：** Stage 1 全身控制基座补 [HTD 解耦 WBC](wiki/entities/htd-decoupled-wbc.md)（[Humanoid Touch Dream](wiki/entities/paper-humanoid-touch-dream.md)，IROS 2026，CMU/UTA/Bosch 的下肢+腰控制器：Isaac Lab 单 GPU 训 teacher、BC→DAgger 蒸馏出可部署 student、G1 零样本落地，把"上下身解耦"从接口图落成可跑代码，BSD-3-Clause 已开源；VR 遥操作与上层 HTD 策略仍待发布）
- **[`roadmap/depth-rl-locomotion.md`](roadmap/depth-rl-locomotion.md)：** Stage 2 RL+Locomotion 核心方法补 [RSL-RL](wiki/entities/rsl-rl.md)（ETH RSL × NVIDIA；legged_gym / Isaac Lab / mjlab 默认后端那一份 GPU PPO 与蒸馏实现，PPO 与 DistillationRunner 共用同一套 runner 抽象，是该 Stage 训练工作流实际依赖的基础组件；可选 BF16 autocast 单次 update 2.39×、显存 −33%，BSD-3-Clause 已开源）
- **[`roadmap/depth-sim2real.md`](roadmap/depth-sim2real.md)：** Stage 1 资产与执行器对齐补 [人形整机闭环惯量标定](wiki/concepts/humanoid-closed-loop-inertia-calibration.md) 与 [PRIME](wiki/entities/prime-system-id.md)（RSS 2026；单关节台架量的是空关节，覆盖不了整机分布式质量与终身漂移，运动学/惯量/IMU 零偏/足底力四张单子要闭环并绑机身序列号，PRIME 给出可微 Anitescu 接触 + Crocoddyl 的 MAP 联合辨识实现，G1/Go2 已验证，BSD-3-Clause 已开源）；Stage 4 部署工程闭环补 [Software-in-the-Loop](wiki/concepts/software-in-the-loop.md) 与 [Hardware-in-the-Loop](wiki/concepts/hardware-in-the-loop.md)（把既有「处理器在环」补成 SIL → PIL → HIL 完整在环阶梯，含 OPAL-RT 与 RHILS 一手工程口径及与 Hybrid Imitation Learning 的同缩写消歧）
- **[`roadmap/depth-humanoid-hardware-design.md`](roadmap/depth-humanoid-hardware-design.md)：** Stage 2 结构详设与验证补 [机器人结构模态分析](wiki/concepts/robot-structural-modal-analysis.md)（固有频率/振型/阻尼比三件套；串联悬臂低阶模态常落在 10 Hz 量级且随姿态漂移，比静强度更早卡死力控带宽，试验 FRF 与仿真 MAC 对账）——正对该 Stage 三张判据表里此前无专页支撑的「模态」一张；Stage 6 整机集成与验收补 [人形机器人量产工程能力](wiki/concepts/humanoid-mass-production-engineering.md)（三大核心件工艺定型、良率 S 曲线与 CPK 门槛、供应链一致性与 ISO/IEC 可靠性体系、汽车 PPAP 与 3C MES 跨行业迁移）
- **口径：** 每条路线仅在既有 Stage 的「推荐读什么」列表内追加 bullet，未新建 Stage、未改动其余既有内容；按本轮维护要求跳过 `make lint` / `ci-preflight` / `export`（纯 Markdown 内容改动），已脚本核对 `roadmap/*.md` 全量 `../` 相对链接 **0 断链**，9 条新增 bullet 的目标页面全部命中

## [2026-09-06] ingest | sources/papers/ahoy_arxiv_2603_17975.md — AHOY（ECCV 2026）遮挡 YouTube 视频可动画 3DGS 数字人；项目页未开源；关键页 paper-ahoy；交叉 LUNA / 4DAnyone / humanoid-training-data-pipeline
