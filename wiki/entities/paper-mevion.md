...
论文以一组重载/高速与柔性物体任务展示系统能力，均为定性、定性演示（原文未给统一量化成功率）：
...
**MEVION 的取舍很直白：用约 14,000 美元和四条闭链臂，把开源双臂采集从「桌面级」推到「重载高速」这一档，代价是它仍然是需要机电集成能力的研究原型。**

- 真正扩边界的是硬件：单臂 7.0 kg、最大 60 Nm、闭链肘部降低远端质量，才撑得起 3.6 kg 哑铃、炒锅翻炒、开瓶这类任务。
- 数据侧闭环成立：采集的双臂示范能直接训练 ACT 策略并回放，说明这套硬件产出的是可支撑模仿学习的数据，而不只是演示视频。
- 软件栈的价值在切换：同一 Python 上层跨 MuJoCo 与 ROS/CAN 实机，限位、碰撞与重力补偿可先在仿真验证再上实机。
- 主要局限是「成本不等于交钥匙」：14,000 美元不含焊接装配、电气安全、相机、工装与维护；论文也未给统一量化成功率，任务集属定性验证。
- 与 [ALOHA](./aloha.md) 的差异在力速工作区而非算法：ALOHA 是低成本双臂采集基线，MEVION 是它的重载高速扩展。
