...
**HiWET** 收录于 [具身智能研究室 · 人形 Loco-Manip 161 篇长文](https://mp.weixin.qq.com/s/pACh9EhsISiyPGdiiR0C3A) **第 041/161** 篇，归类为 **02 上半身中心控制与移动操作接口**。
...
- HiWET 把本体状态与关节序列、仿真交互数据、接触力/触觉信号转成可跟踪的身体目标，并通过PPO/RL 策略训练、VLM 语义规划/路由、分层技能/专家策略训练或组合全身策略，最终输出全身轨迹/动作序列、末端执行器/腕手目标、低层控制器目标。关键点是把任务拆成可路由的技能或专家策略，再用高层模块在执行中选择和组合。
- 人形 Loco-Manip 161 篇 **#041/161** · 上半身中心控制与移动操作接口。
...
**HiWET 的关键选择是把跟踪目标提到 world frame 的末端执行器上：长时程 loco-manipulation 因此可以由高层重新规划末端目标，而不必让底层策略背下整段全身轨迹。**

- 真正起作用的是 **分层 + 世界系末端目标**：VLM 负责语义规划/路由，底层把全身轨迹、末端执行器/腕手目标与低层控制器目标交给 RL 训练的策略执行。
- 输入侧带 **接触力/触觉信号**，是「上半身中心控制与移动操作接口」这一类的典型配置。
- 适用边界是长时程任务；按本页常见误区，分层接口本身不解决 **底层 WBC 鲁棒性**，仍须与运控/接触控制类工作对照。
- 本页为 161 篇清单索引条目，且「论文/项目」一栏标注（见原文）——量化指标与复现细节须回到原文 PDF。
...
- 原始 source：[loco_manip_161_survey_041_hiwet.md](../../sources/papers/loco_manip_161_survey_041_hiwet.md)

## 参考来源

- [loco_manip_161_survey_041_hiwet.md](../../sources/papers/loco_manip_161_survey_041_hiwet.md) — 161 篇策展摘录
