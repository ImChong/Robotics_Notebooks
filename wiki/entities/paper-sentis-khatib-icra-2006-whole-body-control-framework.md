...
**本文不提供可复现的量化评测。** 它是框架论文：证据形态是 **仿真与平台演示** 加上「与 Honda 合作向 ASIMO 实现」的工程叙述，既没有公开基准，也没有随论文发布的数据或代码——ASIMO 实现细节始终未公开。把文内任何性能表述当成可引用的数字会失真。

相对 [IJHR 2004](./paper-khatib-sentis-ijhr-2004-whole-body-dynamic-behavior.md)，本文多出来的可验收项是 **工程层面** 的，验证时应分开测：

| 验收项 | 怎么测 | 通过判据 |
|--------|--------|----------|
| 运动学层可行性监测 | 下发一个在当前支撑/限位下不可行的行为 | 运行时 **提前判不可行** 并降级，而不是进了动力学求解才发散 |
| 阻抗/柔顺交互 | 在跟踪中施加外部接触力 | 末端按设定阻抗顺从，姿态软化；不是刚性轨迹硬顶 |
| 支撑接触切换 | 双脚 → 单脚 → 双脚 | 切换瞬间任务力矩连续，平衡约束不被穿越 |
| 多级操作空间合成 | 同时给运动任务与力任务 | 两者在同一动力学下分解，而非互相抢控制权 |

**读数提醒：** 本文、IJHR 2004 与 [OSF 1987](./paper-operational-space-formulation.md) 之间没有共同基准，不存在可横比的量化指标；要量化，应在 [ControlIt!](./controlit.md) / TSID 等开源栈上用同一机器人与同一任务集重测。与学习型 WBC 比较时更要小心——评测协议根本不同，见 [wbc-vs-rl](../comparisons/wbc-vs-rl.md)。
