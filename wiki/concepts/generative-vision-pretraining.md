...
summary: "生成式视觉预训练指以图像/视频生成为目标训练出的模型，其内部表征可经轻量对齐解锁分割、深度、法线等理解任务；Vision Banana（图像）与 GenCeption（视频 feed-forward）论证其可类比 LLM 预训练，成为 Foundation Vision Model 的中心范式。"
---
...
- **范式对照 NLP：** [Vision Banana](../entities/vision-banana.md) 将 **图像生成训练** 类比 **LLM pretraining**，**instruction-tuning** 类比 Chat 对齐；[GenCeption](../entities/genception.md) 进一步证明 **text-to-video 扩散预训练** 在 **原生视频域** 经 **feed-forward 后训练** 可统一 **深度/法线/位姿/分割/3D 关键点**——若成立，CV 的主干将从「判别预训练 + 任务头」转向 **「生成基座 + prompt 接口」**。
...
- Wang et al., *Video Generation Models are General-Purpose Vision Learners*（GenCeption）, [arXiv:2607.09024](https://arxiv.org/abs/2607.09024)
