...
**SWAP 的主张是把左右镜像对称写进计算图、而不是喂进数据：等变约束压缩了 RSSM 潜空间，才让极限跑酷的探索不坍缩到不对称次优解。**

- 起作用的是 **网络拓扑级硬等变**，不是增广或软惩罚——SymLoss 仅边际改善，纯数据增广基线论文直接不纳入。
- 必须 **世界模型与 Actor 同时约束**：box 攀台上 SWAP w/o Eq-Policy 仍次优，双侧协调的收益来自策略侧等变而非只靠潜空间。
- 泛化证据是可证伪的：单侧倾斜地形训练 → 水平镜像测试，无等变基线深度重建误差陡升并崩溃，全框架仅轻微上升，说明潜空间编码的是对称结构而非外观记忆。
- 成本可控（Isaac Gym 6000 环境、RTX 4090 ~10 h 端到端），真机 2.13 m 远跳 / 1.63 m 攀台建立在 72 kg 的 Apollo 大机体上，跨机体直接比绝对数值需谨慎。
- 边界在 **结构化 box/gap 跑酷课**：论文自承离散踏脚石等非结构化场景尚未充分评测，代码开源状态以项目页为准。
- 与 [Extreme Parkour](./extreme-parkour.md) 的分野是有无多步 latent 想象；与 [MUJICA](./paper-mujica-wheel-legged-multi-skill.md) 的高台对照要先看机体与感知栈差异（轮足纯本体 vs 四足 + 深度 WM）。
