...
- **vs BD 原生 AutoWalk：** AutoWalk 面向 **结构化工业巡检**（预录路径、可控环境）；NeBula 路线面向 **未知极端环境探索** 与 **多机器人信念共享**，属研究型自主栈而非出厂产品功能。
- **vs 同平台低层 RL：** 本页侧重 **任务级探索自主**；同一 Spot 平台的 **端到端低层 RL locomotion** 见 [Spot RL 分布距离 Sim2Real](./paper-spot-rl-distributional-sim2real.md)，两者在栈层级上互补。
- **vs 经典力控轨迹优化：** 低层执行器与在线轨迹优化细节见 [BD 足式控制专利栈](./patent-boston-dynamics-legged-control-stack.md)；本文不展开该层，聚焦上层自主。
