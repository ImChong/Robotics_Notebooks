...
summary: "自变量机器人 开源项目 WALL-WM（世界模型）：联合建模场景视频、机器人状态与动作，预测执行后的环境变化，为动作选择和策略训练提供世界表征；仓库公开模型结构与训练评测入口。…"
institutions:
  - x-square-robot
---
...
**WALL-WM** 是 [自变量机器人](https://github.com/X-Square-Robot) 公开的 **世界模型** 开源项目：联合建模场景视频、机器人状态与动作，预测执行后的环境变化，为动作选择和策略训练提供世界表征；仓库公开模型结构与训练评测入口。
