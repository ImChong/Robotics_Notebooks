...
**LaPA 的关键取舍是「用离散潜在动作换掉动作标签」：把无标签互联网视频变成可用的预训练语料，机器人数据只承担少量微调，就能超过依赖完整标签的 SOTA VLA。**

- 真正起作用的机制是潜在动作这层中介——先从无动作标签视频学离散潜在动作，再用少量机器人数据对齐真实控制，数据瓶颈从「标注」移到「视频规模」。
- 架构上属 **01 级联**：先预测未来视觉/潜特征，再由独立或轻量动作头解码控制指令；好处是模块易拆易复用，代价是误差会沿级联各段传递。
- 使用边界：本页为策展索引级，ICLR 2025 的量化增益以原文为准；开源 WM 项目的引用量与控制一致性/下游增益无简单线性关系，复现前需核对 License 与权重。
