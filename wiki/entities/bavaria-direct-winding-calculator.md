...
对照教科书：槽口所张机械角 γ = b_s/(D/2)，第 n 阶的展宽因子 `sin(nγ/2)/(nγ/2)` ≡ 源码式；斜槽机械角 = sk·2π/Q，`sin(θ/2)/(θ/2)` ≡ 源码式。两式逐位吻合（已逐式数值核验）。
...
- **站点已下线，无上游仓库**：`bavaria-direct.co.za` 已停止解析，工具从未进过 GitHub，只靠站点与论坛离线包流传。要长期用就得自己存档——这也是本页把它完整记下来的原因。
