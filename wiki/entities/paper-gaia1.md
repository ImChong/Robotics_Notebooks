...
**约 90 亿参数的驾驶世界模型：把历史视频、文本指令与自车动作编成离散 token，像语言模型一样自回归预测未来视频帧，为无风险仿真与想象训练提供场地。**

1. **Token 世界模型** — 过去视频 + 文本指令 + 自车动作 → 离散 Token，自回归预测未来帧 Token 并解码为视频。
2. **涌现场景逻辑** — 在下一帧预测目标下涌现运动学、3D 几何与红绿灯等交通逻辑，支持动作条件未来帧与天气/场景可控生成。
3. **形态与定量** — Technical Report + 视频演示形态，侧重生成逼真度与可控性；定量表以原文图表为准，外部难复现。
4. **相对后继** — 相比 M⁴World（多摄 + LiDAR、物体级控制）与 X-World（车载多摄），GAIA-1 条件接口更简，但同样未开源。
5. **局限** — 像素级仿真不等于物理保真，长视界漂移与动作可控性待后续（M⁴World / X-World）推进；闭源大模型，与规划器如何闭环仍是系统工程问题。
