# Keep on Going: Learning Robust Humanoid Motion Skills via Selective Adversarial Training

> 来源归档（ingest · Robot Learning Paper Notebooks progress 待深读）

- **标题：** Keep on Going: Learning Robust Humanoid Motion Skills via Selective Adversarial Training
- **类型：** paper
- **深读状态：** 待撰写（见 [progress.json](https://github.com/ImChong/Robot_Learning_Paper_Notebooks/blob/main/progress.json)）
- **计划笔记路径：** `papers/04_Loco-Manipulation_and_WBC/Keep_on_Going__Learning_Robust_Humanoid_Motion_Skills_via_Selective_Adversarial_/Keep_on_Going__Learning_Robust_Humanoid_Motion_Skills_via_Selective_Adversarial_.md`
- **分类：** 04_Loco-Manipulation_and_WBC
- **路线：** Loco-Manipulation
- **入库日期：** 2026-06-11
- **一句话说明：** 列入 Paper Notebooks 阅读进度，深读笔记尚未完成；本文件为 **进度 → wiki** 溯源锚点。

## 核心摘录（策展，非全文）

- 本文件锚定 **待深读** 论文在姊妹仓库 `progress.json` 中的条目；笔记完成后应改用笔记页链接并深化 wiki 归纳。
- 知识归纳见 wiki 实体页：[paper-notebook-keep-on-going](../../wiki/entities/paper-notebook-keep-on-going.md).

## 对 wiki 的映射

- [paper-notebook-keep-on-going](../../wiki/entities/paper-notebook-keep-on-going.md)
- 分类父节点：[paper-notebook-category-04-loco-manipulation-and-wbc](../../wiki/overview/paper-notebook-category-04-loco-manipulation-and-wbc.md)

## 参考来源（原始）

- [Robot Learning Paper Notebooks · progress.json](https://github.com/ImChong/Robot_Learning_Paper_Notebooks/blob/main/progress.json)

