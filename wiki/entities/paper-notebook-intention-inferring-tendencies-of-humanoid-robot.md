...
**INTENTION 的赌注是「用直觉替代模型」：把精确物理模型加预定义动作序列，换成 VLM 语义推理 + 记忆图经验 + 可供性感知，泛化性来自经验类比而不是建模精度。**

- 三个部件分工明确：Grounded VLM 把语义输出对齐到具体物体与可执行行为，Intuitive Perceptor 从图像抽物理关系与可供性，Memory Graph 把历史交互沉淀成可复用经验。
