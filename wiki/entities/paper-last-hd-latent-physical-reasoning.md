...
**LaST-HD 的判断是：人手数据的价值不在运动学轨迹，而在「动作会导致什么物理后果」——所以跨具身对齐被下沉到动作条件世界模型的潜空间，而不是停在关键点重定向。**

- 真正起作用的是潜式物理监督：取动作条件 WM 最深 U-Net 去噪步的前向动力学特征作对齐目标监督推理专家，去掉它域内 SR 从 **73%** 回落到 **60%**；SigLIP 未来帧与无动作条件 WM 两种替代都更弱。
- 对齐**无需轨迹配对**、动作标签只作弱锚点，这是它能吃下规模化非配对人手数据的前提；而 OOL Glove 保证人手侧仍带精确手–腕动作标签，OOL **73%** > 裸手视觉 **63%** > 同时长机器人数据 **60%**。
- 部署适应给出可操作配方而非只有指标：实机 rollout 找失败态 → OOL 人手纠偏 → 平衡回放，Sort Fruits 上 **60 条（约 20 分钟）** 把未见物体/背景推到 **100%** SR。
- 边界与成本：仍依赖目标本体的部分机器人数据做域内共训，WM + MoT 骨干的训练与部署栈较重，且绑定特定采集/机器人平台，复现门槛偏高。
- 与 [EgoMimic](./paper-ego-03-egomimic.md) 的分野在监督层级：后者做动作/视觉级共训，LaST-HD 把跨具身接口换成动作条件前向动力学潜空间，这也是它相对 π₀.₅、Cosmos-Policy 在泛化场景拉开差距的解释。
