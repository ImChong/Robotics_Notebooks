...
主路线偏"先打通主干"，下列二十二条纵深路线**各自独立**，从主路线某个阶段衔接出去，可单独阅读。
...
- [如果目标是具身模型测评（认知 → 世界模型保真 → 策略成功率 → sim↔real 校准）](depth-embodied-eval.md) —— 想证明/证伪一个具身模型到底好不好（起点：RLBench 标准化视觉操作评测套件，2019）
