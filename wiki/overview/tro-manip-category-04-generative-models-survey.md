---
type: overview
tags: [manipulation, category-hub, survey, generative-models, diffusion-policy, imitation-learning]
status: complete
updated: 2026-09-15
summary: "T-RO 5 篇精选 · 04 生成模型综述（1 篇）— 深度生成模型在多模态机器人演示学习中的模型类型、应用与 OOD 设计。"
related:
  - ./tro-manip-5-papers-technology-map.md
  - ./tro-manip-category-03-video-pretraining.md
  - ../entities/paper-tro-manip-05-dgm-robot-learning-survey.md
  - ../methods/diffusion-policy.md
  - ../methods/behavior-cloning.md
sources:
  - ../../sources/blogs/wechat_shenlan_tro_manip_5_papers_survey.md
  - ../../sources/papers/tro_manip_5_papers_catalog.md
---

# T-RO 分类 04：生成模型综述

> **图谱分类节点**：对应 T-RO 2026 操作学习精选的 **04 生成模型综述** 分组（1 篇）。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| DGM | Deep Generative Model | 深度生成模型 |
| EBM | Energy-Based Model | 能量基模型 |
| GAN | Generative Adversarial Network | 生成对抗网络 |
| OOD | Out-of-Distribution | 分布外泛化 |

## 核心问题

**生成模型在 LfD 里怎么选型？** 须同时看 **模型族**（EBM/扩散/GAN/流匹配）、**任务形态**（抓取/轨迹/代价）与 **OOD 泛化设计**。

## 本组论文（1 篇）

| # | 工作 | Wiki 实体 | Source |
|---|------|-----------|--------|
| 05 | DGM Robot Learning Survey | [paper-tro-manip-05-dgm-robot-learning-survey.md](../entities/paper-tro-manip-05-dgm-robot-learning-survey.md) | [source](../../sources/papers/tro_manip_survey_05_dgm_survey.md) |

## 关联页面

- [T-RO 5 篇技术地图](./tro-manip-5-papers-technology-map.md)
- [Diffusion Policy](../methods/diffusion-policy.md)
- [Diffusion Model](../concepts/diffusion-model.md)

## 参考来源

- [wechat_shenlan_tro_manip_5_papers_survey.md](../../sources/blogs/wechat_shenlan_tro_manip_5_papers_survey.md)
- [tro_manip_survey_05_dgm_survey.md](../../sources/papers/tro_manip_survey_05_dgm_survey.md)

## 推荐继续阅读

- [操作 VLA 与视频-动作架构选型](../queries/manipulation-vla-architecture-selection.md)
