...
**OmniDP 的切口在感知范围而不是策略结构：用全向 3D 感知把超出 FOV 的大工作空间纳入观测，再把策略网络与控制模块放进同一条训练/部署链路，减少高层目标到低层动作之间的断点。**

- 起作用的是数据闭环加一体化链路：遥操作/外骨骼采集人类操作与机器人状态，相机图像/多视角观测与本体状态一起进策略，输出直接是可执行动作命令。
- 定位在 **03 视觉感知驱动的人形移动操作**，它回答的是「看不到的地方怎么操作」，不是「怎么把动作做得更精细」。
- 适用边界：本页为索引级策展摘录，且核心信息表中机构与论文/项目链接均记为「见原文」，外部核验须直接回到原始 source 与 161 篇总表。
