...
**Ctrl-World 的主用途是「多视角 VLA 想象闭环」：评估排序 + 合成成功轨迹做 SFT，低层接触物理仍弱。**

1. **对齐现代 VLA 输入** — 联合预测第三人称与腕部视角；腕部减接触幻觉，可直接接 π₀ / π₀.₅ 等多相机策略。
2. **两件控制精度件** — 帧级动作条件（笛卡尔位姿 cross-attn）+ 位姿条件记忆检索；消融去记忆/去帧级条件均变差；厘米级动作差异可区分 rollout。
3. **策略改进数字** — 扰动指令或重置初态 → 人偏好筛选 → SFT，把 π₀.₅ 未见指令/物体成功率 **38.7% → 83.4%**（约 **+44.7 pt**）。
4. **评估可读性** — 指令跟随排名与真机相关，但低层接触成功常被低估——想象成功率不宜直接当真机执行成功率。
5. **主增益在新指令跟随** — 作者强调不宜期待同一流程大幅拉高低层已见任务；失败「反复重试」等分布仍盖不全。
6. **工程入口** — SVD 1.5B 初始化、DROID 训练；历史 7 帧、未来约 15 步（≈1 s）、192×320；代码与权重已开源。
...
- [CLAP](./paper-clap-cross-embodiment.md) — 同 SVD 骨干的跨本体课程 WM；开源 G1/YAM 适配权重（arXiv:2608.27406）
...
- [WALL-SS](./paper-wall-ss.md) — next-scale AR 长程对照：虚实成功率校准，训练代码待发布
