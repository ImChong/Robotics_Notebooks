...
- [~] 第 2 批，#9（详情端 2026-09-09 完成）：`detail.html` / `roadmap.html` 各去掉 6 行 KaTeX + Mermaid 静态外链（实测合计 1,082,125 响应字节 / 3,648,639 原始字节），改由 `ensureKatexLoaded()` / `ensureMermaidLoaded()` 按正文是否含定界符或 `.mermaid` 节点注入，`integrity`/`crossorigin` 不变，失败静默降级可重试。3,925 页中 2,184 页（55.6%）两者都不需要。5 项新行为测试（前端 50/50）。**完整图谱 2D 初始不加载 3D 库仍待办**；首页 3D 懒加载与 WebGL 失败恢复不变。
