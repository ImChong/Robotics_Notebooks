...
**MetaWorld 的价值不在某个模块，而在于给出一种「可学习的模块化」样本：三层各自学习，但层间接口保持清楚。**

- 机制就是这条分层：**语义层** 用 GPT-4o / VLM 解析高层指令并输出技能权重，**技能迁移层** 做动态专家选择，**物理层** 用 TD-MPC2 加专家动作引导完成控制。
- 最硬的边界是 **没有真实机器人实验**，仅在 HumanoidBench 中验证 — 不能据此推断真实接触与硬件约束下的表现。
- 它在系统栈里的位置是 **04 视觉闭环 · 任务接口 · 世界模型**，代表与 SENTINEL 不同的一条路线，可作为「端到端 vs 模块化」之外的第三种参照。
- 需要警惕的是层级错位：世界模型 / 任务接口解决的是 **接口与预测**，不自动替代已封装的底层 WBC 能力。
