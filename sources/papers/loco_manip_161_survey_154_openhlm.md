# OpenHLM

> 来源归档（ingest · 人形 Loco-Manip 161 篇长文 第 154/161）

- **标题：** OpenHLM: An Empirical Recipe for Whole-Body Humanoid Loco-Manipulation
- **类型：** paper
- **Loco-Manip 161 分类：** 09 人形 VLA、世界模型与通用操作
- **机构：** Tsinghua University、Shanghai Qi Zhi Institute、Spirit AI
- **项目页：** https://openhlm-project.github.io/
- **arXiv：** https://arxiv.org/abs/2606.22174
- **代码：** https://github.com/OpenHLM-project/OpenHLM（**已开源**，2026-07-22 核查）
- **发表日期：** 2026年6月20日
- **入库日期：** 2026-06-26
- **再核日期：** 2026-07-22
- **一句话说明：** OpenHLM 用三阶段经验路线图构建全身原生人形 VLA（关节全身遥操作 → π₀.₅ 系 VLA 适配 → HuMI/stationary 异构共训）；长程果盘任务以更少演示时长超过 GR00T N1.6 / Ψ₀。

## 核心摘录（策展，非全文）

- **在 161 篇地图中的位置：** 09 人形 VLA、世界模型与通用操作，编号 **154/161**。
- **算法实现总结（公众号，历史）：** 语言+视觉+本体 → VLA/流匹配动作头 → 低层跟踪；保留 VLM 语义同时接机器人状态与动作头。
- **深度摘录：** 以 [openhlm_arxiv_2606_22174.md](./openhlm_arxiv_2606_22174.md) 与项目页为准（含开源仓、数据与消融数字）。

## 对 wiki 的映射

- [paper-loco-manip-161-154-openhlm](../../wiki/entities/paper-loco-manip-161-154-openhlm.md)
- [loco-manip-161-category-09-vla-world-models](../../wiki/overview/loco-manip-161-category-09-vla-world-models.md)
- [openhlm_arxiv_2606_22174.md](./openhlm_arxiv_2606_22174.md) · [openhlm-project-github-io.md](../sites/openhlm-project-github-io.md) · [openhlm.md](../repos/openhlm.md)

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_loco_manip_161_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_loco_manip_161_survey.md)
- 原始抓取：[wechat_humanoid_loco_manip_161_2026-06-26.md](../raw/wechat_humanoid_loco_manip_161_2026-06-26.md)

- 官方项目页 / arXiv / GitHub（2026-07-22 ingest）：代码已开源 <https://github.com/OpenHLM-project/OpenHLM>
- 接触专题交叉：[wechat_embodied_ai_lab_loco_manip_contact_survey.md](../blogs/wechat_embodied_ai_lab_loco_manip_contact_survey.md)

