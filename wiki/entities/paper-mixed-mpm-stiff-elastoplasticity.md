...
| **耦合对照** | one-way（地形仅作碰撞几何）vs **two-way**（颗粒推回关节角色） |
...
| 路线 | 擅长材料 | 刚体耦合 |
|------|----------|----------|
| **Mixed MPM** | 沙 / 雪 / 近不可压流体 / **刚性弹塑性** | Newton 内 **双向** |
| 经典显式 MPM | 软颗粒、流体 | 多为 one-way |
| [Genesis](./genesis-world-10.md) 等第三方 MPM | 覆盖面接近 | 依各自引擎 |
| FEM 软体 | 连续弹性体 | 视实现 |
| [DAT](./paper-dat-divide-and-truncate.md) | 不解材料，只解接触 | 服务所有后端 |
| 高度图 / 解析地形模型 | 只是几何 | 无反作用 |

逐条读法：

- **经典显式 MPM** — 刚性与近不可压材料上步长被 CFL 卡死或 stencil 成本爆炸，这正是「混合速度–应力离散」要解的。
- **Genesis 等第三方 MPM** — 能力上有重叠；Newton 侧的差异在 **与 MuJoCo Warp / [Kamino](./paper-kamino.md) 同栈双向耦合**，不必跨引擎搬状态。
- **FEM 软体** — 拓扑不变时更省；一旦发生 **大流动、断裂、颗粒化**，网格就跟不上了。
- **DAT** — **不是竞品，是搭档**：MPM 负责颗粒本构，DAT 负责它与刚体 / 壳相遇时的无穿透几何。
- **高度图 / 解析地形** — 算力便宜得多；只有当 **颗粒反作用力矩会改变步态** 时才值得上 MPM。
