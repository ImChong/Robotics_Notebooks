...
**Gallant 的落点是「感知—局部导航—运控不分家」：把体素/高度图一类的三维地形表征与策略网络、控制模块放进同一条训练与部署链路，减少高层目标到低层动作之间的断点。**

- 起作用的机制链是：相机图像/多视角观测、本体状态与关节序列、深度/点云/高度图先恢复地形/场景表征，再由策略网络与控制模块直接产出导航/到达目标。
- 适用边界由题目本身限定——3D 受限地形上的人形行走与局部导航，归 **08 硬件平台、感知配置与部署扩展**，不覆盖操作任务，也不自动解决底层 WBC 鲁棒性。
- 索引级局限：本页未搬运原文量化 benchmark 与实机指标，数值以 [原文 / 项目页](https://gallantloco.github.io/) 为准。
