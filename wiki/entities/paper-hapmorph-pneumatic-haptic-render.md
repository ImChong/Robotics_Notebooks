...
**HapMorph 解的是气动触觉里「尺寸与刚度绑死」的老问题：用拮抗双腔把两个渲染维度拆开，换来的是 21 g 可穿戴形态，代价是仍拖着外部气源与滞回。**

- 机制核心在 **拮抗**：两组织物 pouch motor 经交叉拉力带耦合，$P_1$ / $P_2$ 的压力组合映射到 $(H_2, k)$ 平面上的可辨工作点，实验上同时做出「变尺寸而刚度近似恒定」（三档 $H_2$ 下维持约 0.1±0.015 N/mm）与「固定尺寸下多档刚度」。
- 指标落在可穿戴约束内：**21 g**、尺寸 **50–104 mm**、峰值刚度约 **4.7 N/mm**、阶跃约 **2 s**；10 人盲测 9 状态识别 **89.4%**，混淆矩阵显示 **刚度比尺寸更难辨**。
- 主要局限是形态与精度：仍依赖 **外部气源与阀岛**，需微型泵与高效阀才能电池供电移动使用；织物–腔体摩擦与弹性带变形带来 **滞回**，精密应用需闭环补偿。
- 覆盖面边界明确：只渲染尺寸 + 刚度（及扩展的截面形状），**不含纹理与温度**。
- 定位是 **操作员端而非机器人端**：它是力反馈显示设备，与 [Tactile Sensing](../concepts/tactile-sensing.md) 的机器人接触感知正交，也不同于 [数据手套 vs 视觉遥操作](../comparisons/data-gloves-vs-vision-teleop.md) 讨论的手部运动采集通道。
