---
type: overview
tags: [depth, depth-contact-force-control, contact, force-control, impedance, admittance, wrench, compliance, hybrid, manipulation]
status: complete
updated: 2026-07-31
summary: "接触力控纵深汇总：从接触感知/估计 → 力旋量表示 → 阻抗/导纳/混合力位控制 → 接触丰富操作策略四层力控闭环的统一入口，串起各层对操作稳定性/安全性的贡献与带宽/刚度/时延取舍，收纳分散的接触估计/力控/力旋量概念页。"
---

# 接触力控（纵深汇总）

> **图谱纵深视图**：本页是知识图谱「🤝 接触力控 (Contact Force Control)」纵深的统一入口；在 [图谱纵深视图](../../docs/graph.html?depth=contact-force-control) 筛选时，本节点为汇总锚点。

## 一句话定义

**接触力控纵深** 关注「机器人如何在接触中稳住力」这条**纵向闭环**：从**接触感知/估计**，经**力旋量表示**与**阻抗/导纳/混合力位控制**，到**接触丰富操作策略**，逐层分析每层如何贡献操作稳定性、各自的带宽/刚度/时延成本与典型失败模式。
...
- **[触觉 (Tactile)](./depth-tactile.md)**：触觉侧重「摸得着」的感知模态，本纵深侧重「稳得住」的控制闭环，二者在感知层交叠、在控制层互补。
- **[物理保真度 (Physics Fidelity)](./depth-physics-fidelity.md)**：接触/摩擦建模是仿真侧的「建准」，本纵深是真机侧的「控稳」，共同决定接触任务能否 sim2real。
- **[抓取 (Grasp)](./depth-grasp.md)**：稳定抓取与灵巧操作依赖力旋量闭环兜底。
...
- 图谱纵深定义：[docs/depth-filters.js](../../docs/depth-filters.js)（`contact-force-control` 命中规则）
