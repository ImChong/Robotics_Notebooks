# Learning Human-Like Badminton Skills for Humanoid Robots

> 来源归档（ingest · Robot Learning Paper Notebooks 深读笔记）

- **标题：** Learning Human-Like Badminton Skills for Humanoid Robots
- **类型：** paper
- **笔记链接：** <https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/04_Loco-Manipulation_and_WBC/Learning_Human-Like_Badminton_Skills_for_Humanoid_Robots/Learning_Human-Like_Badminton_Skills_for_Humanoid_Robots.html>
- **分类：** 04_Loco-Manipulation_and_WBC
- **arXiv：** <https://arxiv.org/abs/2602.08370>
- **项目页：** <https://astrorix.github.io/LHBS/>
- **入库日期：** 2026-06-07（Paper Notebooks 策展）；**2026-06-25** 追加 arXiv 深读归档
- **一句话说明：** 提出 Imitation-to-Interaction 渐进式强化学习框架，将人形机器人的羽毛球学习拆解为四阶段：运动先验建立 → 目标条件蒸馏 → 对抗先验稳定 → 交互流形扩展，实现首个零样本迁移到真实机器人的拟人羽毛球技能。

## 核心摘录（策展，非全文）

- 本文件为 **Paper Notebooks → 本库 wiki** 的溯源锚点；**一手论文深读** 见 [lhbs_learning_human_like_badminton_skills_arxiv_2602_08370.md](lhbs_learning_human_like_badminton_skills_arxiv_2602_08370.md)。
- 知识归纳见 wiki 实体页：[paper-notebook-learning-human-like-badminton-skills-for-humanoi](../../wiki/entities/paper-notebook-learning-human-like-badminton-skills-for-humanoi.md).

## 对 wiki 的映射

- [paper-notebook-learning-human-like-badminton-skills-for-humanoi](../../wiki/entities/paper-notebook-learning-human-like-badminton-skills-for-humanoi.md)
- 分类父节点：[paper-notebook-category-04-loco-manipulation-and-wbc](../../wiki/overview/paper-notebook-category-04-loco-manipulation-and-wbc.md)

## 参考来源（原始）

- arXiv 深读：[lhbs_learning_human_like_badminton_skills_arxiv_2602_08370.md](lhbs_learning_human_like_badminton_skills_arxiv_2602_08370.md)
- 深读笔记：<https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/04_Loco-Manipulation_and_WBC/Learning_Human-Like_Badminton_Skills_for_Humanoid_Robots/Learning_Human-Like_Badminton_Skills_for_Humanoid_Robots.html>
- 项目页：<https://astrorix.github.io/LHBS/>
- 论文：<https://arxiv.org/abs/2602.08370>
