...
**H2O 把「人类动作 → 机器人身体」从离线重定向做成了实时全身遥操作系统：关键不在 pose retargeting 本身，而在把数据、仿真与真机控制接成一条闭环。**

- 真正起作用的是 **可行性过滤**：AMASS → SMPL → Unitree H1 重定向后，用 privileged motion imitator 剔除机器人动力学上不可执行的动作，形成 **feasible motion dataset**，再训练只依赖部署可得状态的 imitator。
- 目标设定是 **全身而非局部**：驱动整个人形身体做动作，而不是只控制手臂或轮式底盘——这也是它比一般遥操作更难的原因。
- 在同栈里的位置很清楚：HumanX / HDMI 解决「从人类视频生产可训练交互技能」，H2O 更进一步把这条转换做成 **实时系统**。
- 边界：重定向 / 遥操作不是训练前脚本，参考质量上限往往早于 RL 策略决定；本页为策展编译，量化 benchmark 与实机指标以原文 / 项目页为准。
