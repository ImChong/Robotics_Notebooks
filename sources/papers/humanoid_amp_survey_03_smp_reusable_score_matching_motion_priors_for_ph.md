# SMP: Reusable Score-Matching Motion Priors for Physics-Based Character Control

> 来源归档（ingest · 人形 AMP 运动先验 19 篇 · 第 03/19）

- **标题：** SMP: Reusable Score-Matching Motion Priors for Physics-Based Character Control
- **类型：** paper
- **AMP 叙事段：** 01 分布约束与先验组件化
- **机构：** Simon Fraser University、Sony Interactive Entertainment、Stanford、Snap、NVIDIA 等
- **论文链接：** <https://arxiv.org/abs/2512.03028>
- **项目页：** <https://yxmu.foo/smp-page/>
- **索引来源：** [具身智能研究室 · AMP 专题长文](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)（<https://mp.weixin.qq.com/s/YZsm3855iP3TNTTt1aou7w>）
- **原始抓取：** [wechat_humanoid_amp_19_survey_2026-05-26.md](../raw/wechat_humanoid_amp_19_survey_2026-05-26.md)（Agent Reach + Camoufox）
- **入库日期：** 2026-05-26
- **深读归档：** 2026-06-25 — [smp.md](smp.md)
- **一句话说明：** SMP 是我觉得非常值得放进这篇文章的一篇。

## 核心摘录（策展，非全文）

- **在 AMP 四段地图中的位置：** 01 分布约束与先验组件化，编号 **03/19**。
- **公众号导读要点：** SMP 是我觉得非常值得放进这篇文章的一篇。
- **方法要点（arXiv 编译）：** 冻结运动扩散 + SDS 奖励；ESM/GSI；100 风格组合先验；下游 RL 可丢弃 MoCap；G1 真机部署见论文 §11。

## 对 wiki 的映射

- [paper-amp-survey-03-smp](../../wiki/entities/paper-amp-survey-03-smp.md) — 主实体页（深读归纳）
- [smp](../../wiki/methods/smp.md)、[smp-g1-mjlab](../../wiki/entities/smp-g1-mjlab.md)

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)
- 姊妹篇 42 篇栈：[wechat_embodied_ai_lab_humanoid_rl_motion_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)
- arXiv 深读：[smp.md](smp.md)
