...
**这条目的地图坐标是「用潜在扩散生成物理一致的全身 loco-manip 轨迹」——本页只给到定位与方法族，「物理一致」具体如何被约束需回原文。**

- 归纳出的机制链是：相机图像/多视角观测、本体状态与关节序列、仿真交互数据先恢复场景/目标/运动表征，再由扩散策略/流匹配与 VLM 语义规划/路由生成全身轨迹/动作序列。
- 适用边界在 **04 生成式运动、语言控制与轨迹规划**：重心在轨迹生成侧，不解决底层 WBC 鲁棒性。
- 这条目的溯源成本高于同组其他页：「机构」与「论文/项目」两栏均为「（见原文）」，引用前需自行补齐出处，量化指标也未搬运。
