...
**SparseDrive 把动态参与者与静态车道全部表示为稀疏实例、前向不建稠密 BEV，使算力∝实例数而非感知面积，在 nuScenes 上安全指标对齐/优于稠密基线同时显著降延迟。**

1. **稀疏实例范式** — 驾驶元素（车辆/行人/车道等）→稀疏实例集合，全程无稠密 BEV；注意力只在关键实例间交互，抑制背景噪声。
2. **量产友好** — 计算量随实例数增长而非感知面积，直面车端部署算力墙。
3. **评测** — nuScenes 上安全性能与主流稠密模型相当或更优，同时显著提升推理速度（FPS）；对照稠密 BEV E2E 与 VAD 类向量化。
4. **开源** — `swc-17/SparseDrive` 已开源。
5. **边界** — 稀疏召回失败（漏检关键实例）会直接伤害规划；与稠密占用/开放词汇感知的结合仍需工程取舍；开环指标外推到闭环控制需额外验证。
