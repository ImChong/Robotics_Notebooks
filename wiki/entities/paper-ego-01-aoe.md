...
**AoE 押的是采集端而非模型端：用颈挂手机 + 端云协同把第一视角数据做成「人人可采」的系统，明知质量会下降，也要先把规模的门槛打开。**

- 起作用的是 **门槛设计** 而非设备精度：颈挂式手机支架、移动端应用、云端自动标注与过滤，背后的判断是具身模型继续 scaling 不能只依赖实验室机器人采集。
- 它赌的资源是 **人类日常操作本身**：做饭、整理、搬放、购物、清洁里已经含有视觉、接触与任务顺序，价值在于把这些原本没被记录的经验变成可整理、可筛选、可训练的数据源。
- 代价被明确承认：手机数据不如专业设备干净，**视角抖、手挡物体、动作不标准**；真正打开的是规模问题，不是质量问题。
- 边界：Ego 视频不天然等于机器人策略数据，仍须 **重建、对齐、重定向与物理过滤**；本页为策展编译，量化指标以原文 / 项目页为准。
