...
**这篇把电机 τ–ω 工作区当成训练期的一等约束：用规格书而不是额外真机数据，换来高速段的 sim2real 对齐，代价是方法与 HOUND 的具体传动强绑定。**

- 真正起作用的是 **训练期 MOR clip**：消融中去掉它，跑步机峰值从 **6.5 m/s** 掉到 **4.5 m/s**，且无 MOR 策略在 **5 m/s 实机摔倒**；对称步态奖（6.0）与轻量化足端（5.5）是次级增益。
- 收益口径不是「reward 更高」，而是 **高速段 gap 不再恶化**——有 MOR 训练后 **>3.5 m/s** 的 sim-to-real reward 差停止单调变差。
- 适用边界：需要 **电机 datasheet + 明确减速器映射**；并联 HFE/KFE 的 gearbox 矩阵说明关节空间矩形力矩限不能当电机 MOR 用，换平台必须重推。
- 主要局限：准静态 DC 模型忽略电感动态、**τ_peak** 只是推荐值；任务限于高速直线/跑步机，无感知地形与多步态切换。
- 工程状态：**未列 GitHub 或项目页**，RaiSim + 并发估计器栈不可直接复现；可迁移的其实只是「PD 输出后插入 关节→电机→clip→关节」这一薄层。
- 对照定位：相对 [Actuator Network](../methods/actuator-network.md) 的数据驱动拟合，本文走规格书路线；相对同平台 [APT-RL](./paper-apt-rl-agile-perceptive-quadruped-locomotion.md)，本篇解 **执行器对齐**，后者解 **感知多技能**。
