...
迭代式数据聚合：先用当前策略跑出轨迹，再让专家对**策略自己访问到的状态**补标注，把新数据并入训练集重训。换的不是模型而是数据收集闭环，因此能有效缓解行为克隆的分布偏移问题。详见 [DAgger](./dagger.md)。
...
代表工作按侧重分三类：**物理动作模仿** / **人类数据侧** / **策略与训练系统**。
