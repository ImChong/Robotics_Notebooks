"""wiki_git_added_dates：git 首次入库日期解析（新增/维护标签数据源）。"""
...
class WikiGitAddedDatesTest(unittest.TestCase):
    def setUp(self) -> None:
        glg._WIKI_ADDED_DATES_CACHE = None
        self.addCleanup(lambda: setattr(glg, "_WIKI_ADDED_DATES_CACHE", None))

    def test_collects_first_add_date_with_rename(self) -> None:
        log_text = (
    ...
    def test_wiki_node_action(self) -> None:
        added = {"wiki/tasks/locomotion.md": "2026-05-01"}
        self.assertEqual(
            glg._wiki_node_action("wiki/tasks/locomotion.md", "2026-05-01", added),
            "added",
        )
        self.assertEqual(
