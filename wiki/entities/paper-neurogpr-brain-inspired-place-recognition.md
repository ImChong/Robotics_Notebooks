...
**NeuroGPR 的核心不是「用了 SNN」，而是用模态互补正面对冲 VPR 的三大死穴：视觉负责语义、LiDAR 负责光照不变的结构，脑启发只是这套分工的解释框架。**

- 真正起作用的是 **跨模态注意力融合**：ANN 分支给语义描述符、SNN 分支给 LiDAR 时序稀疏特征，按置信度加权，因而在光照剧变、视角偏移、动态遮挡三类场景下都优于单分支；消融实验正是为验证这层互补的必要性。
- **SNN 不是唯一实现**：无神经形态芯片时 LiDAR 的稀疏时序特征也可用轻量 LSTM/GRU 处理，真正可迁移的是融合框架而非脉冲网络本身。
- 适用边界由 **传感器配置** 决定：纯摄像头平台上 NeuroGPR 退化为单模态，鲁棒性优势消失。
- 最大缺口是 **能效声称未被兑现**：性能数字主要在 GPU 服务器而非 TianjicX 硬件上得到，实时功耗数据不完整；ANN+SNN 联合训练还需协调反向传播与代理梯度，稳定性低于纯 ANN。
- 落地状态偏保守：截至入库日无公开代码，四足实机演示的路线长度与环境多样性有限，城市级 SLAM 未验证；现实定位更接近 SLAM 栈里的 Loop Closure / 重定位模块。
- 与 [TianjicX](./paper-tianjicx-neuromorphic-chip-robots.md) 构成同团队「芯片 → 应用」一条线：前者证明混合 ANN+SNN 硬件可行，本文把它落到具体高价值导航任务。
