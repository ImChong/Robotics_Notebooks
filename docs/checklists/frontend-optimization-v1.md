...
- [x] 详情页 Mermaid 灯箱：加载态文案 + `stage-pending` 在 `fit` 完成前隐藏，消除「空面板 / 先大后缩」闪烁。
  ...
  - [x] 图谱页时序动画播放方向：按 git 首次加入日 `added` **从最老到最新**显现（缺日期置末）；不再用会塌成同一天的 frontmatter/mtime `recency`（否则退化成路径字母序，新页面先出现、老任务页最后才出现）。「显示维护更新」打开后改用 `activity`。验证：`tests/graph-timeline-order.test.cjs`、`scripts/verify_graph_timeline_order.cjs`。
  - [x] 图谱页时序动画按钮交互：进入后立刻退出、拖到 0 / 回到起点再退出、连点切换、播放中「刷新布局」、播完再播、2D↔3D 切换、3D 时序退出，均恢复可见全图；拆除时打断 pop-in transition 并在恢复力模拟成员后同步 DOM。验证：`scripts/verify_graph_timeline_buttons.cjs`。
...
- [ ] Lighthouse 性能/可访问性评分目标 90+：历史勾选缺少可复用的当前测量记录；2026-09-07 复核未运行 Lighthouse，须按工程计划 #35 补测后确认。

## 2026-09-06 工程可靠性专项

完整顺序与验收见 [根目录工程计划](../../plan.md)（PR #1820），每个改进项独立提交。

- [x] 缓存隔离（编号 10）：只清理本项目旧缓存；请求限定项目路径；缓存读取不跨项目。新增 3 项 Node 行为测试，并接入 Tests 工作流和 `make ci-test`。
- [x] 搜索失败恢复（编号 13、16 搜索端）：失败后可重试，索引与社区请求设 30 秒超时；错误提示提供重试和图谱入口。新增 4 项行为测试。
- [x] 过期搜索保护（编号 14）：覆盖清空后的成功/失败回填、快速换词、Esc 和社区筛选切换，共 5 项新行为测试。
- [x] WebGL 失败恢复（编号 15）：6 项行为测试覆盖库缺失、创建/show/清理异常、正常往返、自动恢复已存 3D 偏好失败。测试模拟渲染器边界；正常 GPU 性能与真机浏览器待后续实测。


## 2026-09-06 正文按页加载

- [x] 正文拆分为目录与内容哈希文件；详情和路线只加载当前页正文，模块与预览只用元数据，保留旧导出兼容现有工具。
- [x] 内链预览、Markdown 深链、实体旧 ID 与路线重定向行为回归；失败可重试、返回搜索并打开原文；正文 404 自动刷新目录一次。
- [x] SW 移除两份正文全集的默认预缓存；目录网络优先，已缓存哈希正文直接复用；其余大数据缓存优化仍待推进。
- [x] 精确导出契约门禁与 15 项新前端行为测试、9 项 Python 导出测试；同内容目录 5.70 MB，对比全集 29.04 MB，正文中位数 4.92 KB。
- [ ] 真实浏览器验收与截图：本轮本地预览被云浏览器拦截；行为测试不等同 E2E。交付与最终 CI 见 [工程计划](../../plan.md)。

## 2026-09-07 性能计划复核

本节同步 [工程计划的近期执行顺序](../../plan.md#下一批执行顺序按投入收益比)，仅更新计划；下面的性能改动尚未实施。基线为 `4b0320bf`，线上资源体积与 Node 微基准的测量边界统一以工程计划为准。

- [x] 核对正文按页加载 PR #1852 已进入 main、四项 PR CI 通过；#16 详情错误重试与原文入口已实现，真实浏览器验收仍待补。
- [x] 更新性能基线：搜索索引 68.65 MB / 线上 gzip 16.82 MB，侧栏索引 29.31 MB / gzip 7.76 MB；本轮前端行为测试 33/33 通过。
- [x] 第 1 批，#4/#12（2026-09-09）：SW 预缓存拆为必要外壳（`/`、`index.html`、`style.css`、`theme-init.js`、`main.js`，整批安装）与可选资源（逐个缓存，失败不阻断安装）；`search-index.json`、`link-graph.json`、`wiki-activity.json`、`hub-rankings.json` 退出安装期预取，同内容本地合计 83.58 MB 原始 / 19.09 MB gzip 不再于安装时下载。首页删除无条件 idle/1.2s 预取，搜索索引改由 hover/pointerdown、`focus`、`#wiki-search` 深链与实际查询触发；加载提示与失败重试不变。6 项新行为测试（前端 39/39）。**未做**真实浏览器安装/升级/离线与首次搜索延迟验收。上文 Phase 4 的 idle 预取是历史实现记录，后续策略以本项为准。
- [x] 第 1 批，#8（2026-09-09）：`ensureLinkGraphData()` 让详情页社区/路线/机构徽标、局部图谱与近期关联更新共用一次 `link-graph.json` 请求与解析（原 5–6 处各自 fetch + parse）；失败清空 Promise 可重试，局部图谱仍用自建副本喂 d3，不写共享对象。同内容本地 Node 单次解析 14–29 ms、连续 5 次 111 ms（5,903,592 字节 / 3,891 节点 / 34,594 边）。3 项新行为测试（前端 42/42）。搜索块社区索引（自带超时/取消）与首页迷你图请求属 #5，未合并。
- [x] 第 1 批，#3（2026-09-09）：图谱侧栏由 `index-v1.json`（29,827,800 字节 / gzip 7,854,231）改读无正文的 `site-catalog-v1.json`（5,911,738 / gzip 1,245,990，−80.2% / −84.1%）；导出端把论文笔记链接（含正文行内链接）写进目录条目，侧栏删掉正文正则扫描，3,925 页逐页比对渲染结果一致。`index-v1.json` 继续生成供旧客户端。2 项 Python 契约测试 + 4 项前端行为测试（前端 58/58，导出质量 13/13）。
- [x] 第 1 批，#36（2026-09-09）：首页紧凑列表只用 `home-stats.json` 的 `latest_wiki_nodes` 取前 5 条新增，`Promise.all` 不再等 `wiki-activity.json`（3,822,794 字节 / gzip 529,232 / Node 解析 17 ms）；凑不满 5 条才按需回填并重渲染一次；`change-log.html` 完整时间线与热力图仍先取活动全集。3 项新行为测试（前端 45/45）。浏览器端 LCP/INP 未测。
- [ ] 第 2 批，#5：导出首页 Top-50 子图及独立社区映射，首页预览与筛选不请求全图。
- [x] 第 2 批，#9（2026-09-09）：`detail.html` / `roadmap.html` 各去掉 6 行 KaTeX + Mermaid 静态外链（实测合计 1,082,125 响应字节 / 3,648,639 原始字节），改由 `ensureKatexLoaded()` / `ensureMermaidLoaded()` 按正文是否含定界符或 `.mermaid` 节点注入，`integrity`/`crossorigin` 不变，失败静默降级可重试。3,925 页中 2,184 页（55.6%）两者都不需要。5 项新行为测试（前端 50/50）。图谱页同步去掉 `vendor/3d-force-graph.min.js` 静态引入（1,313,897 字节 / gzip 346,310），改由 `ensureGraph3DLibrary()` 在进入 3D 时注入，加载期按钮忙、失败仍走既有 2D 恢复路径（4 项新测试，图谱相关 10/10）。G1 推迟的「仅初始化当前视图」仍属图谱专项；首页 3D 懒加载与 WebGL 失败恢复不变。
- [ ] 第 2 批，#1/#7：索引精简、倒排候选集和 Worker 内加载/解析；先降低冷启动与全量扫描成本，再优化 Top-K。
- [ ] 后续，#37：单页元数据与局部邻接数据减少全站目录/图谱依赖，保留深链、预览和路线。
- [ ] 图谱专项，#6：按下节 G0–G6 推进；先测量与减负，再做真实子图，Canvas/Worker/3D 批绘制由瓶颈决定。G0/G1/G2/G3 已落地（2026-09-09），剩余首批为 G4。
- [ ] 并行门禁，#35/#11/#23/#24：每项记录同内容冷/热缓存、首次搜索和详情切换指标；版本兼容、数据契约、浏览器交互随变更验收，真机/GPU缺失明确留空。每完成一项独立提交并推送。

- [x] 本地真实浏览器验收（2026-09-09，headless Chromium + 本地静态站）：首页 0 次 `search-index.json` / `wiki-activity.json` 且「最近新增」5 行正常，聚焦搜索框后才拉索引；SW 激活后缓存 16 条、四份大数据均不在其中（安装下载 19,007,133 → 286,421 gzip 字节）；无公式无图表详情页 0 次组件请求，含公式图表页按序注入并渲染出 15 个 `.katex` 与 1 个 Mermaid SVG，CDN 不可达时静默降级；图谱页 2D 不取 3D 库与 `index-v1.json`，点 3D 后注入 1 次并成功进入 3D。**未测**手机 / Safari / 弱网 / 真机 GPU / LCP / INP；沙箱浏览器直连 jsdelivr 受阻，成功路径用同一份官方文件本地兜底（SHA-384 与 `integrity` 一致）。


## 2026-09-08 图谱动态渲染性能计划

完整依据、估时、设计约束与验收见 [工程计划图谱专项](../../plan.md#2026-09-08-图谱动态渲染性能专项)。G0 已于 2026-09-08 完成并改写了 G4/G5 的先后判断；G1（部分）、G2（部分）、G3 已于 2026-09-09 完成，实测见 [G2/G3 前后对比](graph-perf-g2-g3.md)；G4–G6 实现与真机性能验收仍未完成。原 #6 拆为 G1–G6，关联 #9/#24/#35，其他全站任务状态不变。

- [x] G0 / #35：已完成 —— `scripts/measure_graph_perf.cjs` + [基线报告](graph-perf-baseline-g0.md)。固定数据 SHA `224d420972e9b4c2`（3,867 节点 / 34,409 边）；力 tick p50 45.6 ms、DOM 写入 p50 89.3 ms（社区标签仅 1.2 ms）、`applyFilters()` 132.2 ms；刷新布局 4 s 内 14–16 个长任务、最长 1.33 s；3D 每帧 38,272 draw calls / 1,660,608 三角形。正常 GPU、手机与 Safari **未测**（headless 软件光栅）。
- [x] G1 / #6/#9：**部分完成** —— warmup 按 16ms 时间预算分片 + `warmupToken` 取消旧任务、刷新布局改走分片路径；刷新种子由固定 80px 改为 `max(80, √N × 8)`；三个滑块按帧合并。实测：刷新布局最长长任务 1,141–1,307 → 284–305 ms，帧耗时 p50 252–284 → 52–58 ms，滑块输入主线程占用 1,130 → 0.3 ms（[前后对比](graph-perf-baseline-g0.md#g1-前后对比2026-09-08)）。社区标签按变化更新**不做**（实测 1.2ms 非瓶颈）；只初始化当前视图与 2D 降级**推迟**并入 #9。
- [x] G2 / #6：**部分完成** —— 3D 节点标签按需创建、不可见时不写逐节点样式、先判可见再投影；悬停刷新改走 `__threeObj` 不再遍历整棵场景树。实测：悬停刷新 403.1 → 18.0–22.5 ms（约 19 倍），`syncLabels()` 346.0 → 12.7–14.5 ms（约 24 倍），标签关闭时标签 DOM 3,868 → 0（[前后对比](graph-perf-g2-g3.md)）。**普通边细线经实测否定不做**：`linkWidth→0` 后本库为每条边各建一个 `THREE.Line`，draw calls 38,288 → 38,287 无合批，几何体数反由 2 涨到 34,421 且高亮加粗失效 —— 修正 G0 第 7.4 条，合批只能走 G6。拖拽重新加热已按手势去重，非瓶颈，不改。
- [x] G3 / #6：**已完成** —— `syncFilterSimulation()` 把力模拟成员换成筛选子图，`syncGraphDomFromSimulation` 按 `simulation.nodes()` / `forceLink.links()` 直写坐标；被筛掉的节点速度清零就地冻结，取消筛选原地恢复；节点 ID、DOM 身份与邻接信息不动。实测（Top 300）：`simulation.nodes()` 3,868 → 300、`forceLink.links()` 34,420 → 3,435，力 tick 69.2 → 4.2 ms、DOM 同步 191.2 → 14.1 ms，**每 tick 合计 ≈ 260 → ≈ 19 ms**，首次接近 16.7 ms 帧预算。全图态每 tick 仍 ≈ 203–230 ms，未达标，剩余属 G4。
- [x] G4 / #6（边层部分）：**2D 边改 Canvas 批绘制**（节点/标签/交互仍在 SVG，键盘、搜索与详情导航未动），见 [G4 前后对比](graph-perf-g4.md)。同机同数据（SHA `47e04a996f7c4354`，3,968 节点 / 35,490 边）实测：SVG 元素 51,429 → 15,938、每 tick 坐标属性写入 145,928 → 3,968、渲染同步 p50 143.6 → 19.7 ms、每 tick 合计 ≈217 → ≈96 ms、`applyFilters()` 115.3 → 56.1 ms、首屏就绪中位数 5,214 → 4,749 ms。功能门禁 `scripts/verify_graph_edge_canvas.cjs` 16 项全通过。真机 GPU、手机、Safari 与高 DPR 屏上的像素比上限 **未测**。
- [ ] G5 / #6（**G4 后门槛成立，下一批**）：Worker 力模拟、最新坐标快照、有界传输、拖拽命令与版本取消；保持 2D/3D 数据独立，失败可回退；3D 接入单独评估。
- [ ] G6 / #6（按测量决定）：3D 节点实例化、连线批绘制、按设备控制像素比，验证透明度、高亮、拾取与 GPU 资源释放。
- [ ] #24/#35：固定全图/Top 300/单社区，覆盖展开、刷新、拖拽回弹、悬停、缩放旋转、筛选恢复、2D/3D 切换、标签与时序退出；报告 P50/P95 帧耗时和长任务，不只报平均 FPS。桌面接近 60 FPS/移动端稳定 30 FPS 为待验证目标。
- [ ] 交付：每完成一项独立提交并推送，附同内容前后对比与功能回归；保留全图动态入口，不以缩短动画或仅降低透明度宣称完成优化。
