...
**这篇 2010 年的工作给「紧密交互运动的编辑与重定向」立了一个表示层的答案：不逐一手写接触约束，而是把部位、角色与环境物体统一织进一张交互网格，用最小化其局部形变来保住空间关系。**

- 真正起作用的是表示的 **统一性**：交互网格对单角色不同部位、多角色之间、以及环境物体一视同仁，所以跳舞、摔跤、剑斗、钻车能共用同一套机制。
