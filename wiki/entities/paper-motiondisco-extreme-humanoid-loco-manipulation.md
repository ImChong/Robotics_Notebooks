...
**绕过人体演示瓶颈：把接触计划写成可变异 Python 程序，用 LLM 进化搜索 + 接触显式 TO 反馈闭环从零发现长时程全身 loco-manip，再 RL 跟踪零样本上 G1。**

1. **数据入口范式转换** — 不依赖 MoCap/重定向或遥操作，把探索重心移到自动化接触计划搜索，可为无人类示范的新场景生成参考轨迹并产出多解多样性。
2. **LLM×优化闭环有实证增益** — 相对单次 LLM 调用（SC 多项失败），迭代搜索 + 文本失败反馈在 8 项任务上提高有效接触计划比例并降低 TO cost。
3. **工程可行性** — 首条可行解 1.34–7.49 min（任务依赖），使组合爆炸的接触空间探索具备工程可行性。
4. **真机闭环里程碑** — 作者称首个完全通过自动化进化搜索发现并在 Unitree G1 真机零样本执行长时程 loco-manipulation（攀台、穿障、桌下取放等）。
5. **局限** — 假设已知场景文本、无感知，接触模型仅单边粘附 + 箱状刚体；walk() 为直线平地步态不自动绕障。
