...
**EgoLive 的赌注在数据分布而非模型：把第一视角采集从「日常视频」挪到家政、零售这类任务导向的真实工作场景，让人类视频更接近机器人未来的部署环境。**

- 差异化在采集口径——**大规模、真实场景、任务导向、多模态标注**；价值在于让机器人不只「看到世界」，还能学到一个任务怎么开始、怎么推进、哪里容易失败、人在执行时会怎样调整。
- 边界很明确：Ego 视频不天然等于机器人策略数据，仍需重建、对齐、重定向与物理过滤，本页定位在 [01 数据采集](../overview/ego-category-01-data-collection.md) 层而非策略层。
- 本页为策展编译，量化 benchmark、消融与实机指标以原文 PDF / 项目页为准；数据入口挂在京东云 robotdata-market 控制台。
