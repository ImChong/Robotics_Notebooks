...
**Learning Whole-Body Human-Humanoid Interaction from Human-Human Demonstrations** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：04_Loco-Manipulation_and_WBC），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇工作的核心不是又一个模仿学习模型，而是先指出人-人形交互的两处断点——重定向会毁掉接触、模仿轨迹学不到交互——再用 PAIR 与 D-STAR 各补一处。**

- 数据侧的关键判断是「接触是硬约束」：标准重定向跨形态时丢掉的恰恰是交互的本质，PAIR 以接触为中心做两阶段重定向，才谈得上物理一致的 HHoI 数据。
- 策略侧的关键归纳偏置是时空解耦：相位注意力管「何时动」、多尺度空间模块管「何处动」、扩散头融合二者，使时间相位不被空间噪声带偏，产出的是同步协作而非照搬轨迹。
- 借 HHI 数据补 HHoI 数据是规模化捷径，但整条链路押在转换保真度上：重定向一旦失真，后面再好的策略结构也救不回来。
- 适用边界与风险：本页只报告仿真中显著优于基线，未涉及真机，把结论外推到实机人-人形物理交互需谨慎。
- 与本页提到的 SUGAR / EgoHumanoid「借人类数据」同源，也与 LessMimic、HumanX 的交互方向呼应；差别在于本页把接触保真当成第一性问题。
