...
**这篇对工程读者的价值，在于把「策略多慢、PD 多快、增益要不要随机化」这些通常被藏起来的接口细节写成公开数字；多技能统一是这套观测–接口设计的结果，而不是另加的技巧。**

- 起作用的机制是 **双历史**：长 horizon 捕获步态周期与地形缓变，短 horizon 强化触地瞬态与快扰动响应；行走、跑步、跳跃共享同一策略族，靠命令与随机化切换行为先验。
- 可直接复用的是三个数：策略约 **33 Hz**、关节 PD 内环约 **2 kHz**、训练期名义 PD 增益做 **0.7–1.3 倍** 缩放随机化。
- 调参顺序有讲究：先锁「策略一步内的 PD 子步数 / 真机控制循环」，再调名义 `Kp/Kd`，否则 DR 区间会与真实带宽对不上。
- 0.7–1.3 应理解为 **缩放名义表** 的窄区间实践，与「±30% 标称」类宽随机化不矛盾，二者服务不同保守程度的 sim2real；与 [Domain Randomization 指南](../queries/domain-randomization-guide.md) 并排读最有效（一边通用 checklist，一边双足实测区间）。
- 本页正文侧重方法结构与知识库交叉引用；量化指标、消融与 sim2real / 实机结果以原文 PDF 为准。
