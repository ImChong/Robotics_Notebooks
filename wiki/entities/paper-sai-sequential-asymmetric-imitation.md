...
> 下表只做 **定位对照**：前两行是论文自带的同协议基线（同任务集、同骨干，可比），后几行是跨工作的路线定位，**不共享评测协议**，不要把成功率并排读。

| 对照 | 差异读法 |
|------|----------|
| **Independent Imitation**（论文基线） | 两台各自独立模仿，伙伴分布始终是训练时那个；桌布任务里 B 被暂停后仍继续拉扯——SAI 的三阶段课程正是把「伙伴会变」写进训练分布 |
| **Partner-Conditioned Imitation（PCI）**（论文基线） | 把伙伴状态作为条件输入，等于**假设伙伴可观测**；SAI 不交换任何消息/状态/未来动作，协调从共享物体与接触涌现，因而部署侧不欠通信债 |
| **同步双遥操作示范**（要替代的默认做法） | 需要两名操作员长程同步，采集成本随任务长度爆炸；SAI 只用**一名**操作员，代价是多了两个课程阶段与一次冻结部署 |
| [TRACE](./paper-trace-causal-memory.md) | 同团队的互补答案，解决轴不同：TRACE 补的是**延迟证据的记忆**（早先线索晚才起作用），SAI 补的是**伙伴分布的真实性**。两者可叠加，不是二选一 |
| [Zeno-1](./paper-zeno-1-collaborative-intelligence.md) | 同一协作问题的**另一层解法**：Zeno-1 用 CPI 在基础模型尺度做闭环伙伴交互，SAI 是**课程/数据侧**的轻量答案。极端遮挡或长延迟下纯去中心化可能不足，那正是 Zeno-1 的持久记忆与预测内省要覆盖的区间 |
| [ACT](../methods/action-chunking.md) / Diffusion Policy | **不是竞品而是骨干**：SAI 在两者上都稳定优于 Independent Imitation，这正是「增益来自课程结构而非骨干」的证据——工程上可先定骨干，再套 SAI 采集流程 |
