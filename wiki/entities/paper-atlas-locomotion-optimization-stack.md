...
Atlas Locomotion 的贡献不是某一层的新算法，而是把规划、状态估计与高频反馈整合进同一条闭环。与它常在本库交叉阅读的两类页面正好是这条闭环里被单独抽出研究的组件：

| 维度 | 本工作（Atlas Locomotion） | [whole-body-control](../concepts/whole-body-control.md) | [state-estimation](../concepts/state-estimation.md) |
|------|----------------------------|--------------------------------------------------------|-----------------------------------------------------|
| 关注范围 | 足步/全身规划 + 估计 + 控制整条闭环的接口统一 | 全身多任务跟踪与接触执行（单一控制层） | 浮基位姿/速度/接触状态的融合估计（单一感知层） |
| 核心对象 | 名义状态与输入轨迹 + 时变 LQR 在线反馈修正 | 任务优先级下的加速度/接触力/关节力矩 | 基座位姿、速度、接触集合 |
| 关键假设/接口 | 各层坐标系、频率、状态定义与延迟必须统一，并标记计划版本 | 需一致的浮基动力学模型与接触集合 | 需可靠的传感融合与模型，供控制层消费 |
| 关系/取舍 | 把 WBC 与状态估计作为组件纳入同一闭环，强调接口而非单一算法名 | 是本工作闭环中的控制层，可被替换但需对齐状态接口 | 是本工作闭环中的估计层，其误差直接决定反馈能否收敛 |

对照相关任务页 [humanoid-locomotion](../tasks/humanoid-locomotion.md) 时，重点看这条栈把「谁消费什么状态、旧计划何时失效」讲清楚，而非只比单点行走指标。
