...
**这篇 TOG 工作的关键不是更快的接触检测，而是把障碍势与弹性刚度对齐进同一个牛顿步——用「谁参与刚度组装」这件事，换来 GPU 单精度下大规模接触仍然无穿透、应变可控。**

- 起作用的是 **三次障碍 + 弹性包容动态刚度** 的组合：接触 Jacobian 与弹性力分步处理时会出现虚假刚度、穿透或「橡胶化」拉伸，同一牛顿步对齐可减少分裂迭代振荡；应变限制（示例常取 ~5%）再挡住非物理「橡皮膜」。
- 适用边界很清楚：面向 shell / solid / rod 的 FEM 可变形体与百万–亿级接触的 **离线高保真仿真**，服务服装、软包装、索网等强几何约束场景，不替代 [MuJoCo](./mujoco.md) 这类刚体 RL 基准，也不是百万并行环境的赛道。
- 与 MuJoCo 软接触的类比最容易误导：后者的 convex 软接触服务刚体控制，本文的障碍 + FEM 服务壳/体网格与亿级接触对，问题规模与建模假设根本不同。
- 复现风险在版本口径：`main` 持续演进，严格复现论文应走 `sigasia-2024` 分支或论文 Docker 镜像，但该分支不含后续性能改进与 bugfix，两者不可兼得。
- 工程价值来自配套的 [ppf-contact-solver](./ppf-contact-solver.md) 开源栈（Docker、Jupyter、Blender、stress CI），它把图形学接触算法从 PDF 拉到可跑 demo，对软体操作、可穿戴、柔性夹具方向有直接参考价值。
