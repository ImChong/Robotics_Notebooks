---
type: entity
tags: [paper, loco-manipulation, loco-manip-survey, humanoid]
status: complete
updated: 2026-06-14
arxiv: "2606.08548"
summary: "仿真 teleop + 域随机化 → 零样本宇树 G1；覆盖可超有限真实 teleop。"
related:
  - ../overview/loco-manip-8-papers-technology-map.md
  - ../overview/loco-manip-category-02-synthetic-data.md
sources:
  - ../../sources/papers/loco_manip_survey_04_oasis.md
  - ../../sources/blogs/wechat_embodied_ai_lab_loco_manip_8_papers_survey.md
  - ../../sources/papers/loco_manip_8_papers_catalog.md
---

# OASIS

**OASIS** 收录于 [具身智能研究室 · Loco-Manip 8 篇周报](https://mp.weixin.qq.com/s/Ez87ljBYmCyIpLKjMjEyaQ) **第 04/8** 篇，归类为 **02 生成与仿真数据**。本页为知识库 **策展摘要**；方法细节以论文 PDF 与项目页为准。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| Loco-Manip | Loco-Manipulation | 行走与操作动力学耦合的全身任务 |
| WBC | Whole-Body Control | 协调全身关节满足多任务/约束的控制层 |
| VLA | Vision-Language-Action | 视觉-语言-动作多模态策略 |

## 为什么重要

- 仿真 teleop + 域随机化 → 零样本宇树 G1；覆盖可超有限真实 teleop。
- 在 [Loco-Manip 8 篇技术地图](../overview/loco-manip-8-papers-technology-map.md) 中属于 **[02 生成与仿真数据](../overview/loco-manip-category-02-synthetic-data.md)**。

## 核心信息（索引级）

| 字段 | 内容 |
|------|------|
| 编号 | 04/8 |
| 分组 | 02 生成与仿真数据 |
| 出处 | 2026 · arXiv:2606.08548 |
| 论文/项目 | <https://arxiv.org/abs/2606.08548> |

## 与其他页面的关系

- 技术地图：[loco-manip-8-papers-technology-map.md](../overview/loco-manip-8-papers-technology-map.md)
- 分类 hub：[loco-manip-category-02-synthetic-data.md](../overview/loco-manip-category-02-synthetic-data.md)
- 原始 source：[loco_manip_survey_04_oasis.md](../../sources/papers/loco_manip_survey_04_oasis.md)

## 实验与评测

- 本页为 **策展索引级** 摘要；量化 benchmark、消融与实机指标以 **原文 PDF / 项目页** 为准。

## 参考来源

- [loco_manip_survey_04_oasis.md](../../sources/papers/loco_manip_survey_04_oasis.md) — Loco-Manip 8 篇策展摘录
- [loco_manip_8_papers_catalog.md](../../sources/papers/loco_manip_8_papers_catalog.md)
- [wechat_embodied_ai_lab_loco_manip_8_papers_survey.md](../../sources/blogs/wechat_embodied_ai_lab_loco_manip_8_papers_survey.md)
- 论文/项目：<https://arxiv.org/abs/2606.08548>

## 推荐继续阅读

- [Loco-Manip 8 篇技术地图](../overview/loco-manip-8-papers-technology-map.md)
- [Loco-Manipulation 任务页](../tasks/loco-manipulation.md)
