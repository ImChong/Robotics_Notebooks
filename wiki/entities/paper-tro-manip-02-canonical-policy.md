...
**Canonical Policy 走的是「先把观测规范化、再学策略」的路线：用几何对称性替代对物体位姿的数据覆盖，而不是往网络里堆等变层。**

- 起作用的机制是 **点云归一化到标准参考坐标系内决策、动作再反变换回真实空间**，因此策略学到的是与姿态无关的映射，在未见物体姿态与视角下仍能泛化。
- 报告增益在真机侧更大（12 仿真 + 4 真机任务、16 种配置下仿真 **+18.0%**、真机 **+39.7%**），与「减少对位姿覆盖数据依赖」的动机方向一致；完整消融与 baseline 以项目页 / PDF 为准。
- 适用边界：SE(3) 等变不免除示教——仍需 canonical 空间内足够覆盖任务语义的演示数据。
- 与 EquiBot / DP3 等 3D 策略的区别在实现路径（**canonicalization** vs 网络内置等变层），而非是否具备等变性。
- 规范化层与策略头解耦，可接 [Diffusion Policy](../methods/diffusion-policy.md) 这类生成式策略头，兼顾对称性与表达力。
