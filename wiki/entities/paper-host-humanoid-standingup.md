...
**HoST 把「起身」从跟踪预定义轨迹改写成纯奖励驱动的多阶段 RL 问题；真正让它上真机的不是更强的算法，而是奖励结构的拆分与一整套动作约束。**

- 决定性机制是 **多 critic**：奖励分 task / style / regu / post 四组各自估 return 再聚合，消融显示去掉多 critic 后 **四地形成功率均为 0%**。
- 探索侧靠 **竖直拉力课程** 压低高 DoF + 宽关节限位下的随机探索失败率；真机侧靠 **动作 rescaler β 课程 + L2C2 平滑正则** 抑制暴力冲撞与振荡 — 这是「仿真能站、真机发抖」的正面解法。
- 「直接部署」只指 **无额外真机 RL 微调**，仍依赖四地形多样化初始姿态 + 域随机化 + 运动约束，不是裸策略迁移。
- 复现注意：默认按 ground / platform / wall / slope **分别训练**，全地形联合训练仍是 README TODO，checkpoint 必须与地形匹配。
- 与 [SD-AMP](./paper-unified-walk-run-recovery-sdamp.md) 的 AMP 先验 + 重力门控统一走/跑/起身不同，HoST **无运动先验、专注多姿态起身**，更适合当作 recovery 子技能与 locomotion 策略拼接。
