...
- **本页为索引级节点**（FreeDof 44 篇梳理 [10/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页要点是「在仿真中对动力学参数采样，优化期望回报下的策略」，对应证据是带 / 不带 DR 的真机迁移成功率对比，以及随机化范围的消融。
...
| **横比口径** | 随机化范围是针对该机型标定出来的；范围数字不可跨平台搬用——范围过窄迁不过去，过宽则牺牲标称性能。 |
