...
**G3M 用「图」当中介，把无动作标签的人类视频接进机器人策略：真正被迁移的是物体与交互的空间关系，而不是像素或本体特定的关节细节。**

- 起作用的机制是 **两阶段**——先做图到图生成预训练（当前帧图 → 未来帧图）隐式学操作物理，再把生成图当条件驱动下游动作策略；这个结构化中介正是跨本体迁移 **+33%+**（期刊版 **+35%+**）的来源。
- 关键收益是标注效率：仅 **20% 带标签数据** 即可达到全量标注相当的策略性能，正对应「机器人演示贵、互联网视频便宜」的原始动机。
- 边界要说清：图预训练只提供 **结构化条件**，下游策略的 sim-to-real 与接触鲁棒性仍需单独工程化，不会因预训练自动获得。
- 命名上 G3M 与 CVPR 2025 的 GraphMimic 是同一研究线的期刊/会议版本，本库以 T-RO 版 **G3M** 为主条目，指标口径以期刊版为准。
