...
**这篇的取舍是「不先训检测器」：让深度序列直接蒸馏出策略真正需要的球与对手 latent，代价是整条证据链停在仿真、动态对手仍是硬骨头。**

- 起作用的是 [RMA](./paper-rma-rapid-motor-adaptation.md) 式两阶段——Actor 接口在 Phase 1/2 保持不变，只把 latent 生成从特权 MLP 换成 CNN+GRU，感知与控制的改动互不牵连。
- 课程是动态场景能否学会的分水岭：消融显示 Stage 1 SR **2%** vs 完整课程 **46%**，无课程时追球对手几乎学不会。
- 难度梯度极陡：无障碍 **100%**、静态障碍 **96%**、ball attacker 仅 **46%**，失败以跌倒 52% / 人机碰撞 68% 为主。
- 瓶颈不在感知：球位置误差约 **0.05–0.08 m** 且跨场景稳定；blocked 时段球速角误差上升，说明动态平衡下的球-脚-障碍耦合控制才是主矛盾。
- 边界：仅仿真、单策略种子、动态对手行为脚本化；选深度 + 时序是为平衡遮挡与 sim 可训练性，不等同于断言 RGB 端到端更差。
- 开源状态记为 **项目页发布 / 部分开源待跟进**（截至 2026-07-20 未见独立 GitHub 仓）；sim2real on Booster T1 为论文列出的未来工作。
