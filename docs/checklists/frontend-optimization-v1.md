...
- [x] **响应式体验优化**：
    ...
    - [x] 图谱页 `graph.html`：移动端「按社区着色」图例改为左下角 FAB，点击后自左侧滑出抽屉，点击空白/遮罩或 Esc 收起，避免遮挡画布。
...
- [x] 首页调整为：目标入口 → 学习路线 → 搜索 → 最新内容 → 知识图谱 → 互链榜单。
- [x] 新增四类目标入口：从零开始、强化学习、传统控制、项目查询。
...
## 验收标准 (DoD)
- [x] 首页滚动条长度缩减 50% 以上。
- [x] 首页首屏 (Above the fold) 必须包含：核心定位、规模统计、2 个核心 CTA，并紧接按目标选择的入口。
