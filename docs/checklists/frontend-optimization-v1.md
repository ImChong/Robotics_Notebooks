...
- [x] 更新记录页 `change-log.html`：热力图下方筛选开关；**默认仅展示** `action=added` 新增节点（无新增的日期隐藏），日期窗口仍锚定全部活动日；按钮文案「显示维护节点」，点击后一并展示维护条目（`aria-pressed` / `is-active`），再次点击「只看新增节点」恢复默认；热力图单日筛选与该开关叠加。首页「最新知识节点」紧凑列表同步默认只列新增（不足时从活跃度回填）。
