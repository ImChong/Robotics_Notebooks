...
**这项工作的杠杆不是把驱动器做强，而是把能量的「存」与「放」解耦：双稳态结构预存弹性应变，磁脉冲只负责把它推过势垒。**

- 起作用的机制是 snap-through：3D 折叠磁性薄板比平面双稳态能量密度更高，越过 STP 后弹性能在 **<15 ms** 内倾泻，得到 **>2 m/s** 起跳与 **>108× 体高**，把软跳跃机器人从 0.3–1 m/s、100 ms–10 s 的区间整体抬高一个量级。
- 控制接口极简是附带红利：同一磁脉冲用**强度**切 interwell 大跳 / intrawell 小弹、用**偏转角**调起跳方向，不需更换机构。
- 适用边界卡在磁场源上：驱动依赖外部线圈或永磁铁的实验室台架，机载磁场源的重量/体积问题未解，**完全自主的无线运动目前不可行**。
- 主要失败模式是落点不可控：演示为开环脉冲触发，软体弹性变形带来姿态随机性，闭环感知反馈与精确落点仍是开放问题。
- 复现门槛高：截至入库日无开源仓库，工艺仅见于 Materials and Methods，磁化方向对制造精度敏感；可直接取用的公开资产是 Dryad 上的实验数据。
