...
sources:
  - ../../sources/papers/loco_manip_161_survey_144_n144.md
...
**学习人对人的实时全身远程操作。** 收录于 [具身智能研究室 · 人形 Loco-Manip 161 篇长文](https://mp.weixin.qq.com/s/pACh9EhsISiyPGdiiR0C3A) **第 144/161** 篇，归类为 **08 硬件平台、感知配置与部署扩展**。
...
- 这篇工作主要解决数据闭环：用相机图像/多视角观测、本体状态与关节序列、遥操作/外骨骼数据采集人类操作和机器人状态，再通过ACT/行为克隆模仿学习、扩散策略/流匹配、MM-DiT/Transformer 动作头转成可训练、可复用的关节位置/力矩命令、全身轨迹/动作序列、末端执行器/腕手目标。关键点是把动作生成看成条件生成问题，用扩散或流匹配在多模态动作分布里采样可执行轨迹。
- 人形 Loco-Manip 161 篇 **#144/161** · 硬件平台、感知配置与部署扩展。
...
**这条目真正的价值在数据侧：沉浸式主动视觉反馈的遥操作先把人的操作变成可采集的示范，下游的动作生成模型才有多模态分布可采样。**

- 起作用的机制链是：相机图像/多视角观测、本体状态与关节序列、遥操作/外骨骼数据同时记录人的操作与机器人状态，再经 ACT/行为克隆、扩散策略/流匹配与 MM-DiT/Transformer 动作头，转成关节位置/力矩命令、全身轨迹与末端执行器/腕手目标。
- 适用边界在 **08 硬件平台、感知配置与部署扩展**：它给的是采集接口与数据闭环，不等于底层 WBC 鲁棒性，须与运控/接触控制条目对照。
- 索引级局限：本页未搬运原文量化 benchmark 与实机指标，评测口径以 [原文 / 项目页](https://robot-tv.github.io/) 为准。
...
- 原始 source：[loco_manip_161_survey_144_n144.md](../../sources/papers/loco_manip_161_survey_144_n144.md)

## 参考来源

- [loco_manip_161_survey_144_n144.md](../../sources/papers/loco_manip_161_survey_144_n144.md) — 161 篇策展摘录
