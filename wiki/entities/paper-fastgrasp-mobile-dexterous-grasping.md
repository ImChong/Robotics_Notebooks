...
**FastGrasp 把「移动 × 灵巧 × 高速」拆成可学的两段——先用几何包络度挑姿态，再用全身 RL 冲过去执行，二值触觉兜住短暂的冲击窗口；代价是真机成功率仍停在 30% 量级。**

- 起作用的是 **引导质量** 而非策略容量：CVAE 采 150 候选、经前向锥+碰撞过滤与 **GWC/GDC 包络度** 排序，比随机引导（**14.45%**）与单阶段 PointNet（**3.03%**）高一个量级；包络度也胜过力方向质量（**50.09% vs 41.99%**），且不依赖显式法向，在部分点云下仍有 **38.51%**。
- 触觉在这里是 **一等公民** 而非锦上添花：去掉触觉观测掉 **15.65%**，真机去掉触觉适应从 32% 掉到 **26.87%**；刻意选二值压力而非高分辨率视觉触觉，是拿分辨率换 **15 Hz 实时** 与更小的 sim2real 差距。
- Sim2Real 不是可选项：高速设定下 **无 DR 或无 LPF 直接 0%**——这条消融比主指标更能说明部署难度来自哪里。
- 边界与风险：仿真 **50.09%** 与真机 **~32%** 差距明显，任务是最长 **2 s** 的短窗抓取–撤离；平底/桌面场景为主，极扁平物体作者自述仍困难，臂第 4 关节锁定与底盘 1.3 m/s 上限约束动态包络；截至论文页 **暂无开源代码**。
- 定位互补而非替代：相对 [HRDexDB](./hrdexdb-dataset.md) 的静态台架数据与 T-Rex 类高维触觉 VLA 路线，本文占的是 **动态移动执行 + 低维触觉** 这一格。
