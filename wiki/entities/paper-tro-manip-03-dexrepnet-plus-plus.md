...
**DexRepNet++ 押的是表征而不是算法：把「指节与物体表面的相对空间关系」显式编码进 DexRep 之后，灵巧操作的泛化瓶颈就从 sample efficiency 转移到了输入表征设计。**

- 真正起作用的是 DexRep 的三个分量（**Occupancy / Surface / Local-Geo**）刻画的是手-物空间关系与表面几何，而非物体全局形状——这解释了为何 40 个训练物体能外推到 5000+ 未见物体、抓取成功率仍达 87.9%。
- 收益不是单任务偶然：手内重定向相对既有手物表征 **+20%–40%**，双手交接同量级，且真机多/单摄像头下 sim-to-real gap 较小，说明表征本身而非某个训练 trick 在承担泛化。
- 适用边界必须划清：它解决的是 **输入表征泛化，不自动解决低层力控与触觉闭环**；需要接触力调节时应与 [Contact-Rich Manipulation](../concepts/contact-rich-manipulation.md) 对照阅读。
- 指标口径：87.9% 等数字来自 **仿真 benchmark**，真机协议与完整指标以原文 PDF / 项目页为准。
