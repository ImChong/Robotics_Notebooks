...
| **可自测的判据** | 穿透深度、单元翻转、切向阻尼、接触死锁（详见下） |
| **可复现路径** | Newton 多物理示例中对比开 / 关 DAT |
| **不适用的对照** | 与 MuJoCo 经典接触模型比「谁更准」（详见下） |

- **四条自测判据：** 穿透深度是否恒为 0；软体单元体积是否出现负值（翻转）；切向滑动是否被人工阻尼吃掉；密堆接触是否死锁。
- **复现路径：** 在 `cloth_*`、`mpm_*`、刚–软耦合 demo 中分别对比开 / 关 DAT，以及 全向 DAT 与 Planar-DAT 两档。
- **为何不能与 MuJoCo 接触模型比「谁更准」：** 后者管 **摩擦与冲量物理**，DAT 管 **几何可行性**，量的不是同一件事。
- **读法：** 这是一篇 **图形学求解器** 论文，评价轴是稳定性与保证强度，不是成功率。想要数字，须等 SIGGRAPH 2026 正式材料，或在自己的场景里按上述判据实测。
...
| 路线 | 接触在哪解 | 异构材料怎么办 |
|------|------------|----------------|
| **DAT / Planar-DAT** | 求解器 **之后** 的几何后处理 | 各物体只看自身约束 |
| MuJoCo 经典软接触 | 求解器 **内部**（凸优化 / LCP 近似） | 需同一接触模型表达所有材料 |
| [Particles4All](./particles4all.md) 式统一粒子 PBD | **单一求解环** 内的位置约束 | 全部降到同一套粒子表示 |
| IPC 类障碍函数方法 | 求解器内部的能量项 | 需为每种材料接入障碍项 |
| [Kamino](./paper-kamino.md) | 闭链刚体专用后端 | 只管刚体 |
| [Mixed MPM](./paper-mixed-mpm-stiff-elastoplasticity.md) | MPM 内部的本构处理 | 只管颗粒 / 弹塑性 |

逐条读法：

- **MuJoCo 经典软接触** — 管的是摩擦与冲量，DAT 管几何可行性；两者不是替代关系，可叠加。
- **Particles4All 式统一粒子 PBD** — 问题分解相反：统一表示 vs 保留异构求解器 + 统一后处理。
- **IPC 类障碍函数方法** — 同样追求无穿透，但代价进到 **优化目标里**（步长与收敛受限）；DAT 把代价放在投影步。
- **Kamino / Mixed MPM** — 是 DAT 的 **被服务方**，不是竞品；DAT 负责它们与刚体 / 壳相遇时的边界。
