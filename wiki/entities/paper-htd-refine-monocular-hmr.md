...
| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| HMR | Human Motion Recovery | 从单目视频恢复人体 3D 运动（含 world-grounded 全局轨迹） |
| PVA | Position–Velocity–Acceleration | PVA-Net 联合预测 2D 关键点、相机系 3D 速度与加速度 |
| MPJVE | Mean Per-Joint Velocity Error | 关节速度误差均值，衡量动力学保真度 |
| MPJAE | Mean Per-Joint Acceleration Error | 关节加速度误差均值，衡量高阶时序保真度 |
| SMPL | Skinned Multi-Person Linear Model | 参数化人体网格，HTD-Refine 优化的运动表示 |
| RoPE | Rotary Position Embedding | 时序 Transformer 中的旋转位置编码，建模 motion onset / reversal |
| GMR | General Motion Retargeting | 视频人体→机器人动作重定向（HTD-Refine 下游可选链路） |
