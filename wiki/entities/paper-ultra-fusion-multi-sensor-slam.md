...
**Ultra-Fusion 的取舍是「一个可配置估计器覆盖多平台」：与其为每种传感器组合维护一条管线，不如把 WIO/VIO/LIO/LVIO 放进同一滑窗，用图内门控而非子系统硬切换来对付退化。**

- 三个机制各对应一类失效：**可观测性感知初始化** 压缩启动延迟（平均 **4.642 s → 0.153 s**）；**FRS** 在因子层对退化模态门控降权（消融中对 LiDAR / 视觉 / 轮速分别带来显著 ATE 下降）；**OSC** 在线估时间偏移与外参（**±300 ms** 延迟下仍 ~4 cm 级，**10°** 外参扰动下优于 FAST-LIVO2 数个数量级）。
- 覆盖面是它的主要证据：M3DGR 夜间 **0.08 m**、长廊退化 **0.02 m**，M2DGR-Plus 平均漂移 **0.59% / 0.24 m**，GrandTour 四足 RTE 厘米级，MARS-LVIG 低空平均 rank/RMSE **1.5 / 1.47**；实时滑窗每步优化 **5.48–10.73 ms**。
- 适用边界：面向 **多模态 ITS 平台**（轮式 / 腿式 / 低空 UAV）；若只做极简 3D LIO，全模态 LVWIO + 在线标定的复杂度并不划算，[FAST-LIO](./fast-lio.md) 仍是更合适的轻量基线。
- 不要把结论外推成「传感器全开更好」：视觉在弱纹理 / 夜间仍会失效，LVIO 不必然优于 LIO，选型要回到 [LiDAR/VIO 选型对比](../comparisons/lidar-slam-lio-vio-selection.md)。
- 工程侧：代码与项目页已公开，M3DGR 提供退化与多模态评测数据；但真机落地仍需自行验证标定、时间同步与算力预算，ROS 2 / Nav2 衔接不在论文覆盖范围。
...
- [状态估计知识链汇总](../overview/hub-state-estimation.md) — 纵深入口
