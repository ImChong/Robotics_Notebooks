...
主路线偏"先打通主干"，下列二十六条纵深路线**各自独立**，从主路线某个阶段衔接出去，可单独阅读。
...
- [如果目标是 RSI（递归自我改进）](depth-rsi.md) —— 想让「改进机制本身」被自动改进，并把这套闭环用到真机策略上（起点：EURISKO 自改启发式，1983）
...
- [如果目标是模型架构（骨干族谱 → 感知编码 → 动作头 → 多模态基座 → 规模部署）](depth-model-architecture.md) —— 想知道一条策略该用什么网络、多宽多深、动作怎么出（起点：反向传播，1986）
