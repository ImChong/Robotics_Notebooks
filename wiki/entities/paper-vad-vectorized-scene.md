...
**全向量化把地图元素与 agent 轨迹变成显式实例约束，安全与速度同时受益，为 hustvl 系「实例优先」路线定基调。**

1. **四类向量表征** — 边界/车道/运动/自车向量直接在向量空间建模 ego–环境交互，省去密集栅格渲染与大量后处理。
2. **主数字** — 盘点称 VAD-Base 平均碰撞率较前作降约 29.0%，推理约 2.5×；VAD-Tiny 约 9.3×（以原论文为准）。
3. **路线影响** — 为后续 SparseDrive / DiffusionDrive 等 hustvl 系工作定下「实例优先」基调。
4. **对照定位** — 相对 UniAD 更省算力与级联误差，代价是工程栈更新；数据集为 nuScenes。
5. **局限** — 底层特征提取仍可能依赖稠密图；向量化质量受上游地图/检测影响，开环指标外推需谨慎；与纯语言/VLM 路线正交。
