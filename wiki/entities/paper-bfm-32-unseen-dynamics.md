...
**这篇处理的是基座落地之后的现实问题：负载、地面与硬件参数一变，能不能不重训就继续用——即零样本动力学适配。**

- 归位在 **04 Adaptation**（#32/41）：靠 task token、动力学适配或少量示范迁移，核心诉求是 **保留基座能力的同时低成本吸收新约束**，而非从零重训。
- 前提条件很硬：这类工作默认 **已有强预训练基座**，弱基座上加 adapter 收益有限——所以它检验的其实是基座质量。
- 与 [Any2Any](../entities/paper-any2any-cross-embodiment-wbt.md) 同题材但 **并非同一工作**：后者是本库对跨具身适配的深读页，本页只保留 survey 坐标，量化结论回原文。
