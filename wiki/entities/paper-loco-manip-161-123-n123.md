...
sources:
  - ../../sources/papers/loco_manip_161_survey_123_n123.md
...
**从嘈杂的感官输入中学习人形足球机器人的敏捷前锋技能** 收录于 [具身智能研究室 · 人形 Loco-Manip 161 篇长文](https://mp.weixin.qq.com/s/pACh9EhsISiyPGdiiR0C3A) **第 123/161** 篇，归类为 **06 特殊任务、接触规划与视觉闭环**。
...
- 这篇工作先从相机图像/多视角观测、本体状态与关节序列、仿真交互数据恢复场景、目标或运动表征，再用教师-学生知识迁移、PPO/RL 策略训练、全身控制器/WBC/MPC生成可执行动作命令。关键点是用特权信息训练教师策略，再把能力蒸馏到只能使用部署观测的学生策略。
- 人形 Loco-Manip 161 篇 **#123/161** · 特殊任务、接触规划与视觉闭环。
...
- 评测原始出处：[原文 / 项目页](https://humanoidsoccer.github.io)（见上方「核心信息」表「论文/项目」一行）。
...
**这篇工作的难点写在标题里：不是敏捷前锋技能本身，而是部署时只剩嘈杂感知——于是把能力先放进特权信息教师，再蒸馏给只看部署观测的学生。**

- 真正起作用的是教师-学生知识迁移这一层：噪声鲁棒性由蒸馏阶段承担，而不是靠在策略里堆更多滤波。
- 输入含仿真交互数据，而特权信息本来也只有仿真里拿得到，因此观测噪声在仿真中如何建模，直接决定学生策略在真机上的表现。
- 输出是可执行动作命令，仍需 WBC/MPC 承接；单篇工作不自动解决底层 WBC 鲁棒性。
- 适用边界是 **06 特殊任务、接触规划与视觉闭环** 中的高动态接触任务，前锋射门这类场景的结论不宜外推到静态操作。
- 本页为清单索引条目，量化 benchmark 与实机指标以 [原文 / 项目页](https://humanoidsoccer.github.io) 为准。
...
- 原始 source：[loco_manip_161_survey_123_n123.md](../../sources/papers/loco_manip_161_survey_123_n123.md)

## 参考来源

- [loco_manip_161_survey_123_n123.md](../../sources/papers/loco_manip_161_survey_123_n123.md) — 161 篇策展摘录
