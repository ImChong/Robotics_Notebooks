...
**TeCH 的关键取舍是「换表征而不换框架」：用对比时间距离直接替掉 FB 的值分解，换来更紧的跟踪与更小的根部漂移，代价是放弃 FB 那种任意奖励推理接口。**

- 真正起作用的是隐空间密集进度奖励（朝隐目标每步更近即给正奖励）配合随机/在线/专家三源混合的 z 采样：跟踪 E_mae 在 LAFAN1 为 0.1318、100-Style 为 0.1474，优于 SONIC 全轨迹，与 SONIC (TER.) 相当或略优。
- 成本是它最硬的卖点：单卡 GPU + 数小时 LAFAN1，相对 SONIC 的 128 GPU × 3 天降近两个数量级 GPU 小时——但跨论文比 wall-clock 必须注明硬件与 early-termination 口径。
- 零样本双接口（轨迹跟踪用滑动窗口聚合隐条件、目标到达用单点隐嵌入）让躺→坐→站这类不连续过渡可做，这正是纯跟踪方法难覆盖的区间；真机上大外力推搡后能自主站起。
- 不适用场景要认清：需要统一值函数做 arbitrary reward inference 时应回到 [BFM-Zero](./paper-bfm-zero.md) 的 FB 路线；时间距离目标偏好最少转移步数，真机响应可能更激进、略不平滑。
- 发布形态与落地：截至入库日以 Lab 成果页为主，方法已集成于 [UFO](./roboparty-ufo.md)（MJLab backend），arXiv 与独立代码仓库须以官方后续发布为准。
