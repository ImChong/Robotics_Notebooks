...
**把驾驶决策写成场景描述→分析→层次化规划的类人 Chain-of-Thought，用 Dual 快慢系统正视 VLM 延迟，补齐模块化与纯数值 E2E 的语义缺口。**

1. **三阶段 CoT** — 场景描述（环境/关键物体，如前方闪灯警车）→场景分析（物体意图与影响）→层次化规划（高层决策到轨迹航点）。
2. **Dual 快慢分工** — VLM 低频深度思考 + 传统感知规划高频实时控制，给出可工程落地的分工。
3. **主打长尾** — 面向警车/锥桶/动物等预定义类别之外的开放世界常识，定量以论文/项目页表格为准。
4. **相对相邻路线** — 相比 EMMA「一切皆语言」更易接传统栈；相比 Senna 决策/数值未显式解耦。
5. **开源与局限** — 部分开放（项目页），未核到可运行官方训练/推理仓库；VLM 存在延迟与幻觉风险，语言决策到数值轨迹的对齐误差需额外模块消化。
