...
**ConstrainedMimic 走的是「不动策略、加一层过滤」的路线：motion tracking 该怎么跟还怎么跟，安全交给动作之外的实时约束层兜底。**

- 关键机制是在策略输出上叠加约束投影/安全过滤，让跟踪参考时不突破关节、接触或碰撞边界，目标是动作表现与安全同时保住。
- 定位在 **D 全身跟踪基座**，属身体层工作，不替代 VLA/世界模型的任务规划。
- 本页在策展编译基础上做机制归纳，量化 benchmark、消融与实机指标以 [原文 / 项目页](https://danielpmorton.github.io/) 为准。
