...
**DecARt Leg: Design and Evaluation of a Novel Humanoid Robot Leg with Decoupled Actuation for Agile Locomotion** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：12_Hardware_Design），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**DecARt Leg 证明「拟人外观」和「敏捷」不是二选一：决定摆腿速度的是摆动惯量而不是膝盖朝向，把电机全部挪到膝上、用连杆把踝力矩远程传过去，前向膝的腿一样能拿到 Cassie 级 FAST。**

- 真正起作用的机制是**把腿长、腿俯仰与 2-DoF 踝全部解耦到膝上电机**：FAST 0.17 s vs 同一条腿的串联消融版 0.25 s，是最干净的因果证据——收益来自机构，不是来自整机尺度或调参。
- FAST 本身可能比这条腿更值得复用：它是**腿级、脱离上半身重量**的标尺，让 Cassie（0.24 s）与 GR1T2（0.24 s）可横向对比，且计算代码开源。
- 收益与控制范式无关：IK / QP-WBC / RL 三套控制器都跑通了免缆行走，说明这是在硬件层先替控制器减负，而非某个控制栈的专属结果。
- 适用边界：FAST 只刻画摆腿敏捷度这一个维度，机构在负载、传动效率等方向的代价本页未讨论；量化 benchmark、消融与实机细节以深读笔记与论文 PDF 为准（见 [参考来源](#参考来源)）。
