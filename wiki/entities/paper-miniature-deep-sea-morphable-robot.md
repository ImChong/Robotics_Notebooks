...
- **深渊级压力验证：** 在深海现场（含深渊级压力段，静水压 **>60 MPa**）完成游泳 / 爬行 / 滑翔三模态运动及模态间切换。
- **重量-深度解耦：** 压力等效设计消除刚性耐压舱，使机体重量近乎与深度无关，实现厘米级（"Miniature"）微型化。
- **材料与驱动稳定性：** 超弹性硅胶在高压下弹性模量变化 <5%（文献背景）；DEA / 液压软驱动在深渊压力下仍正常工作。
- **评测局限：** 现场测试为单次 / 少次任务，软材料在持续极端压力下的疲劳寿命尚未系统量化（详见下文局限与风险）。

## 结论

**真正的突破不是运动模态多，而是压力等效设计把「耐压舱重量随深度增长」这条约束整个删掉，让厘米级软体机器人得以进入深渊。**

- 起作用的机制是内外压平衡：开放/充液腔体 + 压力不敏感的 DEA/液压软驱动 + 充液浸没的电子封装，把结构工程问题换成材料与驱动器兼容性问题，重量与深度就此解耦。
- 现场验证的含金量在于三模态（游泳/爬行/滑翔）及其切换是在真实深海、含 **>60 MPa** 深渊级压力段完成的，不是压力舱里的静态耐压试验。
- 代价写在驱动上：DEA 与液压软驱动的功率密度远低于刚性电机，速度与载荷受限，深海巡航效率需要仔细权衡。
- 复现门槛极高：截至 2026-07-20 无官方代码仓库或 CAD 公开，且全功能验证需要深海测试船与作业支持，普通实验室做不了。
- 在北航文力组序列中的位置：[章鱼软臂（2023）](./paper-octopus-inspired-esoam-soft-arm.md) 解决软体传感、[两栖印鱼（2022）](./paper-aerial-aquatic-remora-hitchhiking-robot.md) 解决跨介质运动，本工作把极端环境适应推到深渊级压力。
