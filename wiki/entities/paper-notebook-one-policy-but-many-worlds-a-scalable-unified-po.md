...
1. **多地形专家数据采集**：对每类地形先用专家（含奖励工程）采集示范数据——这一步仍需专家与奖励。
2. **自回归扩散世界模型训练**：用专家数据训练自回归扩散 WM，学会生成未来身体状态轨迹（AMP 判别器约束长时风格）。
3. **未来状态生成**：WM 生成未来状态作为参考，输出的是状态而非最终力矩，保留低层纠偏空间。
4. **goal-conditioned RL 统一策略**：以生成的未来状态为目标条件，学一个统一跟踪策略覆盖多地形，减少混合地形的重复奖励工程。
5. **异步部署**：控制约 50 Hz、感知/生成约 20 Hz 异步运行，生成必须可丢弃并重规划。

一句话读法：要点是「生成状态参考 + 统一跟踪」，不是消灭前期专家。
...
| 维度 | 本工作（DreamPolicy / P018） | [去噪 WM Loco / P017](./paper-notebook-advancing-humanoid-locomotion-mastering-challeng.md) | [DreamWaQ](../methods/dreamwaq.md) | [World Action Models](../concepts/world-action-models.md) |
|------|-----------------------------|--------------------------------------------------------------------------------------------|------------------------------------|-----------------------------------------------------------|
| 方法族 | 自回归扩散世界模型 + goal-conditioned RL | 去噪世界模型 + RL 策略 | 隐式地形想象（本体编码器）单阶段 RL | 世界-动作模型族（模型视角综述） |
| 世界模型角色 | **生成**未来身体状态轨迹作参考 | 从带噪观测**去噪**出潜在动态/表征 | CENet 从本体历史**推断**隐式地形上下文 | 联合建模世界演化与动作 |
| 关键假设 | 多地形专家数据可训出可生成的 WM | 观测噪声/地形不确定性可显式建模并剥离 | 盲走：仅本体历史即可支撑鲁棒行走 | 世界预测与动作可耦合学习 |
| 优化目标 | 统一多地形策略、减少重复奖励工程 | 对噪声观测/地形不确定性的鲁棒表征 | 无外感知的鲁棒行走 | 给族谱与评测坐标 |
| 关系/取舍 | 生成误差会累积；异步生成须可丢弃 | 强调鲁棒表征而非生成 | 最轻量的盲走基线 | 提供分类坐标，非单一系统 |
