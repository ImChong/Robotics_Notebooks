...
**VPP 是「先预测视觉未来、再解码动作」这条级联路线的代表：真正被复用的是视频扩散学到的当前+未来视觉表征，动作侧只是一层隐式逆动力学。**

- 关键取舍就在级联本身：模块可拆可换、工程上易复用，代价是预测段误差会沿链路传给动作段。
- 本页为 **清单索引策展编译**；Calvin 与真机灵巧操作的提升口径以原文 PDF / 项目页为准，不要把公众号摘要当可复现指标。
- 复现前需核对 License 与权重——开源 WM 项目的引用量与下游控制增益之间没有简单线性关系。
