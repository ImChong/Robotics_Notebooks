# BeyondMimic: From Motion Tracking to Versatile Humanoid Control via Guided Diffusion

> 来源归档（ingest · 人形 RL 身体系统栈 42 篇 · 第 15/42）

- **标题：** BeyondMimic: From Motion Tracking to Versatile Humanoid Control via Guided Diffusion
- **类型：** paper
- **系统栈层：** 02 参考跟踪 · 通用控制
- **机构：** 伯克利；斯坦福大学
- **项目/论文链接：** <https://arxiv.org/abs/2508.08241> · 正式发表 [DOI 10.1126/scirobotics.adx8924](https://doi.org/10.1126/scirobotics.adx8924)（*Science Robotics*，2026-08-26）
- **SciRob 中文导读：** [wechat_shenlan_beyondmimic_science_robotics_2026-09-10.md](../blogs/wechat_shenlan_beyondmimic_science_robotics_2026-09-10.md)
- **索引来源：** [具身智能研究室 · 42 篇 RL 运动控制长文](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)（<https://mp.weixin.qq.com/s/hz9JXtJeUPRfUGzfD-pZuA>）
- **原始抓取：** [wechat_humanoid_rl_42_survey_2026-05-26.md](../raw/wechat_humanoid_rl_42_survey_2026-05-26.md)（Agent Reach + Camoufox）
- **入库日期：** 2026-05-26
- **一句话说明：** BeyondMimic 的完整题目是 From Motion Tracking to Versatile Humanoid Control via Guided Diffusion。它放在 OmniTrack、RGMT 旁边很合适，因为它同样在处理“参考动作如何变成可执行控制”这个问题，但切入点更偏生成式。

## 核心摘录（策展，非全文）

- **在身体系统栈中的位置：** 02 参考跟踪 · 通用控制，编号 **15/42**。
- **公众号导读要点：** BeyondMimic 的完整题目是 From Motion Tracking to Versatile Humanoid Control via Guided Diffusion。它放在 OmniTrack、RGMT 旁边很合适，因为它同样在处理“参考动作如何变成可执行控制”这个问题，但切入点更偏生成式。
- **读者动作：** 方法细节以论文 PDF / 项目页为准；总框架见 [人形 RL 身体系统栈](../../wiki/overview/humanoid-rl-motion-control-body-system-stack.md)。

## 对 wiki 的映射

  - [beyondmimic](../../wiki/methods/beyondmimic.md)

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_rl_motion_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)
- 姊妹篇 AMP 专题：[wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)
