...
**OmniH2O 的野心不在动作跟得多准，而在把 kinematic pose 立成「通用身体接口」——让 VR、RGB 摄像头、语言指令与 GPT-4o / diffusion policy 都能从同一个入口驱动人形。**

- 真正的论证是分工而非精度：上层系统很难直接输出每个关节的力矩，需要一个中间接口把「想做什么」翻译成可稳定执行的全身姿态与运动目标；底层控制器负责从稀疏输入补齐全身协调、接触稳定与灵巧手动作。
- 相对 H2O 的推进在覆盖面：从证明实时全身遥操作可行，扩到灵巧手、移动操作、户外行走与多种输入来源——把接口 **做宽** 是这篇的主线。
- 依赖关系值得记住：接口足够稳，语言模型与 VLA 才可能可靠接上来；因此遥操作/重定向不是「训练前脚本」，其质量上限往往早于 RL 策略决定。
- 本页为 42 篇栈策展编译（#08/42，01 数据 · 重定向 · 遥操作），量化 benchmark、消融与实机指标以原文 PDF / 项目页为准。
