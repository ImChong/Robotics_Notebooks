...
arxiv: "2604.25859"
venue: "2026"
summary: "Privileged Foresight Distillation (PFD) compares two attention masks on the same world-action backbone, then learns the future-enabled change in action velocity through an output adapter. Deployment uses only the current"
related:
  ...
  - ../tasks/locomotion.md
sources:
  - ../../sources/papers/rcl_awesome_wam_2604_25859_privileged-foresight-distillation-zero-c.md
...
**Privileged Foresight Distillation: Zero-Cost Future Correction for World Action Models** 收录于 [Awesome World-Action Models (RCL)](https://github.com/rcl-robotics/Awesome-World-Action-Models) **第 455/564** 篇，分组 **WAMs**。本页为知识库 **策展索引级** 详情节点；方法细节与量化指标以原文 PDF / 项目页为准。

## 一句话定义

Privileged Foresight Distillation (PFD) compares two attention masks on the same world-action backbone, then learns the future-enabled change in action velocity through an output adapter. Deployment uses only the current frame and corrected action denoising. The reported LIBERO mean improves by 1.15 percentage point...
...
- 评测口径与具体数值以 [原文 / 项目页](https://arxiv.org/abs/2604.25859) 为准。
...
**本条目的站内价值是把「Privileged Foresight Distillation」从 RCL Awesome WAM 列表提升为可链接的知识节点，并保留清单 Contribution 作为阅读锚点。**
...
- [`sources/papers/rcl_awesome_wam_2604_25859_privileged-foresight-distillation-zero-c.md`](../../sources/papers/rcl_awesome_wam_2604_25859_privileged-foresight-distillation-zero-c.md) — 本条目策展摘录
...
- [原文](https://arxiv.org/abs/2604.25859)
