...
| 维度 | 本工作（ExBody） | [ExBody2](./paper-loco-manip-161-007-exbody2.md) | [Human2Humanoid](./human2humanoid.md) |
|------|------------------|--------------------------------------------------|----------------------------------------|
| 方法族 | RL 全身控制，上身表达跟踪 + 下身命令约束 | 同源后续工作，扩展到更完整的全身运动跟踪 | 人到人形的全身模仿/遥操作管线 |
| 关键假设 | 腿部不逐帧复制人体参考，只需完成根部速度/朝向目标 | 放宽上下身拆分，追求更贴近参考的全身表现 | 需把人体动作重定向为可执行的机器人全身参考 |
| 输入/输出 | 观测省略机身线速度与绝对高度，输出 19 个关节目标 | 以更强跟踪目标为导向的观测-动作接口 | 以人体动作（含实时来源）驱动全身跟踪 |
| 优化目标 | 保留上身表现力的同时保证下肢可部署、可行走 | 提升整体跟踪保真度与运动多样性 | 让机器人实时复现人的全身动作 |
| 关系/取舍 | 牺牲腿部精确追参考，换取平衡与真机稳定 | 相对本工作追更高保真，但对下肢约束更强 | 侧重人-机对齐与遥操作闭环，而非只跟踪库中片段 |

参考通用背景见 [全身协调](../concepts/whole-body-coordination.md)。
