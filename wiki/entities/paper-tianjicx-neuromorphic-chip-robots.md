...
**TianjicX 把「要 ANN 精度还是要 SNN 功耗」的二选一，改写成运行时可调度的资源分配问题，并用真实轮式机器人的三任务并发把这套调度证到系统级，而不是停在芯片 benchmark。**

- 真正起作用的是 **时空弹性**：空间弹性让计算核在 ANN/SNN 模式间动态重配，时间弹性让各任务时隙比例运行时可调，二者合起来才使三任务在计算资源竞争下不崩溃。
- 可迁移的设计原则是分工而非芯片本身：**高精度、低频决策（视觉跟踪、语音识别）交 ANN 核，高频、时序响应（距离传感脉冲）交 SNN 核**——这条准则在其他低功耗边缘平台上同样成立。
- 相对 Tianjic（Nature 2019）已证明的 ANN+SNN 单芯片共存，本工作的增量在 **调度而非共存**，机器人演示也从 2 任务扩到 3 任务并发。
- 主要风险在生态与规模：**SNN 工具链成熟度远低于 PyTorch/TensorFlow**，ANN 核受片上资源约束、复杂场景精度低于专用加速器，且截至入库日 **无公开代码或 IP 授权**，学术复现只能依赖论文描述。
- 平台边界也需注意：演示限于轮式机器人，足式/人形等动力学复杂平台尚未验证；后续 [NeuroGPR](./paper-neurogpr-brain-inspired-place-recognition.md) 才把混合网络用于四足场所识别。
