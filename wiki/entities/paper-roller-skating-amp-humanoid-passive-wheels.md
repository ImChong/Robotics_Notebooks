...
**这篇的真正贡献不在「让人形穿上轮滑」，而在于：被动滚动接触必须单独建模，且 Pump / Push 两种推进机制不能共用一套先验与奖励。**

- 起作用的关键是 **9 片圆柱轮碰撞模型**：整球近似会让约一半体积提供虚假侧向支撑，策略据此学到的平衡在真机上不成立。
- 两种 gait 用 **完全独立** 的 MoCap 参考集、判别器与策略；Push 还额外加了轮腾空、支撑切换等课程化奖励——AMP 风格先验替代的是「手工相位规则」，不是任务奖励设计本身。
- 适用边界：Pump 在 $0.10$–$0.45$ m/s 完成率 0.77–0.81，$0.50$ m/s 掉到 0.53；Push 虽能跑满 20 s，实际速度却系统性偏高——速度跟踪精度是当前主要失败模式。
- 切片轮仍是近似，且缺乏标准化轮滑质量基准，跨物理引擎（Isaac Lab / MuJoCo）的速度误差尚未消除。
- 工程侧：截至入库日项目页 **未列代码 / 权重 / 数据集**，按未开源看待，可复现的只有方法框图与真机视频。
- 与 [SKATER](./paper-notebook-skater-synthesized-kinematics-for-advanced-trave.md) 对照：后者做合成回形轮滑运动学与效率，本页押注 **接触建模 + 风格先验 + 双 gait 真机**。
