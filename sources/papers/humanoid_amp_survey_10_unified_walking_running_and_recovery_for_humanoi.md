# Unified Walking, Running, and Recovery for Humanoids via State-Dependent Adversarial Motion Priors

> 来源归档（ingest · 人形 AMP 运动先验 19 篇 · 第 10/19）

- **标题：** Unified Walking, Running, and Recovery for Humanoids via State-Dependent Adversarial Motion Priors
- **类型：** paper
- **AMP 叙事段：** 02 人形走跑
- **机构：** 香港大学等
- **论文链接：** <https://arxiv.org/abs/2605.18611>
- **索引来源：** [具身智能研究室 · AMP 专题长文](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)（<https://mp.weixin.qq.com/s/YZsm3855iP3TNTTt1aou7w>）
- **原始抓取：** [wechat_humanoid_amp_19_survey_2026-05-26.md](../raw/wechat_humanoid_amp_19_survey_2026-05-26.md)（Agent Reach + Camoufox）
- **入库日期：** 2026-05-26
- **深读归档：** 2026-06-25 — [unified_walk_run_recovery_sdamp_arxiv_2605_18611.md](unified_walk_run_recovery_sdamp_arxiv_2605_18611.md)
- **一句话说明：** 这篇是我认为最贴近“人形 AMP 未来”的论文之一。

## 核心摘录（策展，非全文）

- **在 AMP 四段地图中的位置：** 02 人形走跑，编号 **10/19**。
- **公众号导读要点：** 这篇是我认为最贴近“人形 AMP 未来”的论文之一。
- **方法要点（arXiv 编译）：** SD-AMP 投影重力门控双判别器；三条 LAFAN1 参考；G1 单策略走跑起身；50 Hz ONNX 无部署 FSM。

## 对 wiki 的映射

- [paper-unified-walk-run-recovery-sdamp](../../wiki/entities/paper-unified-walk-run-recovery-sdamp.md) — AMP 专题 #10 策展页
- [paper-unified-walk-run-recovery-sdamp](../../wiki/entities/paper-unified-walk-run-recovery-sdamp.md) — 主深读实体页
- [amp-mjlab](../../wiki/entities/amp-mjlab.md)、[amp-reward](../../wiki/methods/amp-reward.md)

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)
- 姊妹篇 42 篇栈：[wechat_embodied_ai_lab_humanoid_rl_motion_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)
- arXiv 深读：[unified_walk_run_recovery_sdamp_arxiv_2605_18611.md](unified_walk_run_recovery_sdamp_arxiv_2605_18611.md)
