...
**UniAct 的取舍是「一条链路到底」：把语言指令→可跟踪身体目标→低层控制器目标的策略网络与控制模块放进同一训练/部署链路，用消除断点换取高层意图与低层动作的一致性。**

- 起作用的是链路合并本身，而不是某个单点模型：输出同时是全身轨迹/动作序列与低层控制器目标，因此它被归到 **01 运控基座与通用全身跟踪** 而非上层规划分组。
- 合并链路不等于底层鲁棒：loco-manip 单篇工作不自动解决 **WBC 鲁棒性**，仍须与运控/接触控制类工作对照阅读。
- 本页为 161 篇 **#023/161** 清单索引条目，未搬运原文量化 benchmark 与实机指标；数值以 [原文 / 项目页](https://jnnan.github.io/uniact/) 为准。
