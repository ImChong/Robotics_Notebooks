...
| 路线 | 输入 | 输出语义 |
|------|------|----------|
| **本文** | 单张 RGB | **生物力学关节角 + 人体尺度** |
| 裸 [SAM 3D Body](./sam-3d-body.md) | 单张 RGB | MHR 网格 + 运动学树角 |
| 图像直接回归生物力学 | 单帧 | 生物力学角 |
| 轨迹级推理时优化 | **整段序列** | 生物力学角 |
| [GENMO](../methods/genmo.md) 等时序 SMPL 生成 | 视频 | SMPL 参数 |
| [WiLoR](../methods/wilor.md) | 单图手部 | 手部网格 / 姿态 |

逐条读法：

- **裸 SAM 3D Body** — 同一编码器；本文是在它上面 **加一头**，只有需要临床语义角时才值得多这一层。
- **图像直接回归生物力学** — 本文 **优于** 这一档，差别在有没有 IK 教师提供的高质量伪标签。
- **轨迹级推理时优化** — 本文 **略逊于** 这一档；对方靠时序一致性与在线优化换精度，代价是延迟与算力。
- **GENMO 等时序 SMPL 生成** — 语义不同：SMPL 角 ≠ 生物力学角，不能互相当输入直接喂。
- **WiLoR** — 部位互补，不重叠；全身生物力学 + 手部细节可拼成一套上游。
