...
**ULC 把「统一且细粒度的全身控制器」做成可路由的技能/专家结构：高层在执行中选择与组合，低层直接输出关节位置/力矩命令，避免为每类移动操作任务各做一套控制器。**

- 起作用的机制是分层技能/专家策略加高层路由，配合 PPO/RL 训练与 WBC/MPC，把接触力/触觉信号与本体状态一起转成可跟踪的身体目标。
- 定位在 **02 上半身中心控制与移动操作接口**：它提供的是移动操作的控制接口层，不承担任务级规划。
- 适用边界：本页为索引级策展条目，机构与量化指标须回到原文 / 项目页；单篇控制器工作也不自动解决底层 WBC 鲁棒性。
