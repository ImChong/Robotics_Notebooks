...
### 毫米波 / 相控阵雷达
- **适用场景**：雾雨、强光下 **主动测距** 仍可用；开源硬件参考见 [AERIS-10](../entities/aeris-10-plfm-radar.md)。
- **失效模式**：多径反射、地杂波、金属围栏导致 **鬼影点迹**；角分辨率通常低于旋转 LiDAR。
- **排障**：
  - 检查 MTI/CFAR 门限与波束扫描范围是否覆盖任务区。
  - 与 GPS/IMU 融合时确认时间戳与杆臂标定；勿把稀疏雷达点迹直接当稠密点云喂给地面分割算法。
