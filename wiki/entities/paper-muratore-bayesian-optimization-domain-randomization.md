...
- **本页为索引级节点**（FreeDof 44 篇梳理 [15/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页要点是「BO 搜索随机化超参，以任务表现作为黑盒目标」，对应证据是 BO 迭代过程的黑盒目标曲线，以及与随机 / 网格搜索基线的**样本效率**对比。
...
| **横比口径** | BO 的收益与随机化参数维度、单次评估噪声强相关；低维低噪声下的加速比不可外推到高维设置。 |
