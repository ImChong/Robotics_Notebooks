...
**EgoMimic 的赌注押在数据来源而不是模型结构：它主张 Aria 第一视角视频加 3D 手部轨迹可以和机器人遥操作数据进同一个 IL 训练循环，而不只是被当成高层意图读一遍。**

- 关键判据是 **缩放效率**——本页口径为人类数据的缩放效率高于等量机器人数据；这决定共训值不值得做，比单任务成功率更能说明这条路线的价值。
- 代价全在跨具身 gap：人手与机器人的运动差异、外观差异、数据分布差异都要显式处理，**Ego 视频不天然等于策略数据**，须经重建、对齐、重定向与物理过滤。
- 本页为 Ego 9 篇专题的 **策展索引级** 条目（归 [02 人→机器人](../overview/ego-category-02-human-to-robot.md)），量化 benchmark、消融与实机指标以原文 PDF / 项目页为准。
...
- [机器人论文阅读笔记：EgoMimic](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/06_Manipulation/EgoMimic_Scaling_Imitation_Learning_via_Egocentric_Video/EgoMimic_Scaling_Imitation_Learning_via_Egocentric_Video.html)
