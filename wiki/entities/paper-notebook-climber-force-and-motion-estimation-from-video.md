...
**Climber Force and Motion Estimation from Video** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：14_Human_Motion），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这项工作把「受力测量」从岩壁上的传感器挪进了视频里——真正的产出不是更准的姿态，而是一条非侵入地拿到人-环境接触力的路径。**

- 关键在于运动与受力的 **联合** 估计：单靠 3D 姿态回答不了「发力」问题，把人体姿态估计与接触/力推断绑在一起，才把运动学与动力学接上。
- 场景选择本身是方法的一部分：攀岩是强接触、离地、全身的运动，人-岩交互又发生在明确的岩点上，适合作为力推断的载体。
- 落地价值主要在数据侧：免力传感器意味着可以低成本扩充「带力标注」的人类数据，直接对接人形 loco-manip 对外力来源建模的诉求。
- 适用边界：输入是普通单目视频，产出定位为面向运动科学、教练辅助与机器人攀爬的分析工具，而不是控制器；精度与失败模式本页未给，以深读笔记与论文 PDF 为准。
