...
**DiffCoTune 把 sim-to-real 从「单向迁移」改写成「双向适配」：不只调控制器，也调仿真器，用可微仿真把原本靠手工试凑的部署调参变成有梯度方向的优化。**

- 真正起作用的是 **协同调参 + 多步目标 + 交替优化**：轮流优化仿真器参数与控制器参数，比只调控制器一边、或比无方向的域随机化更有针对性。
- 最有价值的指标是样本效率——目标域 **少数几次 rollout** 内完成迁移；对真机试验昂贵且危险的人形/双足而言，这比最终性能高一点更关键。
