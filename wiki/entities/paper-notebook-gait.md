...
**GAIT: Legged Robot Proprioceptive State Estimation with Attention over Inertial-Leg Tokens** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：09_State_Estimation），深读笔记已完成。本页为 **深读笔记索引实体**，正文要点编译自笔记；细节以笔记页与论文 PDF 为准。
...
| 深读笔记 | <https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/09_State_Estimation/GAIT__Legged_Robot_Proprioceptive_State_Estimation_with_Attention_over_Inertia/GAIT__Legged_Robot_Proprioceptive_State_Estimation_with_Attention_over_Inertia.html> |
...
**GAIT 的真正贡献不是又一个学习式状态估计器，而是把「测量的传感器语义」写进网络输入结构，用注意力权重顶替掉接触估计器这条最脆弱的链路。**

- 起作用的机制是 **Inertial-Leg 分词 + Perceiver IO 交叉注意力**：一条腿只有触地时的前向运动学速度才可靠，注意力自然学成「按接触状态重新赋权」，于是显式接触估计与打滑剔除逻辑都可以省掉。
- 泛化证据落在 **跨步态** 上：只用 trot 训练即可在 bound / pace / pronk 等未见步态上超越「拼大向量」的 NMN——说明收益来自输入结构，而不是更大的模型。
- 分工边界要守住：网络只输出机身速度与不确定度，完整位姿交给 IEKF，几何一致性由滤波保证；把它当端到端位姿估计器用属于误用。
- 部署可行性是硬指标：0.12 MFLOPs / 步、0.7 ms 延迟（比标准自注意力低约 2.8×、比 NMN 低约 3.3×），机载算力受限的四足 / 人形可直接跑。
- 局限：纯本体感知路线（不含视觉 / LiDAR），且截至当前未见公开源码，复现需以深读笔记与论文 PDF 为准。
...
- [机器人论文阅读笔记：GAIT](https://imchong.github.io/Robot_Learning_Paper_Notebooks/papers/09_State_Estimation/GAIT__Legged_Robot_Proprioceptive_State_Estimation_with_Attention_over_Inertia/GAIT__Legged_Robot_Proprioceptive_State_Estimation_with_Attention_over_Inertia.html)
