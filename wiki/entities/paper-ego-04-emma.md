...
**用人类移动操作 Ego 数据与静态机器人数据共训，绕开大规模移动机器人遥操作成本，把「人如何在空间里组织身体和任务」这一最缺的部分补上。**

1. **动机** — 移动操作贵在机器人既动手又移动身体，遥操作成本远高于固定机械臂；EMMA 用人类移动操作 + 静态机器人数据共训规避。
2. **Ego 的价值** — 只要任务涉及人怎么走过去、靠近、把身体对准操作对象，第一视角就比外部视频更贴近执行过程。
3. **清单索引信息** — 具身智能研究室 Ego 9 篇第 04/9，归类 02 人→机器人，2025 arXiv（2509.04443），项目页 ego-moma.github.io。
4. **常见误区** — Ego 视频不天然等于机器人策略数据，须经重建、对齐、重定向与物理过滤。
5. **命名注意** — 勿与 Waymo 驾驶端到端模型 EMMA（arXiv:2410.23262）混淆；量化 benchmark 与实机指标以原文 PDF / 项目页为准。
