...
**这篇工作的主张是「从专家到通才」：与其为每类任务各留一个专家控制器，不如把世界模型/视频预测与全身控制器（WBC/MPC）放进同一条训练与部署链路，消掉高层目标到低层动作之间的断点。**

- 真正起作用的是 **链路一体化** 而非单点更强：预测模块与全身控制器共享同一路径，减少接口转换处的信息损失。
- 输入侧仍以 **相机图像/多视角观测 + 本体状态与关节序列 + 仿真交互数据** 为主，输出是全身轨迹与低层控制器目标。
- 分类坐标是 **01 运控基座与通用全身跟踪**，读它是为了看通用全身控制的组织方式；量化对比以 [原文 / 项目页](https://beingbeyond.github.io/BumbleBee/) 为准。
