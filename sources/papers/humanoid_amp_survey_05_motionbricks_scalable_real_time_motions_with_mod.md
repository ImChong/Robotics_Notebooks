# MotionBricks: Scalable Real-Time Motions with Modular Latent Generative Model and Smart Primitives

> 来源归档（ingest · 人形 AMP 运动先验 19 篇 · 第 05/19）

- **标题：** MotionBricks: Scalable Real-Time Motions with Modular Latent Generative Model and Smart Primitives
- **类型：** paper
- **AMP 叙事段：** 01 分布约束与先验组件化
- **机构：** NVIDIA、ETH Zürich、UT Austin
- **论文链接：** <https://arxiv.org/abs/2604.24833>
- **项目页：** <https://nvlabs.github.io/motionbricks/>
- **索引来源：** [具身智能研究室 · AMP 专题长文](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)（<https://mp.weixin.qq.com/s/YZsm3855iP3TNTTt1aou7w>）
- **原始抓取：** [wechat_humanoid_amp_19_survey_2026-05-26.md](../raw/wechat_humanoid_amp_19_survey_2026-05-26.md)（Agent Reach + Camoufox）
- **入库日期：** 2026-05-26
- **深读归档：** 2026-06-25 — [motionbricks.md](motionbricks.md)
- **一句话说明：** MotionBricks 比 Kimodo 更靠近机器人。

## 核心摘录（策展，非全文）

- **在 AMP 四段地图中的位置：** 01 分布约束与先验组件化，编号 **05/19**。
- **公众号导读要点：** MotionBricks 比 Kimodo 更靠近机器人。
- **方法要点（arXiv 编译）：** Smart Primitives + 模块化潜空间；~15k FPS / 2ms；35 万+ BONES-SEED 片段；GR00T WBC 栈；G1 真机。

## 对 wiki 的映射

- [motionbricks](../../wiki/methods/motionbricks.md) — AMP 专题策展页
- [motionbricks](../../wiki/methods/motionbricks.md) — 主方法页（Smart Primitives 细节）
- [kimodo](../../wiki/entities/kimodo.md) — 同段生成先验对照

## 参考来源（原始）

- 微信公众号编译：[wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_amp_motion_prior_survey.md)
- 姊妹篇 42 篇栈：[wechat_embodied_ai_lab_humanoid_rl_motion_survey.md](../blogs/wechat_embodied_ai_lab_humanoid_rl_motion_survey.md)
- arXiv 深读：[motionbricks.md](motionbricks.md)
