...
最后更新：2026-07-26（v30 全数完成后新建：聚焦「机器人视觉感知栈选型闭环」知识链——把近周密集 ingest 的一批**目标检测 / 分割 / 2D→3D 语义建图**资料，从分散的实体页沉淀为一条贯通的「传感与标定 → 2D 检测/分割选型 → 2D→3D 提升与语义建图 → 下游策略消费」感知栈选型链，补感知栈层间矛盾检测规则与专题视图）
...
| 社区结构 | 16 社区，最大社区占 21.6%（`community_quality_warning: false`） | **保持 ≤ 25%，新增专题不破坏均衡** |
| 技术专题 | 执行器驱动链选型闭环链路（V30 交付） | **建立"机器人视觉感知栈选型闭环"知识链** |
| 图谱专题视图 | V30 扩至 20 项（新增「执行器驱动链」） | **新增「机器人感知栈」专题至 21 项** |
...
- [ ] **机器人视觉感知栈选型闭环知识链 (+2)**：
    ...
    - [ ] `wiki/concepts/2d-to-3d-semantic-lifting-gap.md`（「2D 检测/分割结果」↔「可供策略消费的 3D 语义几何」取舍概念页：明示把 2D 框/掩码提升到 3D 语义地图时的信息损失与歧义——尺度不确定、遮挡、时序一致性、类别语义 vs 几何占据的分离——并把这条 gap 讲成「感知输出能否被下游导航/操作忠实消费」的物理根因；配 lifting 成立条件表、缩小 gap 的三条工程路线（深度融合 / 多视角一致性 / 语义-几何联合建图）与常见误判速查）。与 Query 页双向回链。
...
- [ ] **图谱页"机器人感知栈"专题视图**：
    - [ ] `docs/topic-filters.js` 单一事实源新增「机器人感知栈」专题（`perception-stack`，👁 emoji），复用 path 片段并集机制（`detection` / `segment` / `perception` 等干净片段，与既有 `vision-backbone` 专题保持最小重叠——后者聚焦特征骨干，本专题聚焦任务级感知流水线）并用 `ids` 显式纳入未被片段命中的感知页（`robot-perception-stack-selection-loop` / `2d-to-3d-semantic-lifting-gap` / `ultralytics` / `rf-detr` / `paper-yolo-unified-realtime-detection` / `paper-segment-anything` / `paper-sam2` / `findanything` / `cmu-mscv-semantic-3d-mapping` / `object-detection-model-selection` 等）；同步在 `docs/graph.html` `#filter-topic-chips` 增加对应 chip。专题汇总枢纽页 `wiki/overview/topic-perception-stack.md` 需新建（从相关感知/query 页交叉回链），`graph-stats.json` 0 orphans。专题视图落稳后截图归档至 `.cursor-artifacts/screenshots/graph-topic-perception-stack.png`。
- [ ] **详情页"同专题相关页"提示**：
    - [ ] 复用 `docs/topic-filters.js` 单一事实源（`renderMetaTopicBadges` → `topicsForNode` 已数据驱动），感知/新建页命中「机器人感知栈」专题时自动渲染对应轻量徽标 + 跳转 `graph.html?topic=perception-stack`（空态降级隐藏）。P3① 把 `perception-stack` 写入单一事实源后，详情页「所属专题」徽标行即自动联动；选一页感知实体页端到端验证并归档截图至 `.cursor-artifacts/screenshots/detail-topic-perception-stack.png`。
