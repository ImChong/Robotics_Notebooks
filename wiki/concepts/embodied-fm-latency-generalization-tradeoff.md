...
| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| VLA | Vision-Language-Action | 端到端视觉–语言–动作策略；本页取舍线上的高时延一端 |
| VLM | Vision-Language Model | 视觉–语言感知模型；分层方案里的低频规划/感知层 |
| WM | World Model | 世界模型；推演步长长、τ 高，天然不进实时控制回路 |
| WBC | Whole-Body Control | 全身控制；分层方案里承接高频控制的轻量底座 |
| FLOPs | Floating-Point Operations | 浮点运算量；随参数规模近似线性↑，推高单步推理时延 τ |
| MINT | Mimic Intent, Not Just Trajectories | 频域意图分词方法（RSS 2026），破边界路线 ② 代表 |
...
- [ReflexVLA](../entities/paper-reflexvla.md) — 1B 侧预测+时序+CUDA Graph；ReflexBench 把延迟写进动态任务评测（代码待开放）
