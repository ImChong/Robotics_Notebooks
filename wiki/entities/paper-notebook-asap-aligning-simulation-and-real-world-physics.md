...
**ASAP 把 sim2real 差距当成「可学习的残差动力学」来处理：仿真里先学跟踪，真机 rollout 训一个 delta 动作模型，冻结后回灌进仿真做物理对齐微调，部署时再把它摘掉。**

- 真正起作用的是「回灌」这一步：论文的对照显示，仅学习 delta 动力学而不用它在仿真里微调策略并不够，纯 SysID 与纯域随机化同样落后。
