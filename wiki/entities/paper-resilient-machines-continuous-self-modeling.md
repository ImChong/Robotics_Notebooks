...
summary: "Resilient Machines (Self-Modeling)（具身世界模型六路线专题）：从动作—感觉关系推断自身结构，损伤后搜索替代行为；身体也是世界的一部分。"
---

# Resilient Machines (Self-Modeling)

**Resilient Machines (Self-Modeling)** 收录于 [具身智能研究室 · 具身世界模型六路线综述](https://mp.weixin.qq.com/s/mmIJRp9g6NqblMCjd9D5GQ) **规划主导型** 段。本页为知识库 **策展编译** 详情节点；量化指标以原文 PDF / 项目页为准。

## 一句话定义

从动作—感觉关系推断自身结构，损伤后搜索替代行为；身体也是世界的一部分。
...
|----|------|
| **路线** | 规划主导型 |
| **出处** | [arXiv:1903.00572](https://arxiv.org/abs/1903.00572) |
| **文内角色** | 从动作—感觉关系推断自身结构，损伤后搜索替代行为；身体也是世界的一部分。 |

## 结论

**Resilient Machines (Self-Modeling) 在六路线框架下的读法：先看预测输出被谁消费，再看是否改善真实行动——而非只看网络新旧或画面观感。**
...
- **本页无量化数字**：六路线综述只给出该工作在 taxonomy 中的定位，未转述实验表格；成功率、消融与实机协议以 [arXiv:1903.00572](https://arxiv.org/abs/1903.00572) 为准。
...
- **同路线邻接工作**（综述 **规划主导型** 段）：[PETS](./paper-pets-probabilistic-dynamics-mpc.md)、[MuZero](./paper-muzero-planning-latent-dynamics.md)。
- **对照要点**：本文的预测对象是 **机器人自身的形态与动力学**（损伤后重建自模型再规划），而同路线多数工作预测的是 **环境如何响应动作**——自模型与环境模型的失效方式不同。
...
- [arXiv:1903.00572](https://arxiv.org/abs/1903.00572)
