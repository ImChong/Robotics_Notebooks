...
**守门这类极短反应窗的动态交互，胜负手是让任务目标与运动先验受同一个路由信号（球落点区域）条件化，而不是把扑救动画跟得更准。**

- 真正起作用的是 **双侧分区**：区域任务奖励（动态末端目标 + 区域调制 $\nu^{(\mathcal{R})}$）与 per-region 判别器 $D^{(\mathcal{R})}$ 同步切换；去掉运动约束成功率跌至 ~32%，去掉 AMP 分区则 $E_{\text{match}(\mathcal{R})}$ 从 67.8% 降到 25.7%——区域–风格对齐不是锦上添花。
- **软 AMP** 是让先验服务而非压制任务的关键：对扰动后的转移取最高判别分计奖，避免硬模仿牺牲拦截精度，这也是交互段 AMP 与高精度任务冲突的通用解法。
- 感知不是演示外挂：训练内嵌球位/区域估计器并注入 ±5 cm 噪声与 dropout，真机 MoCap 与 D435i 机载相机两种模态都能扑救，但相机模态上区成功率明显低于 MoCap。
- 能力边界要按数字读：仿真 $E_{\text{succ}}\approx 80.9\%$、真机 MoCap **21/30**，属于「宽范围内可用」而非「可靠拦截」；非分区基线还会过拟合一侧，说明路由信号一旦缺失能力就塌回单侧。
- 与 [MoRE #08](./paper-amp-survey-08-more.md) 同为多判别器，但路由信号从用户 gait 命令换成球落点区域，面向的是 **动态物体交互** 而非地形 locomotion；同一框架已泛化到躲球与抓球。
