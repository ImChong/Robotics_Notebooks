...
- **仿真增益：** 相较不含动力学感知表示的基线策略，DAPL 在仿真杂乱场景取物成功率上带来 **>25%** 增益。
...
- **vs 一般学习方法：** 不显式建模接触诱导动力学的学习方法难以处理杂乱堆叠；DAPL 用世界模型提炼接触动力学表示引导 RL，使推 / 拨 / 翻转等 **外在灵巧性** 行为自主涌现，而非依赖预设动作原语。
- **vs 外在灵巧性早期工作：** 相较 Zhu et al.（借助外部力的在手操作）等聚焦抓取内调整的工作，DAPL 将外在灵巧性从「在手」扩展到「杂乱场景取物」。
