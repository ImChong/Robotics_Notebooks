...
type: entity
tags: [cassie, reinforcement-learning, sim2real, reward-design, legged, berkeley]
status: stable
...
**这篇论文真正的研究对象不是某个策略，而是 sim2real 的开发流程本身：它把「写好 reward」还原成奖励、观测与动作语义的多轮共同演化。**

- 起作用的机制是**迭代闭环 + 数据复用**：诊断失败模式 → 改观测/动作/奖励 → 再训练 → 再上机；**DASS** 在策略随机性下抽取确定性片段，使奖励重写后旧策略数据仍可迁移。
- 结果的分量在于**无动力学随机化也完成了迁移**——说明接口与奖励的共同设计能顶掉一部分域随机化的活。
- 最可迁移的工程教训：改 `q_des` 缩放、默认姿态或接触奖励会改变**同一组 Kp/Kd 的有效动力学**；出现「仿真漂亮、真机抖/摔」时先核对动作接口与 PD 表是否同版本文档化，再进增益扫描。
- 边界：本页正文侧重方法结构与交叉引用，量化指标、消融与实机结果需回到原文 PDF。
