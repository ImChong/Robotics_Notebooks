...
**Robot Trains Robot: Automatic Real-World Policy Adaptation and Learning for Humanoids** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：10_Sim-to-Real），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**RTR 的核心主张是「真机 RL 的瓶颈在基础设施而非算法」：用一台 UR5 当老师，把保护、课程、柔顺引导、扰动、判摔、复位六件事全部自动化，长时间低人工监督的真机训练才第一次成立。**

- 真正起作用的是硬件层的课程：Z 向逐步放手是**物理课程**，比纯软件课程更直接地控制难度；自动复位则解掉了真机长训最实际的瓶颈。
- Sim-to-real 侧刻意做得很轻：仿真把动力学压进**单个隐变量**，真机只微调这个隐变量并重训 critic，用最小可调参数量换取适配的高效与稳定。
- 适用边界由「老师」决定：整套方案建立在机械臂能托举并柔顺跟随学生这一前提上（UR5 + F/T 传感 + Toddlerbot 规模），更大更重的人形能否照搬，本页未作承诺。
- 两个真机任务的证明目标不同：把已有行走策略微调到精确速度跟踪，证的是适配效率；从零学会摆动，证的是纯真机技能可学。
- 工程可复现性是这篇的实际卖点：代码（Brax + RSL_RL）、硬件搭建（Toddlerbot + UR5）与复现指引齐全开源，CoRL 2025 录用。
