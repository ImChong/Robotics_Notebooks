...
**GMP 用「冻结的生成式先验 + 稠密轨迹级 guidance」替掉对抗判别器，把自然性问题从「判得准不准」换成「参考生得好不好」。**

- 真正起作用的是 **监督粒度**：AMP 给标量风格分，GMP 给逐帧关节角与关键点目标（$r_{dof} + r_{keypos}$），并把预测参考显式写进策略观测。
- 训练稳定性来自 **先验离线训练、RL 阶段冻结**，规避策略–判别器共训的不稳定。
- 量化上 JFID **0.931** vs PBRS+AMP **2.088**，MELV、JDTW/KDTW 相对 SaW、PBRS、HumanMimic 及 +AMP 基线全面更优。
- 风险随之转移到先验侧：**CVAE 离线质量锁死自然性上限**，分布外命令仍可能参考漂移，靠 command conditioning 与 scheduled sampling 兜底。
- 边界：管线本身平台无关，但 **跨形态需重训 CVAE 与 retarget**；命令区间为 $v_x \in [0, 1.5]$ m/s 一类走跑范围。
- 与 [AMP #01](./paper-amp-survey-01-amp.md) 的对照定位：同解「自然性从哪来」，机制却是 **生成式参考 + 跟踪型 shaping** 而非对抗，是该问题的另一条支路而非改名。
