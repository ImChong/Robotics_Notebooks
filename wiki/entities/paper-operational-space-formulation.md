...
OSF 是任务空间控制的源头。把它与相邻页面对照，能看清它相对经典力控的推进，以及后续框架继承了什么：

| 维度 | 本工作（OSF） | [hybrid-force-position-control](../concepts/hybrid-force-position-control.md) | [Stack of Tasks](./paper-hmi-stack-of-tasks.md) |
|------|---------------|------------------------------------------------------------------------------|-------------------------------------------------|
| 方法族 | 在末端任务空间统一表达运动与力的动力学 | 混合力/位置控制：按方向划分位置环与力环 | 软件化广义逆运动学任务栈 |
| 运动与力如何结合 | 用动态一致映射把任务空间惯量 Λ 映射为关节力矩，二者在同一动力学下 | 用选择矩阵把方向划为位置控制或力控制 | 只处理运动学/速度层，不直接管力 |
| 冗余自由度 | 通过动态一致零空间完成次任务，不扰动主任务 | 非核心关注 | 前一任务雅可比零空间内递归堆叠 |
| 关系/取舍 | 为任务优先级与 WBC 奠基；比纯运动学映射多考虑方向惯量 | 可视为 OSF 在统一动力学下的特例/前驱思路 | 继承操作空间思想但落在速度层，接触力控需下游补齐 |
