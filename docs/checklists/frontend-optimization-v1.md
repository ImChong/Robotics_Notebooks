...
- [x] 首页 Hero 规模数字（知识节点 / 互链关系 / 主路线 / 纵深路线）首次访问 count-up 翻滚；`localStorage` 跨刷新/新标签不重播（兼容旧 `sessionStorage` 标记）；`prefers-reduced-motion` 时直接显示终值。验证脚本 `scripts/verify_hero_stats_countup.cjs`。
---
