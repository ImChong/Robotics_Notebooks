...
| 对照 | 差异读法 |
|------|----------|
| 传统相移干涉（PSI） | 同为「用相移帧解相位」，差别在**参考束从哪来**：传统 PSI 需要稳定的外参考束与更多帧；Coprime-PSI 让物体波与自身的共质数位移副本干涉，8 帧即可，硬件约束松一档 |
| 启发式相位解缠 | 同为解 wrapped 相位，但**保证不同**：启发式方法在噪声/遮挡下失败模式难预测；共质数位移图上的最短路径传播给的是可证明的恢复条件——代价是位移标定必须准，否则路径假设本身就塌了 |
| [立体匹配基础模型](../methods/stereo-matching-foundation-models.md) | **物理量不同，最常被混读**：立体匹配估的是 RGB 场景深度；Coprime-PSI 恢复的是**波前相位**。表格里的 PSNR 不可跨页比，机器人侧要用还得先算「波前 → 深度/点云」的转换成本 |
| [感知坐标后处理](../concepts/perception-coordinate-postprocessing.md) | 提醒读法：波前恢复再准，接到机器人坐标系仍受 4f/SLM 平台标定与外参约束——精度上限常卡在标定而非算法 |
| [目标检测](../methods/object-detection.md) | 同属感知栈但层级不同：本文解决的是**成像之前**（透过散介质把像拿回来），检测是拿到像之后的语义层；散射场景下二者是串联而非并联 |
