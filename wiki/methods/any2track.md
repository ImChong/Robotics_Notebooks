...
## Extreme-RGMT：在 RGMT 上做高动态持续扩展

同作者线的 [Extreme-RGMT](../entities/paper-extreme-rgmt.md)（arXiv:2607.20110）保留动力学条件参考聚合，并加强分支出编码与 FSQ；Stage II 用 **PACE**（已掌握集 consolidation + 困难集 acquisition）与 **STAR**（高优势片段重采样）扩展空翻等技能，同时尽量保住 generalist 跟踪。选型时：要抗扰参考选择看 RGMT/Any2Track；要「同一策略覆盖高动态 + 脏在线 MoCap」看 Extreme-RGMT（截至 2026-07-23 **未开源**）。
...
| RGMT | Robust and Generalized Humanoid Motion Tracking | 动力学条件聚合参考命令的通用全身跟踪器 |
| Extreme-RGMT | Extreme Robust and Generalized Motion Tracking | RGMT 的两阶段高动态持续学习扩展 |
...
- [Extreme-RGMT 论文归档](../../sources/papers/extreme_rgmt_arxiv_2607_20110.md)

## 关联页面

- [Model-Based RL](./model-based-rl.md) — 世界模型在 Any2Track 中的辅助作用。
- [BeyondMimic](./beyondmimic.md) — 同样强调参考轨迹跟踪。
- [Extreme-RGMT](../entities/paper-extreme-rgmt.md) — RGMT 后作：PACE + STAR 高动态扩展。
- [RGMT 论文实体](../entities/paper-hrl-stack-14-robust_and_generalized_humanoid_moti.md) — 42 篇栈策展页。
