...
4. **生态支持**：完美适配 [Isaac Lab](../entities/nvidia-omniverse.md) 和 `legged_gym`，研究者可以快速在仿真中训练策略并部署。

## 在具身智能中的作用

G1 的出现极大地加速了大规模数据的采集。由于其成本低廉，研究者可以构建“机器人机房（Robot Farms）”，利用海量实体机器人通过 [自动化标注](../methods/auto-labeling-pipelines.md) 快速生成训练基础策略（Foundation Policies）所需的真实数据。
