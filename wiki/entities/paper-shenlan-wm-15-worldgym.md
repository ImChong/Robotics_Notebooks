...
**WorldGym 把世界模型的用途定位在「评估」而不是「控制」：用 VLM 奖励 + MC 模拟在想象中给策略打分，价值取决于排名与真机是否一致。**

- 真正的成立条件是 **与真机排名高度相关**，而不是生成画面的观感——只要排序对，就能替代一部分昂贵真机试错。
- 属于 [世界模型 15 项目地图](../overview/world-models-15-open-source-technology-map.md) **路线 03 虚拟沙盒**：强调物理一致性与下游策略增益，而非渲染质量。
- 与 [DriftWorld](./paper-driftworld.md) 可对照：后者走 1-step drifting 快评估 / 推理时搜索的路径。
- 本页为清单索引条目（引用 13，2026-06-02 策展）；量化 benchmark、消融与实机指标以原文 PDF / 项目页为准。
