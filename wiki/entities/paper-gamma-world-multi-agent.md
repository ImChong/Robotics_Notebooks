...
**γ-World 的赌注押在归纳偏置而非数据规模：用正单纯形相位替代 slot ID、用 hub token 替代全连接跨体注意力，使多智能体世界模型的「主体数」成为可外推的维度，而不是每换一个人数就重训。**

- 最能验证这一主张的是 **2 玩家训练 → 4 玩家零样本测试**，它直接检验 SRAE 的置换等价；跨体注意力从 $O(N^2)$ 降到 $O(N)$ 是可扩展性的另一半。
- 实时性来自工程路径而非骨干更快：**双向扩散教师 → 块因果学生蒸馏 + KV cache**，得到约 **24 FPS** 的动作响应流式 rollout。
- 报告口径是 **视频保真 / 动作可控性 / 智能体间一致性**；真实多机协调目前是 Gallery 里的 **定性扩展**，闭环成功率未作主指标。
- 主要风险是把 24 FPS 生成误当物理正确模拟器：**动作–结果对齐** 与 **闭环任务增益** 仍需另测（见 [训练闭环 taxonomy](../overview/robot-world-models-training-loop-taxonomy.md)）；接触力级精度与操纵评测协议需回到全文与代码。
- 与 [WEM](./paper-wem-world-ego-modeling.md) 对照：后者解单主体的 world/ego 解耦与长程导航–操作，本页解的是 **多主体共享同一世界时的身份与一致性**。
