...
**这是一张选型地图而不是一篇方法论文：它沿「模型类型 × 应用场景 × OOD 泛化」三条轴，把 EBM / 扩散 / 动作值图 / GAN 与 VAE→扩散→流匹配的演进摆进同一个坐标系。**

- 真正的价值在把分散于多篇论文的生成模型族与 LfD 子任务（抓取生成、轨迹生成、代价函数学习）对齐，从而能按任务反查可用模型族。
- 它把落地约束写进了结论而非附注：**实时推理速度**、多模态输入对齐与安全保障仍未解决，所以「生成模型更好」必须连同任务延迟预算与部署安全一起判断。
- 使用边界：综述无单一 benchmark 分数，不能替代 [Diffusion Policy](../methods/diffusion-policy.md) 这类单篇方法的实验协议深读；具体表格与引用链以原文为准。
