...
**Humanoid-LLA 的关键不是「让大模型直接生成动作」，而是先造一个人与人形共用的离散动作词表当接口，再用物理反馈把生成拉回可执行——语义对齐与物理稳定被拆成两步分别解决。**

- 真正起作用的机制是「**统一人-人形动作词表 + 两阶段微调**」：词表把高层语言语义与物理可控的底层控制连起来，监督式动作思维链先学映射，物理反馈 RL 再补物理保真。
