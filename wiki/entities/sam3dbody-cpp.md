...
**SAM3DBody-cpp**（[AmmarkoV/SAM3DBody-cpp](https://github.com/AmmarkoV/SAM3DBody-cpp)）是 [SAM 3D Body](./sam-3d-body.md) 的 **独立 C++ 推理引擎**：运行时 **不依赖 Python/PyTorch**（ONNX Runtime + ggml），把单目 RGB 图像或视频转为 **每人一份 MHR 姿态参数、相机平移、可选 18439 顶点网格与 70 个 3D 关键点**，并可导出 **标准 BVH** 供 [Blender](./blender.md) / DCC 与 [Motion Retargeting](../concepts/motion-retargeting.md) 使用。
