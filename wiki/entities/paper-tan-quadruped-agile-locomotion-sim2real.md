...
- **本页为索引级节点**（FreeDof 44 篇梳理 [11/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页要点是「多阶段校准 + 随机化 + RL 训练；强调工程模块顺序」，对应证据是逐步加入校准 / 随机化 / 延迟建模各模块后的真机迁移成功率阶梯——**消融顺序本身就是结论**。
...
| **横比口径** | 「模块顺序」结论可迁移到其他平台；但具体成功率与文内四足平台绑定，不可直接横比。 |
