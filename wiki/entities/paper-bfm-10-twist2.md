---
type: entity
tags: [paper, bfm, behavior-foundation-model, awesome-bfm-papers]
status: complete
updated: 2026-05-26
summary: "可扩展全身数据采集系统；BFM 进化依赖持续数据管线而非一次性动作集。"
related:
  - ../concepts/behavior-foundation-model.md
  - ../overview/bfm-41-papers-technology-map.md
  - ../tasks/teleoperation.md
sources:
  - ../../sources/papers/bfm_awesome_twist2_arxiv_2505_02833.md
  - ../../sources/papers/bfm_awesome_41_catalog.md
  - ../../sources/blogs/wechat_embodied_ai_lab_bfm_41_papers_survey.md
---

# TWIST2

**TWIST2** 收录于 [awesome-bfm-papers](https://github.com/friedrichyuan/awesome-bfm-papers) **第 10/41** 篇，归类为 **02 Goal-conditioned 学习**（2025 · arXiv）。本页为知识库 **策展摘要**；方法细节以论文 PDF 与项目页为准。

## 为什么重要

- 可扩展全身数据采集系统；BFM 进化依赖持续数据管线而非一次性动作集。
- 在 [BFM 41 篇技术地图](../overview/bfm-41-papers-technology-map.md) 的五类问题坐标中，属于 **02 Goal-conditioned 学习** 簇，可与 [Behavior Foundation Model](../concepts/behavior-foundation-model.md) taxonomy 对照阅读。

## 核心信息（索引级）

| 字段 | 内容 |
|------|------|
| 编号 | 10/41 |
| 分组 | 02 Goal-conditioned 学习 |
| 出处 | 2025 · arXiv |
| 论文 | <https://arxiv.org/abs/2505.02833> |
- **代码/项目：** <https://github.com/amazon-far/TWIST2>

## 与其他页面的关系

- 技术地图：[bfm-41-papers-technology-map.md](../overview/bfm-41-papers-technology-map.md)
- BFM 概念：[behavior-foundation-model.md](../concepts/behavior-foundation-model.md)
- 原始 source：[bfm_awesome_twist2_arxiv_2505_02833.md](../../sources/papers/bfm_awesome_twist2_arxiv_2505_02833.md)

## 参考来源

- [bfm_awesome_twist2_arxiv_2505_02833.md](../../sources/papers/bfm_awesome_twist2_arxiv_2505_02833.md) — awesome-bfm 策展摘录
- [bfm_awesome_41_catalog.md](../../sources/papers/bfm_awesome_41_catalog.md) — 41+10 总表
- [wechat_embodied_ai_lab_bfm_41_papers_survey.md](../../sources/blogs/wechat_embodied_ai_lab_bfm_41_papers_survey.md) — 微信公众号编译导读
- 论文：<https://arxiv.org/abs/2505.02833>

## 推荐继续阅读

- [awesome-bfm-papers](https://github.com/friedrichyuan/awesome-bfm-papers) — 完整列表与数据集表
- [A Survey of Behavior Foundation Model](https://arxiv.org/abs/2506.20487) — TPAMI 2025 综述
