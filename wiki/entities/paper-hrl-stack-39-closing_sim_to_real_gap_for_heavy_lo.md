...
**HALO 把重载 sim-to-real 当成建模问题而不是随机化问题：不是靠更强的域随机化糊平负载带来的动力学变化，而是用可微仿真把这部分结构化模型误差显式辨识出来。**

- 真正起作用的是两阶段系统辨识：第一阶段校准名义机器人模型、压掉固有 sim-to-real 差异，第二阶段识别 payload mass distribution、处理带负载后的动力学变化。
- 收益的落点是**重载条件下的零样本迁移稳定性**，而不是空载敏捷性本身——空载表现好的策略拿重物后失效，才是它要解决的场景。
- 本页归在 **05 接触 · 柔顺 · 安全恢复**：这类策略要在接触丰富与长期稳定之间折中，只看单帧姿态会误判成败。
- 本页是策展编译加机制归纳，量化 benchmark、消融与实机指标以原文 PDF / 项目页为准。
- 延伸判断：人形一旦进入搬运与工具使用场景，空载控制器就不够用，负载辨识会从可选项变成基础能力。
