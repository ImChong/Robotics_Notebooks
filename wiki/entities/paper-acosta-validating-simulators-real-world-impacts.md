...
- **本页为索引级节点**（FreeDof 44 篇梳理 [21/44]）：正文固化文内角色与机制要点，**未转存原文实验表**。
- **回原文须核对的证据**：本页要点是「对比三引擎在冲击阶段的轨迹与接触力复现」，对应证据是真机冲击实验与三引擎仿真的轨迹 / 接触力逐项比对曲线——关键看**冲击瞬态段**而非稳态段的误差。
...
| **横比口径** | 三引擎的接触求解器、步长与求解迭代数设置不同；未对齐这些设置时，引擎优劣排名不可搬用。 |
