...
**YOLO v1 的贡献不是刷高精度，而是证明「单网络端到端回归」能把检测推到真·实时且精度仍可接受——它明码标价地用定位精度换延迟，并顺带拿到了全图上下文。**

- 真正的差异化优势是 **看全图**：背景假阳性约为 Fast R-CNN 的 **三分之一**，因此两者互补而非替代——组合可 **+3.2 mAP**，VOC 2012 上达 **70.7 mAP**。
- 速度–精度取舍摆在明处：**63.4 mAP @ 45 FPS**，Fast YOLO **52.7 mAP @ 155 FPS**；对比 Faster R-CNN 的 7 FPS，是「真·实时 vs 准实时」的分界。
- 主要失败模式来自 **v1 的网格设计**：每格仅 2 框 + 1 类、物体须落在格中心附近，对小目标与密集目标吃力；sum-squared error 对大小框的误差权重也不优。
- 泛化是个被低估的附带结论：在 **artwork** 跨域上优于 DPM / R-CNN，与「学到的是全局表征而非局部 patch」一致。
- 今天的落地价值在 **谱系而非 v1 本身**：机载边缘部署走 [Ultralytics](./ultralytics.md) 的当代版本，选型判据是延迟预算与目标尺度，而不是「单阶段一定不如两阶段」。
