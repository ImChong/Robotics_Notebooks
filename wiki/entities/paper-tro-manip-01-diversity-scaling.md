...
**这篇工作把「多样性越多越好」拆成三条可分别验证的轴，结论是多样性有方向而不是标量：任务多样性值得堆，本体与演示者多样性未必。**

- 三轴分解（任务 / 本体 / 演示者）后，**任务多样性** 比在单任务上堆演示更关键，场景多样性对分布偏移鲁棒性尤其重要。
- 反直觉结论直接影响采购与采集预算：**跨本体预训练非必要**，高质量单本体数据在微调时的 scaling 可能优于多本体预训练。
- **演示者多样性可能有害**——人类示教的速度多模态性会 confound 策略学习；针对这一点做分布去偏得到 **GO-1-Pro**，策展导读报告 +15%、等效 2.5× 预训练数据效率。
- 使用边界：本页为策展编译级，具体 benchmark 与消融以 T-RO / arXiv 原文为准；不要把「更多演示者」直接读成「更多多样性收益」。
