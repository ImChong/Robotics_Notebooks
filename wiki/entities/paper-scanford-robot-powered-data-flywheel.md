...
### 模型增益（Qwen2.5-VL-7B 微调）

| 任务 | 微调前 | 微调后 | 增益 |
|------|--------|--------|------|
| 域内书识别 | 32.0% | **71.8%** | **+39.8 pp** |
| 困难 **英文** OCR | — | — | **+21.8 pp** |
| 困难 **中文** OCR | — | — | **+7.2 pp** |

### 数据效率消融

- 约 **1352 张图（≈1.5 h 部署）** 后，域内增益与 OCR 外溢 **双双趋平**——飞轮的启动门槛远低于「先攒几个月遥操作数据」的直觉。

**证据边界：** 单域（图书馆盘点）单实例；对照的是 **同一模型微调前后**，不是与其他数据获取路线同台竞争；任务成功率 **未达 100%**，作者明说单靠微调不够；**未开源**，LiDAR 漂移校正与 catalog 管线细节无法复现。

## 与其他工作对比

「机器人数据从哪来」有几条路，RPDF 的独特处是 **标注由任务结构免费产出**：

| 路线 | 数据从哪来 | 标注成本 | 数据是否落在真实部署分布 | 与 RPDF |
|------|------------|----------|--------------------------|---------|
| **RPDF / Scanford** | **机器人干正事时顺带采** | **≈0**（catalog + RAG + 字符串校验自动产标签） | **是**——本来就在目标环境 | 本页；前提是任务自带 **可校验的结构**（图书馆目录） |
| 遥操作采集（见 [数据采集产业地图](../queries/humanoid-robot-data-collection-landscape.md)） | 人工操作机器人 | **高**（人时 × 场景） | 是，但场景由采集方选定 | 成本结构完全不同；RPDF 不产 **动作** 数据，只产感知标注 |
| 互联网爬取 | 公开语料 | 低 | **否**——正是本文要补的分布空洞（褪色书脊、多语言、遮挡） | 互补：预训练打底，RPDF 补野外尾部 |
| 仿真合成 | 渲染器 | 低 | 否——有 sim2real gap | 正交；RPDF 的数据天然无 gap，但只覆盖它去过的地方 |
| [Arcadia](./paper-arcadia.md) | 部署反馈写回模型 | 低 | 是 | 最近的同类闭环；同属「部署即数据源」思路 |

**边界提醒：** 本文实例只做到 **VLM**（感知侧）。扩展到 [VLA](../methods/vla.md) 是论文自陈的 **未来工作**——动作数据没有「图书馆目录」这种现成的免费校验器，飞轮能不能同样转起来尚未验证。选任务的判据是页内提的 **ZPD**：太难要人工兜底，太易则无增益。
