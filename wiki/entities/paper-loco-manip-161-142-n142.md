...
**这篇的取舍是把地形重建搬进控制回路内部：高度图、步态相位与全身姿态共用一个闭环，而不是让感知模块与运控模块松散串联。**

- 三件套决定成败：下视深度相机加 U-Net 高度图重建、步态相位/频率调节、教师-学生知识迁移，输出关节位置/力矩命令与地形表征。
- 硬件前提明确——需要机身下视深度相机；只有前视或纯本体感知的平台无法照搬这条链路。
- 实时重建质量是单点风险：高度图估错，跟着调整的步态相位也会一并错，闭环耦合放大了感知误差的后果。
- 有公开项目页；本页为清单索引条目，量化 benchmark 与实机指标以原文为准。
