...
- [x] **具身大模型分类学选型闭环知识链 (+2)**：
    ...
    - [x] `wiki/concepts/embodied-fm-latency-generalization-tradeoff.md`（具身大模型实时性 ↔ 泛化能力取舍概念页：明示模型规模、多模态跨度、世界模型推演步长三个旋钮如何共同推高推理时延、压缩控制带宽的可达边界，并把这条边界讲成分层/端到端选型分界的物理根因；配三旋钮代价表、破边界三条工程路线与常见误判速查）。已与 Query 页双向回链（Query ③ 执行层判据注脚 → 本页，本页 related/正文 → Query），不新增 `embodied_fm_crosslink` 预警。
- [x] **具身大模型家族层专题交叉补强**：
    - [x] 在 `wiki/comparisons/vlm-vln-vla-vlx-world-model-taxonomy.md`（P1① 已回链）、`wiki/methods/vla.md`、`wiki/tasks/vision-language-navigation.md`、`wiki/concepts/world-action-models.md`、`wiki/methods/generative-world-models.md` 五页与 P1 新页（`queries/embodied-fm-taxonomy-loop.md`）形成双向回链：后四页各在 `related` 与「关联页面」补入选型闭环 Query 页并标注本页所在层——VLA=③ 动作执行层、VLN=② 空间导航层、WAM=⑤ 世界模型推演层「联合建模」范式、生成式世界模型=⑤「级联预演」范式；Query 页 `related` 早已含全部五页，双向闭合，消除孤儿页。

## P2: 事实库与矛盾检测扩展 (Quantity)

- [x] **事实库扩展**：
    - [x] `schema/canonical-facts.json` 由 220 → **230 条**：新增 10 条具身大模型选型矛盾检测规则（端到端 VLA 泛化 vs 分层 VLN 可解释/可调、显式世界模型预测 vs 无模型反应式实时性、大模型参数量↑ 与控制带宽/推理时延冲突、统一 VLX 通用性 vs 专精分立精度、多模态跨度↑ 与注意力开销、世界模型推演步长↑ 与累积误差、VLM 语义理解 ≠ 可执行动作接口、VLN 仅底盘移动无力控/操作分支、共享 Transformer 底座不免真机数据、模型规模不替代真机动作数据）；逐条经脚本校验对现存 wiki 页有 pos 命中且 0 误报（`make lint` 潜在矛盾 0 个、0 errors）。
