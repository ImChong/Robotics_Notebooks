...
| 对照对象 | 差异要点 |
|----------|----------|
| 传统串行层级 MPC | 传统「先简后繁」串行求解易接口不一致；MHPC 把模型层级放进单一多相滚动时域 TO |
| [HS-DDP](./paper-hs-ddp-legged.md) | HS-DDP 是可配对的混合系统 TO/DDP 求解族；MHPC 侧重问题表述（多保真模型同场耦合） |
| Mini Cheetah cMPC + WBC 工程栈 | 工业常用 cMPC+WBC 分层部署成熟；MHPC 偏规划理论，嵌入实时需裁剪、迁移成本高 |
