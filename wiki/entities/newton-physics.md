...
- **与 NVIDIA 机器人栈对齐**：官方叙事与 **Isaac Sim / Isaac Lab**、**MuJoCo Playground** 兼容；Isaac Lab 侧已有 `feature/newton` 集成探索（3.0 Beta develop 仍在快速开发），适合作为 **OpenUSD / Warp / MuJoCo Warp 与上层学习框架之间的连接器**（见 [训练栈分层地图](../overview/robot-training-stack-layers-technology-map.md)）。
