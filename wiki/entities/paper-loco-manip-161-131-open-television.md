---
type: entity
tags: [paper, loco-manipulation, loco-manip-161-survey, humanoid]
status: complete
updated: 2026-08-09
venue: curated
summary: "Open-TeleVision 主要解决数据闭环：用相机图像/多视角观测、本体状态与关节序列、遥操作/外骨骼数据采集人类操作和机器人状态，再通过ACT/行为克隆模仿学习、扩散策略/流匹配、MM-DiT/Transformer 动作头转成可训练、可复用的关节位置/力矩命令、末端执行器/腕手目标、动作 chunk/token。关键点是把动作生成看成条件生成问题，用扩散或流匹配在多模态动作分布里采样可执"
related:
  - ../overview/humanoid-loco-manip-161-papers-technology-map.md
  - ../overview/loco-manip-161-category-07-data-teleop.md
  - ../overview/loco-manip-161-category-08-hardware-deployment.md
  - ../tasks/loco-manipulation.md
  - ./paper-xrobotoolkit.md
sources:
  - ../../sources/papers/loco_manip_161_survey_131_open-television.md
  - ../../sources/papers/loco_manip_161_survey_139_open-television.md
  - ../../sources/blogs/wechat_embodied_ai_lab_humanoid_loco_manip_161_survey.md
  - ../../sources/papers/humanoid_loco_manip_161_catalog.md
  - ../../sources/papers/xrobotoolkit_arxiv_2508_00097.md
---

# Open-TeleVision

**Open-TeleVision** 收录于 [具身智能研究室 · 人形 Loco-Manip 161 篇长文](https://mp.weixin.qq.com/s/pACh9EhsISiyPGdiiR0C3A) ，在该 survey 中出现 **两次**（第 **131/161** 篇归类 **07 数据采集与遥操作系统**，第 **139/161** 篇归类 **08 硬件平台、感知配置与部署扩展**；策展分类不同，canonical 实体仅此页）。

## 一句话定义

Open-TeleVision 主要解决数据闭环：用相机图像/多视角观测、本体状态与关节序列、遥操作/外骨骼数据采集人类操作和机器人状态，再通过ACT/行为克隆模仿学习、扩散策略/流匹配、MM-DiT/Transformer 动作头转成可训练、可复用的关节位置/力矩命令、末端执行器/腕手目标、动作 chunk/token。关键点是把动作生成看成条件生成问题，用扩散或流匹配在多模态动作分布里采样可执行轨迹。

## 英文缩写速查

| 缩写 | 英文全称 | 简要说明 |
|------|----------|----------|
| Loco-Manip | Loco-Manipulation | 行走与操作动力学耦合的全身任务 |
| WBC | Whole-Body Control | 协调全身关节满足多任务/约束的控制层 |
| VLA | Vision-Language-Action | 视觉-语言-动作多模态策略 |

## 为什么重要

- Open-TeleVision 主要解决数据闭环：用相机图像/多视角观测、本体状态与关节序列、遥操作/外骨骼数据采集人类操作和机器人状态，再通过ACT/行为克隆模仿学习、扩散策略/流匹配、MM-DiT/Transformer 动作头转成可训练、可复用的关节位置/力矩命令、末端执行器/腕手目标、动作 chunk/token。关键点是把动作生成看成条件生成问题，用扩散或流匹配在多模态动作分布里采样可执行轨迹。
- 人形 Loco-Manip 161 篇 **#131/161** · 数据采集与遥操作系统；**#139/161** · 硬件平台、感知配置与部署扩展（同一论文双槽位）。

## 核心信息（索引级）

| 字段 | 内容 |
|------|------|
| 编号 | 131/161 与 139/161（同一论文双槽位） |
| 分组 | 07 数据采集与遥操作系统；08 硬件平台、感知配置与部署扩展 |
| 原文题目 | Open-TeleVision: Teleoperation with Immersive Active Visual Feedback |
| 机构 | UC San Diego1、MIT2、San Diego、and GR- operator are at San Diego (approximately miles away). j: interactions with humans |
| 发表日期 | 2024年7月8日 |
| 论文/项目 | https://robot-tv.github.io/ |

## 核心机制（归纳）

### 策展导读要点

Open-TeleVision 主要解决数据闭环：用相机图像/多视角观测、本体状态与关节序列、遥操作/外骨骼数据采集人类操作和机器人状态，再通过ACT/行为克隆模仿学习、扩散策略/流匹配、MM-DiT/Transformer 动作头转成可训练、可复用的关节位置/力矩命令、末端执行器/腕手目标、动作 chunk/token。关键点是把动作生成看成条件生成问题，用扩散或流匹配在多模态动作分布里采样可执行轨迹。

## 评测与指标（索引级）

- 本条目为 161 篇策展索引级摘录，**未搬运原文量化 benchmark 与实机指标**；评测口径与具体数值以原文 PDF / 项目页为准。
- 评测原始出处：[原文 / 项目页](https://robot-tv.github.io/)（见上方「核心信息」表「论文/项目」一行）。
- 横向评测对照请回到 [分类 hub](../overview/loco-manip-161-category-07-data-teleop.md) 与 [技术地图](../overview/humanoid-loco-manip-161-papers-technology-map.md)。

## 结论

**Open-TeleVision 攻的是数据闭环的入口而不是控制器：用沉浸式主动视觉反馈把人的操作稳定地采下来，剩下的交给模仿学习与动作头。**

- 起作用的是「主动视觉反馈 + 遥操作采集」这一段管线：输出的关节位置/力矩命令、末端执行器/腕手目标与动作 chunk/token 可直接对接 ACT/扩散策略/MM-DiT 动作头。
- 它在 161 篇里占两个槽位（#131 数据采集与遥操作、#139 硬件平台与部署），说明同一套系统既被当数据管线也被当硬件配置引用；canonical 实体仅本页，不要按两条工作理解。
- 延迟不是它的强项：后作 [XRoboToolkit](./paper-xrobotoolkit.md) 在同硬件（ZED Mini→Quest 3）上报告约 **22%** 视频流延迟下降，引用延迟数字应以该文 Table II 为准，不能反推本页策展摘要。
- 2024 年 7 月的工作，在本页记录的后续工作里已被当作对照基线；采集入口做得好并不代表底层 WBC 鲁棒，两者须分开评估。

## 常见误区

1. 161 篇策展条目提供 **地图坐标**；量化 benchmark 与实机指标以原文 PDF / 项目页为准。
2. Loco-manip 单篇工作不自动解决 **底层 WBC 鲁棒性**；须与运控/接触控制对照。
3. **延迟数字勿与后作混用**：[XRoboToolkit](./paper-xrobotoolkit.md)（arXiv:2508.00097）在同硬件（ZED Mini→Quest 3）上相对 Open-TeleVision 报告约 **22%** 视频流延迟下降；对照实验以该文 Table II 为准，不宜反推本页策展摘要。

## 与其他页面的关系

- 技术地图：[humanoid-loco-manip-161-papers-technology-map.md](../overview/humanoid-loco-manip-161-papers-technology-map.md)
- 分类 hub：[loco-manip-161-category-07-data-teleop.md](../overview/loco-manip-161-category-07-data-teleop.md)、[loco-manip-161-category-08-hardware-deployment.md](../overview/loco-manip-161-category-08-hardware-deployment.md)
- 延迟后作对照：[paper-xrobotoolkit.md](./paper-xrobotoolkit.md)
- 语音高层 locomotion + 社交对照：[Immersive Social VR+LLM（NYUAD）](./paper-immersive-social-vr-llm-humanoids.md)
- 原始 source：[loco_manip_161_survey_131_open-television.md](../../sources/papers/loco_manip_161_survey_131_open-television.md)

## 参考来源

- [loco_manip_161_survey_131_open-television.md](../../sources/papers/loco_manip_161_survey_131_open-television.md) — 161 篇策展摘录（#131）
- [loco_manip_161_survey_139_open-television.md](../../sources/papers/loco_manip_161_survey_139_open-television.md) — 161 篇策展摘录（#139）
- [humanoid_loco_manip_161_catalog.md](../../sources/papers/humanoid_loco_manip_161_catalog.md)
- [wechat_embodied_ai_lab_humanoid_loco_manip_161_survey.md](../../sources/blogs/wechat_embodied_ai_lab_humanoid_loco_manip_161_survey.md)
- [xrobotoolkit_arxiv_2508_00097.md](../../sources/papers/xrobotoolkit_arxiv_2508_00097.md) — 同硬件视频流延迟对照实验

## 推荐继续阅读

- [Loco-Manipulation 任务页](../tasks/loco-manipulation.md)
- [XRoboToolkit（论文实体）](./paper-xrobotoolkit.md) — OpenXR 跨平台套件与延迟表
- [Immersive Social VR+LLM](./paper-immersive-social-vr-llm-humanoids.md) — AVP + LLM 语音腿控 + 双向音频社交（未开源）
- [Open-TeleVision 项目页](https://robot-tv.github.io/)
