...
- **补齐 FastSAC 短板：** [FastSAC / FastTD3](../entities/paper-notebook-learning-sim-to-real-humanoid-locomotion-in-15-m.md) 用 ~0.2M 小网络换墙钟，渐近性能受限；FlashSAC 用 **2.5M 参数** 与稳定性机制，在 scaling 下兼顾 **速度 + 上限**。[LooperMuscle](../entities/paper-loopermuscle.md) 则在 **WBT** 上走另一条路：不改网络规模，而用 **语义 MoE + 专家 critic + 配额 replay** 在 ~45 min 追回大部分 PPO 质量。
