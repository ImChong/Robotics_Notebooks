...
**Simulator Adaptation for Sim-to-Real Learning of Legged Locomotion via Proprioceptive Distribution Matching** 收录于 [Robot Learning Paper Notebooks](https://imchong.github.io/Robot_Learning_Paper_Notebooks/index.html)（分类：10_Sim-to-Real），深读笔记已完成。本页是 **笔记摘要**：要点编译自深读笔记，细节以笔记原文与论文 PDF 为准。
...
**这篇走的是「改仿真器」而非「改策略」那条路，真正的解锁点是把相似性度量从逐时刻轨迹换成本体感知的统计分布，从而甩掉时间对齐与动捕/特权传感这两个工程负担。**

- 关键机制是分布匹配：只比较「关节观测 + 动作」的统计分布，再用黑盒优化在这个距离上统一驱动参数辨识、action-delta、残差执行器三类自适应手段。
- 最有说服力的数字是数据量：**不到 5 分钟**真机数据即可显著降低漂移，效果可比肩用特权状态对齐的基线——这决定了它能不能当作现场快速校准来用。
- 校准完成后下游可直接跑标准 RL，流程比域随机化 / 在线适配更清晰；代价是所有 sim-real 差异都必须能被那三类手段表达出来。
- 适用边界与推广：本页验证在腿足运动（含双腿站立行走这类高难行为），方法本身不限四足，「少量本体数据 + 分布匹配」的范式可推广到人形 sim-to-real 校准。
- 落地状态需注意：截至记录日无项目主页、未检索到官方仓库，短期内只能按论文自行复现。
