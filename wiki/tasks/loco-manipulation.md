...
下面 32 条路线按「**控制栈的哪一层被替换、动作与数据从哪来**」分 7 组；分组是本页阅读编排，不是学界公认 taxonomy，每条路线的方法归属仍以其自身描述为准。

| 分组 | 这一组在回答什么 | 代表路线 |
|------|------------------|----------|
| [分层控制与接触规划](#分层控制与接触规划-经典栈-mpc-rl-接口诊断) | 谁给目标、谁做执行，接触序列怎么搜 | 经典分层 / MPC-over-RL / MotionDisco / FARO |
| [感知统一低层控制器](#感知统一低层控制器-地形-载荷与攀爬) | 低层要不要自己看地形与载荷 | PILOT / RPL / LadderMan |
| [冻结先验之上](#冻结先验之上-模仿-残差与在线适配) | 已有全身先验时，任务能力怎么叠加 | VisualMimic / ResMimic / SplitAdapter / CoorDex |
| [生成式与 VLA 全身基础模型](#生成式与-vla-全身基础模型) | 语义指令怎么落到全身动作 | 统一生成式 / VLA 路线 / POT-VLA |
| [世界–动作模型](#世界-动作模型-用未来预测驱动全身动作) | 预测未来能不能换来更好的动作 | DiT4DiT / MotionWAM / ABot-M0.5 / LUCID |
| [数据入口](#数据入口-无机器人示范-视频生成与合成演示) | 一小时可用全身数据从哪来 | ExoActor / BifrostUMI / LEGS / Curr-0 |
| [轮式 / 移动底座对照](#轮式-移动底座对照-学术定义属-mobile-manipulation) | 换成轮式底座后问题重心怎么变 | FastGrasp / 3D-IC |

### 分层控制与接触规划：经典栈、MPC × RL、接口诊断

共同点是**接口写在高层与低层之间**：高层给目标或接触计划，低层做全身执行；难点在接触序列怎么搜、分层接口本身可不可靠。

#### 经典分层路线 (Modular/Hierarchical)
...
- **Cluttered 持物穿越（势场 + dual-agent RL）：** [HOTICE](../entities/paper-hotice.md)（USC，arXiv:2609.25363）— **HOD-PF** 解耦人形/物体避障引导 + 上下身 dual-agent + 75→1 specialist 蒸馏；MuJoCo **88.5%** SR、G1 真机 side/overhead/ground **6/6·5/6·5/6**；代码待发布。
...
#### 分层 policy–GMT 接口基准（HumanoidArena · 双 tracker 扰动诊断）
...
把感知直接塞进低层：单策略吃高程图/多向深度并输出全身指令，给上层 VLA、遥操作或分层 RL 留一个稳健的 loco-manip API。

#### 感知统一低层 LLC（单阶段全身 RL + 高程图）
...
#### 协调 body–hand 潜先验 + 连续 dexterous 残差 RL（CoorDex）
...
#### 复合全身模仿：上身全库 + 下身双 AMP + 多 critic（CWI）
...
把全身动作当成可生成的序列或 token：扩散 / 流匹配负责多模态运动，VLA 负责语义接口，长时程再用对象状态闭环兜底。

#### 统一生成式路线 (Unified Generative)
...
先预测未来（像素、潜表征或技能级宏动力学），再让动作头读这份未来；本组也收明确反对「冻结运动接口 + 未来预测」的对照工作。
...
#### 移动操作 WAM：latent action 桥接 + D-MoT 解耦 + Dream Forcing（ABot-M0.5）
...
#### 潜空间 video-motion 先验 + action expert 接地（Being-M0.7）
...
### 数据入口：无机器人示范、视频生成与合成演示

瓶颈不在控制器而在数据：这一组比的是**一小时可用数据从哪来、怎么对齐到目标本体**。

#### 视频生成驱动路线 (Video-Generation-Driven)
...
#### 混合数据入口周报（ego / 生成 / 仿真 / 触觉 / 跨本体 teleop）
...
#### 可穿戴人类数据 + 三系统耦合单策略 + 世界模型迭代环（HumanEx · Curr-0 · CurrentWorld-0）
...
#### MimicGen 式全身规划合成示范（HumanoidMimicGen · 单 demo → 千级 IL 数据）
...
### 轮式 / 移动底座对照（学术定义属 mobile manipulation）

按上文术语辨析，这两条严格说属 mobile manipulation；因方法论与腿式路线相通，收在本页作对照。

#### 轮式移动全身 RL + 点云抓取引导 + 二值触觉（FastGrasp · 高速灵巧抓取）
...
#### 共享 3D 地图的导航–操作联合路点链（3D-IC · OVMM 规划）
...
分组与上文 **技术路线演进** 对应，便于从某一组的问题直接跳到对应详情页。

### 先读：上游任务与概念

先把 loco-manipulation 放回任务与控制栈的坐标系，再读下面任何一组。
...
对应技术路线的前两组：分层接口、接触规划、低层执行栈与接口诊断。
...
### 全身模仿、视觉 sim2real 与跟踪先验

对应「冻结先验之上」的模仿与视觉 sim2real 一侧：教师蒸馏、跟踪器与真机迁移。
...
同一组的另一侧：在冻结策略上做残差、负载适配，或把接触与触觉显式接进来。
...
### 数据入口：无机器人示范、可穿戴与合成演示

对应「数据入口」组：采集设备、重定向、视频生成与合成数据工厂。
...
严格属 mobile manipulation 的对照工作，以及导航–操作联合规划。
...
体育、维修拆解、社交与情感等专项落点，非本页主线但共享全身协调问题。
