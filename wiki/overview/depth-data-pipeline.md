---
type: overview
tags: [depth, depth-data-pipeline, dataset, motion-retargeting, training-data]
status: complete
updated: 2026-07-31
summary: "训练数据管线纵深汇总：从原始动作捕捉 / 人体视频 → 质量评估 → 重定向 → RL/IL 策略训练输入的端到端选型链路，统一收纳人形参考运动数据集、动作数据质量与重定向相关页面。"
---

# 训练数据管线（纵深汇总）

> **图谱纵深视图**：本页是知识图谱「📦 训练数据 (Data Pipeline)」纵深的统一入口；在 [图谱纵深视图](../../docs/graph.html?depth=data-pipeline) 筛选时，本节点为汇总锚点。

## 一句话定义

**训练数据管线纵深** 关注人形策略训练的**上游数据链路**：从原始动作捕捉 / 人体视频，经**质量评估**与**重定向**，到 RL/IL **策略可用的训练输入**。
...
- **V25 纵深**：本库把分散的数据集实体页、质量与重定向页收成单一图谱视图。
...
- **[动作重定向](./depth-motion-retargeting.md)**：重定向是数据管线的第③段，把参考动作映射到机器人骨架。
- **[WBT](./depth-wbt.md)**：训练数据管线产出的参考动作直接喂给全身跟踪训练。
- **[IL/RL](./depth-learning.md)**：训练范式层决定数据输入形态与质量门槛。
...
- 图谱纵深定义：[docs/depth-filters.js](../../docs/depth-filters.js)（`data-pipeline` 命中规则）
