# Pelican-Sim 1.0: A General World Model Simulator for Embodied Intelligence（arXiv:2609.12036）

> 来源归档

- **arXiv：** <https://arxiv.org/abs/2609.12036>
- **项目页：** https://zoushilong1024.github.io/Pelican-Sim1.0/
- **代码：** https://github.com/Open-X-Humanoid/Pelican-Sim1.0
- **开源状态：** 待核实（步骤 2.5 核查，2026-09-14）
- **入库日期：** 2026-09-14
- **索引来源：** [wechat_embodied_station_11_papers_vla_tamp_planning_2026-09-14.md](../blogs/wechat_embodied_station_11_papers_vla_tamp_planning_2026-09-14.md)
- **一句话说明：** 28 维统一动作空间 + URDF 渲染动作视频 + 稀疏 MoE + 四步 rollout；约百万轨迹上支持数据生成、策略评测、动作选择与策略改进。

**对 wiki 的映射：** [paper-pelican-sim](../../wiki/entities/paper-pelican-sim.md)
