...
**RoboInter1.5 把中间表示从策略内部的技巧升格为一个双向接口：同一套稠密 IR 既正则化 plan-then-execute 的动作空间，又给世界模型提供比语言或电机命令更贴像素的结构条件。**

- 真正的资产是数据与标注链路：**>230k** episode · **571** 场景 · **6** 类臂 · **10+** 类 IR，经 RoboInter-Tool 半自动标注 + 人机校验产出，填的正是「语言 + 低层动作」之间与动作时间对齐的稠密监督空洞。
- 1.5 相对 1.0 的增量是 **RoboInter-World / RoboInter-CV**：控制视频把物体跟踪点与夹爪轨迹渲染到黑底画布，去外观、留时空结构，作为未来 latent 去噪的条件；论文报告 World 预测可提升 VLA 动作精度。
- 复现边界必须讲清：当前社区端唯一跑得通的口径是 **数据 → VLM 训练/评测**；VLA 权重与 World 代码待齐，per-task 成功率无法端到端复现，不要假设 `RoboInterVLA/` 已可训练。
- 跨本体迁移有限度：核心 IR（分割点 / 夹爪轨迹 / 接触帧）是形态相对无关的，比原始电机命令更易做重定向条件，但 DiT Executor 动作头仍需按目标本体重训——IR 不等于可部署关节指令。
- 主要风险来自 IR 选择本身：F-CoT 组合空间大，不按任务裁剪就会把 Planner 噪声灌进 Executor；而 World 终究是像素推演，IR 条件能减幻觉，替不了物理引擎的保证。
