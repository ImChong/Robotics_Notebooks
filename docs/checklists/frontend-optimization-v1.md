...
- [x] 更新记录页 `change-log.html`：超量日先预览 10 条；提供「再展开 10 项 / 展开全部 N 项 / 收起至前 10 项」，视觉沿用箭头+文案样式（PR#1245）。
  ...
  - [x] 图谱页 2D 力模拟稳态（Obsidian 风格）：`velocityDecay`→0.82、`alphaDecay`→0.055；弹簧改为度数加权（`0.65/√minDeg`）；初始/刷新先静默 warmup 140 tick 再以 alpha=0.15 可见落稳；叶序铺开 + 弱向心位置力；3D / mini-graph 同步基础阻尼。
