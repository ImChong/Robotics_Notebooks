...
- **大规模仿真：** >1000 形态 + 1536 随机腿向变体，在平地速度跟踪、10% 腿失效、0–20 kg 负载、≤10 cm 离散障碍四项任务上，均随 η 升高而 **跟踪误差 / COT↓、成功率↑**；固定腿数下 η 仍是主导因子，对称 Thomson 布局位于 Pareto 前沿（详见「仿真：η ↔ 四项任务表现」小节）。
- **实物验证：** 20 腿正十二面体（η≈0.91）在草地 / 沙地 / 树皮 / 铺装 / 窄廊行走，快速自稳定，单 / 多腿失效仍可移动，推拒扰动，边推边跟 1 m 立方体，并完成模拟月重力双墙攀爬。
- **失败分析：** 真机感知 IL 成功率下降主因为 **ToF 过热引发延迟与 20 路不同步**，而非控制策略失效——对未来分布式深度传感共设计具警示意义。

## 结论

**Argus 真正的贡献是把「对称」从几何外形改写成可计算的驱动能力指标 η，并用它当形态设计的排序函数；20 腿实物是这个指标的极限存在证明，而不是结论本身。**

- 起作用的是统一度量 $\eta = a_{\min}/a_{\max}$：它让四旋翼、张拉整体、四足/人形与 Argus 在同一坐标下可比，也拆穿「几何对称即动态对称」——随机腿向可低至 **~0.25**，Thomson 布局才逼近极限。
- 证据是规模化而非单点演示：>1000 形态 + 1536 随机腿向变体上，跟踪误差与 COT 随 η 升高而降、腿失效/负载/障碍成功率随之升，且**固定腿数下 η 仍是主导因子**。
- 收益有饱和点：6→40 腿中 η 与任务指标在 **~16–22 腿**后边际递减，实物取 20 腿正十二面体（η≈0.91）是刚度、布线腔体与制造性的折中，不是「越多越好」。
- 主要失败模式落在感知侧而非控制侧：真机 IL 成功率下降主因是 **20 路 ToF 过热引发延迟与不同步**——这是对分布式深度传感共设计的直接警示。
- 工程可复现性较好：Isaac Gym 训练代码、预训练 checkpoint 与两阶段感知 IL 流程均开源。
- 与 [Digit 人形 RL Locomotion](./paper-digit-humanoid-locomotion-rl.md) 互补定位：后者在既定双足形态上做控制，Argus 先问**形态该长成什么样**。
