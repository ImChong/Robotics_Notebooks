...
**这篇工作的论点是遥操作性能由「最慢的一环」决定，因此它不押注单点算法，而是把感知（免标定动捕）、控制（低延迟运动学流式）、驱动（高带宽摆线执行器）三层同时换掉。**

- 真正起作用的是 **协同**：7 个 IMU 的免标定动捕降低穿戴与标定门槛，运动学流式压端到端延迟，摆线执行器提供高速与抗冲击的物理前提；任一层拖后腿，跟手性都会被拉回去。
