...
论文（§IV）在 **31-DoF LimX Oli**（1.65 m / 50 kg）上以 IsaacLab 仿真加真机验证，评测围绕「复合模仿是否既保上身操作统计、又保下身稳定」展开。

- **对比协议：** 重实现 HOVER\* / FALCON\* / HOMIE\* 作公平基线，统一观测、奖励权重与 PD 增益。CWI 在速度 / 身高 / 上身 keybody 等多指标上综合最优；成功率仅较 FALCON\* 低 **<0.1%**（严格上身跟踪下边界命令偶发不稳），属可接受权衡。
- **关键消融（手端误差 $p_{ee}$ 与风格 DTW $d_{DTW}$）：** 去 multi-critic → $p_{ee}$ **42.9→55.5 mm**、角速度跟踪退化；去蒸馏 → $p_{ee}$ **173.2 mm**（单阶段无法从稀疏双手命令学会操作）；去双 AMP → $d_{DTW}$ **0.45→1.41**（下身风格明显变差）；去 AMASS 上身全库 → $p_{ee}$ **62.3 mm** 并拖累下身跟踪。四项消融共同支撑「数据角色解耦 + 多目标隔离 + 两阶段蒸馏」缺一不可。
- **真机与遥操作：** 拧盖、小件装配、开门、击鼓、长程 pick-place-carry、搬箱等任务由 Meta Quest VR 双手 + 速度/身高命令驱动，无需全身 MoCap；搬箱分析显示腰俯仰随双手前伸自动增大、起立时收回，属 AMASS 上身统计与复合模仿目标涌现，而非显式躯干命令。

> 上述数字取自论文 §IV 与项目页；因训练代码未开放，指标为论文口径、非独立复现。
...
**复合全身模仿的关键是「数据角色解耦 + 多目标隔离 + 两阶段蒸馏」，而不是上下身拆成两个策略。**

1. **上身全库、下身精选** — AMASS 上身保留操作统计；下身只用 walk/squat 专家 clip + 双 AMP，避免全库下肢污染稳定步态。
2. **同一全身 actor 内解耦** — 用奖励分组与 multi-critic（loco / upper / style 分 GAE）隔离冲突目标，而非 FALCON/HOMIE 式双策略。
3. **multi-critic 不可省** — 去掉后手端误差约 **42.9→55.5 mm**，角速度跟踪也退化。
4. **蒸馏才能压到稀疏接口** — Student 仅双手 9D keypoint + 速度/身高；无蒸馏时 $p_{ee}$ 达 **173.2 mm**。
5. **双 AMP 管下身风格** — 去掉后 DTW **0.45→1.41**；walk/squat 分判别器、不混上身参考。
6. **部署要轻** — Meta Quest 双手 + $g_l$ 即可做拧盖/开门/搬箱等；无官方代码，指标以论文口径为准。

## 与其他工作对比

| 维度 | CWI | 全身 MoCap 跟踪（HOVER\* 类） | 上下身双策略 / 分层（FALCON\* / HOMIE\*） | 全库过滤策展（LIMMT / GQS） |
|------|-----|-------------------------------|--------------------------------------------|------------------------------|
| MoCap 数据用途 | **按身体角色解耦**：上身全库、下身精选 | 全库统一跟踪 | 命令采样 / 分层，不强依赖全库 | 全库统一后按质量过滤 |
| 上下身耦合 | 同一全身 actor 内经奖励/critic/数据分组解耦 | 单策略统一跟踪 | 上下身两个策略或分层控制 | 取决于底层控制器 |
| 下身风格先验 | 双 AMP（walk / squat 分判别器） | 隐含于全库参考 | 命令跟踪为主，风格弱约束 | 依赖被保留片段 |
| 冲突目标优化 | **multi-critic PPO**，三组 advantage 分离 | 单 critic，易被对抗/稀疏信号淹没 | 分策略天然隔离但协调难涌现 | 不涉及多目标优化 |
| 部署接口 | 双手 9D keypoint + 速度/身高命令，可 VR 遥操作 | 常需全身参考 | 视具体接口 | 与控制器解耦，聚焦数据侧 |
