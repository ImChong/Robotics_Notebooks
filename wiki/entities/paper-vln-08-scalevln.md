...
**ScaleVLN 给出的是 VLN 的数据规模答案：与其改模型，不如把 HM3D/Gibson 的 1200+ 场景自动变成 490 万条指令-轨迹对，R2R 成功率因此从约 69% 抬到 80%。**

- 真正起作用的是 **全自动数据管线**（路径采样 → 风格迁移 → 指令生成），它把「场景多」转化成「可用监督多」；规模本身不自动等于收益，管线质量才是前提。
- 收益口径必须绑定设定：抬升发生在 **R2R 等离散图 benchmark** 上，而 benchmark 提升不等于真机部署，连续环境（VLN-CE）与离散图设定不可直接混比。
- 本页为策展索引级（ICCV 2023，澳大利亚国立大学 + 上海 AI Lab，归入 **02 算法框架**）：**完整指标与消融以原文 PDF / 项目页为准**。
