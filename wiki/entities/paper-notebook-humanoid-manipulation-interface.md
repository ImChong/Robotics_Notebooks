...
**HuMI 的核心取舍是「宁可让人在采集时迁就机器人，也不缩放轨迹」：用示教阶段的人机闭环 IK 预览换回人–物空间对齐，再靠操作中心 WBC 把这份原始尺度的全身轨迹在 G1 上执行出来。**

- 真正起作用的是采集端与执行端的四个补丁，缺一即崩：**在线 IK 预览**（去掉后求婚 85%→10%）、**变速增强**（拔剑 85%→50%）、**上一目标位姿作 chunk 参考**（投掷 75%→40%）、**盲关键点相对跟踪**（清扫 75%→0%）；它们共同对冲人形 4–6 cm 跟踪残差带来的 action chunk 断裂。
- 收益集中在「遥操作本就难采」的高关节度全身动作：深跪求婚 TWIST2 **零** 可用示范而 HuMI 拿到 50 条；若任务遥操作已经轻松，无机器人路线的边际价值有限。
- 适用边界绑死在 G1：IK 预览与低层控制器均按 G1 运动学配置，论文也承认 WBC **未做到跨任务通用底座**；泛化证据仅深蹲拾瓶 70%（7 环境、350 条训练）。
- 落地风险：依赖 Vive Ultimate ×5 这类专用追踪硬件（受环境纹理与光照影响），不是纯视觉方案；截至 **2026-07-19** 未见代码与权重开源，复现需自建 UMI + Vive + IK 栈。
- 谱系定位：与 [BifrostUMI](./paper-bifrost-umi.md) 的 SKR 缩放、[HALOMI](./paper-halomi-humanoid-loco-manipulation.md) 的「只录头手、下身交给 BFM-Zero 流形控制器」相比，HuMI 选择 **显式记录骨盆与双脚且拒绝缩放**——三者构成「下身参考写多细」的连续谱，而 [TWIST2](./paper-twist2.md) 代表另一端的真机遥操作。
