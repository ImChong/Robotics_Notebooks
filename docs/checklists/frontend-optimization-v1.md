...
完整依据、估时、设计约束与验收见 [工程计划图谱专项](../../plan.md#2026-09-08-图谱动态渲染性能专项)。本次只新增计划；以下实现与真机性能验收均未完成。原 #6 拆为 G1–G6，关联 #9/#24/#35，其他全站任务状态不变。

- [ ] G0 / #35：固定数据和环境，分别记录 tick、绘制、标签、交互、长任务以及 3D draw calls/三角形数；正常 GPU/手机/Safari 缺失时明确未测。
