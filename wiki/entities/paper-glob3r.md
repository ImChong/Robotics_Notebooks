...
| [VGG-T³](./paper-vgg-ttt.md) | **单趟前馈** + TTT 线性全局 attention（千图 ~1 min）；Glob3R 走 **tracks + BA 精炼** 换更高离线精度 |
...
**Glob3R 的定位是「用 3D 基础模型换匹配质量，再把精度交还给经典全局 SfM」——它赢的不是前馈速度，而是离线位姿精度。**

- 真正起作用的是分工：冻结 Pi3X 提供鲁棒几何初值，只训练稠密 warp 匹配头把前馈预测转成可优化 tracks；消融显示完整 BA 相对 Init 大幅抬升，而换成 VGGT / RoMa 匹配头更差更慢——**匹配精修与 BA 缺一不可**。
- 关键结构选择是 **帧级而非窗级**：相对 VGGT-Long / VGGT-SLAM 的窗级 Sim(3) 缝合，Glob3R 用重叠半窗跨窗传播 tracks 再做全局运动平均 + BA，直指 chunk 拼接的尺度与位姿漂移。
- 收益集中在弱纹理与前向运动场景：T&T PSNR 19.56（> COLMAP 18.73、Pi3X 17.62）、TUM 3.2 cm、KITTI 13.21 m、ETH3D 无序 RRA@1/RTA@1 ≈ 91.6/73.3，且下游 novel-view synthesis 有约 +2–3 dB 的可验证提升。
- 适用边界：全局平均 + BA 面向 **离线批处理**，非实时设计；窗口默认固定 N=20，难场景需人工加大；初值很差时对 Pi3X 的依赖仍是开放问题。在线流式几何需求应走 [LingBot-Map](../methods/lingbot-map.md)。
- 工程状态：官方仓为占位（仅 README，Inference / Evaluation 仍 TODO），无法今日复现论文数字；排期上应按「占位仓」管理而非可用工具。
...
- [VGG-T³](./paper-vgg-ttt.md) — 线性时间离线 VGGT 前馈（可作 Glob3R 上游初值）
