...
**E-SOAM 补的是软体机器人「只有驱动、没有本体感知」这块短板：把液态金属传感做进一条能伸长 7 倍的臂尖里，换来的代价是制造复杂度、控制带宽与可复现性。**

- 真正起作用的是 **BEP 机制的物理实现**：偏心内腔近→远时序充气产生弯曲传播，外圈延伸腔径向膨胀叠加轴向伸长，两者合成后有效抓取域达 **1.5 倍臂长**，最大伸长 **7×**——这是把章鱼 OA/TA 协同当作控制时序而非仅仿形态的结果。
- 传感能成立的关键指标是 **EGaIn 在 7× 拉伸下电阻漂移 < 5%、500 次循环无断路**；没有这一条，可伸长与可感知就无法共存于同一段结构。
- 适用边界：气压驱动带宽仅 **2–5 Hz**，适合缓慢适形抓取（鸡蛋、异形物、水下悬浮物），不适合高动态或快速移动目标；水下验证为 **短时浸泡**，深水压差下的长期密封未系统评估。
- 主要局限是 **闭环缺位与复现门槛**：当前为手套遥操模式，传感仅用于反馈而未进入自主控制回路；控制代码与制造 CAD 截至 2026-07-20 未开源，多层硅胶铸模 + EGaIn 注入 + 无线封装的工艺一致性也逊于商业方案。
- 定位上它属于北航文力组「软材料极端形变下的传感/驱动稳定性」脉络，与 [两栖印鱼机器人](./paper-aerial-aquatic-remora-hitchhiking-robot.md)、[深海可变形机器人](./paper-miniature-deep-sea-morphable-robot.md) 互为系列；单臂系统向 [双臂操作](../tasks/bimanual-manipulation.md) 的扩展尚未探索。
